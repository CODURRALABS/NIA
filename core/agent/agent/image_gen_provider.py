...
as ``kind: backend``) or ``~/.nia/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nia tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``nia tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``nia tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$NIA_HOME/cache/images/``, creating parents as needed."""
    from nia_constants import get_nia_home

    path = get_nia_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$NIA_HOME/cache/images/``.
