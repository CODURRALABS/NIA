...
A pre-run snapshot of ``~/.nia/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.nia/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.nia/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_nia_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.nia/cron/jobs.json``)."""
    return get_nia_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.nia/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.nia/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.nia/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.nia/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `nia curator rollback --list` to see available snapshots)",
            None,
        )
