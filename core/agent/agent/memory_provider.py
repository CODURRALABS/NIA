...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - nia_home (str): The active NIA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.nia``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "nia").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'nia memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], nia_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'nia memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``nia_home`` is the active NIA_HOME directory path.
