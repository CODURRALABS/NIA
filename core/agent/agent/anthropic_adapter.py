"""Anthropic Messages API adapter for NIA.

Translates between NIA's internal OpenAI-style message format and
...
# NIA effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    NIA historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by NIA)
    ...
    # 1. NIA-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration NIA configs.
...
# ── NIA-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.nia/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_NIA_OAUTH_FILE = get_nia_home() / ".anthropic_oauth.json"
...
def run_nia_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run NIA-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize NIA with your Claude Pro/Max subscription.")
    print()
...
def read_nia_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read NIA-managed OAuth credentials from ~/.nia/.anthropic_oauth.json."""
    if _NIA_OAUTH_FILE.exists():
        try:
            data = json.loads(_NIA_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read NIA OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See nia-agent#13848.
            #
    ...
    # are preserved.  See nia-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("NIA", "Claude Code")
                text = text.replace("NIA", "Claude Code")
                text = text.replace("nia-agent", "claude-code")
                text = text.replace("Anonymousinsaan & Codurra Labs", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that NIA surfaces in its CLI. We
