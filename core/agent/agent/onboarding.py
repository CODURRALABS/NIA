...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time NIA starts and finds ``~/.openclaw/``.

    Points users at ``nia claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nia claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to NIA, run "
        "`nia claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nia claw cleanup` (renames it to ~/.openclaw.pre-migration — "
