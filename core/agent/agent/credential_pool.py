...
from nia_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``nia model`` / ``nia auth`` which
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("nia_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("nia_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other NIA profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `nia model` / `nia auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `nia auth remove <provider> <N>` is stable across all source types.
    try:
        from nia_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, NIA PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_nia_oauth_credentials

        for source_name, creds in (
            ("nia_pkce", read_nia_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `nia auth add nous --label <name>`).  Fall back to the
        ...
        # NIA auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from nia_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.nia/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `nia auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `nia auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the NIA auth store.  Without this gate the removal is instantly
        ...
        # NIA owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.nia/.env over os.environ — the user's config file is the
    # authoritative source for NIA credentials. Stale env vars from parent
    ...
    # Honour user suppression — `nia auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.nia/.env.
    ...
    try:
        from nia_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.nia/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "nia_pkce"}
        )
    ]
