...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``NIA_PLATFORM`` env var and
    ``NIA_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("NIA_PLATFORM")
            or get_session_env("NIA_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("NIA_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.nia.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_nia_home()}/config.yaml):"]
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.nia/skills/`` and any ``skills.external_dirs`` so the
