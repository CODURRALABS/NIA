...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.nia/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: NIA ↔ models.dev
# ---------------------------------------------------------------------------

# NIA provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → NIA (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_nia_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a NIA provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal NIA traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a NIA provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve NIA ID → models.dev ID
...
def get_model_info(
    ...
    Accepts NIA or models.dev provider ID.  Tries exact match then
