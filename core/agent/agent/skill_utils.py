...
from nia_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``NIA_PLATFORM`` or
            ``NIA_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.nia/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against NIA_HOME, not cwd
        if not p.is_absolute():
            p = (nia_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.nia/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    nia = metadata.get("nia") or {}
    if not isinstance(nia, dict):
        nia = {}
    return {
        "fallback_for_toolsets": nia.get("fallback_for_toolsets", []),
        "requires_toolsets": nia.get("requires_toolsets", []),
        "fallback_for_tools": nia.get("fallback_for_tools", []),
        "requires_tools": nia.get("requires_tools", []),
    }
