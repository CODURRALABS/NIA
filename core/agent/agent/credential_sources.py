"""Unified removal contract for every credential source NIA reads from.

NIA seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.nia/.env
    claude_code   — ~/.claude/.credentials.json
    nia_pkce   — ~/.nia/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``nia auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), nia_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.nia/.env  → clear it
    ...
    """
    from nia_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to NIA.",
            f"  The pool entry is now suppressed — NIA will ignore "
            f"{env_var} until you run `nia auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so NIA stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `nia auth add anthropic` to re-enable if needed.",
    ])


def _remove_nia_pkce(provider: str, removed) -> RemovalResult:
    """~/.nia/.anthropic_oauth.json is ours — delete it outright."""
    from nia_constants import get_nia_home

    result = RemovalResult()
    oauth_file = get_nia_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared NIA Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `nia login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `nia auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the NIA auth store is not enough — _seed_from_singletons() would
    ...
    via ``nia auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from nia_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `nia auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nia auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    NIA stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `nia auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="nia_pkce",
        remove_fn=_remove_nia_pkce,
        description="~/.nia/.anthropic_oauth.json",
    ))
    register(RemovalStep(
