...
def _state_file() -> Path:
    return get_nia_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.nia/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``nia update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``nia curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `nia curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as NIA' background skill CURATOR. This is an "
    ...
    "into ~/.nia/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: nia-config-*, nia-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.nia/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.nia/logs/curator/``)
    ...
    authored skill data in ``~/.nia/skills/``.

    ``ensure_nia_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_nia_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every nia install already has it
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.nia/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.nia/skills/.archive/` for "
            "safety and can be restored via `nia curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.nia/skills/.archive/`. "
            "Restore any via `nia curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `nia curator restore <name>`")
    lines.append("- All archives live under `~/.nia/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `nia curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `nia curator status` can point at it.
        try:
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``nia model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `nia model` → auxiliary
