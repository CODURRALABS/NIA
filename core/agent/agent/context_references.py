...
_SENSITIVE_NIA_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    nia_home = get_nia_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(nia_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(nia_home / rel for rel in _SENSITIVE_NIA_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal NIA path and cannot be attached")
