...
# `export NIA_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in nia_cli/main.py and gateway/run.py) or `NIA_REDACT_SECRETS=true`
# in ~/.nia/.env.
_REDACT_ENABLED = os.getenv("NIA_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across NIA — used by
    ``nia config``, ``nia status``, ``nia dump``, and anywhere
