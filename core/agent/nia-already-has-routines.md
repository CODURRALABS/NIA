# NIA Has Had "Routines" Since March
...
NIA equivalent — works today:
```bash
nia webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working NIA implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | NIA |
|---|---|---|
...
## Things NIA Does That Routines Can't
...
NIA has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
NIA is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install nia-agent
nia setup
```
...
```bash
nia gateway setup    # enable webhooks
nia webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [nia-agent.nousresearch.com/docs/guides/automation-templates](Local Sovereign Hub/docs/guides/automation-templates)

Documentation: [nia-agent.nousresearch.com](Local Sovereign Hub)

GitHub: [github.com/NousResearch/nia-agent](Local Sovereign Environment)

---

*NIA is built by [Anonymousinsaan & Codurra Labs](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
