...
_NIA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (NIA_KANBAN_TASK env set), otherwise
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NIA_KANBAN_TASK env "
    ...
    "pipecat": {
        "description": "Pipecat real-time voice and multimodal framework integration",
        "tools": ["pipecat_voice"],
        "includes": []
    },
    ...
    # ==========================================================================
    # Full NIA toolsets (CLI + messaging platforms)
    #
    ...
    "nia-cli": {
        ...
        "tools": _NIA_CORE_TOOLS,
        "includes": []
    },

    "nia-cron": {
        # Mirrors nia-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nia tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nia-cli; gated by `nia tools`",
        "tools": _NIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nia-email": {
        "description": "Email bot toolset - interact with NIA via email (IMAP/SMTP)",
        "tools": _NIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nia-sms": {
        "description": "SMS bot toolset - interact with NIA via SMS (Twilio)",
        "tools": _NIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nia-gateway": {
        ...
        "tools": [],
        "includes": ["nia-telegram", "nia-discord", "nia-whatsapp", "nia-slack", "nia-signal", "nia-bluebubbles", "nia-homeassistant", "nia-email", "nia-sms", "nia-mattermost", "nia-matrix", "nia-dingtalk", "nia-feishu", "nia-wecom", "nia-wecom-callback", "nia-weixin", "nia-qqbot", "nia-webhook", "nia-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nia-<name>).
        # Gives them _NIA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("nia-"):
            platform_name = name[len("nia-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NIA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
