"""
Timezone-aware clock for NIA.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``NIA_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.nia/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — NIA never
...
from nia_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — NIA requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("NIA_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
