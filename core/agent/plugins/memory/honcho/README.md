...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/nia>
...
```bash
nia honcho setup    # full interactive wizard (cloud or local)
nia memory setup    # generic picker, also works
```

Or manually:
```bash
nia config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.nia/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NIA_HOME/honcho.json` | Profile-local (isolated NIA instances) |
| 2 | `~/.nia/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active NIA profile: `nia` (default) or `nia.<profile>`.
...
| 4 | `per-session` strategy | NIA session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nia-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nia`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-nia-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening nia in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — NIA session ID (timestamp + hex). Every `nia` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple NIA profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nia`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active NIA profile: `nia` (default) or `nia.<profile>` (e.g. `nia -p coder` → host key `nia.coder`).
...
| `NIA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `nia honcho setup` | Full interactive setup wizard |
| `nia honcho status` | Show resolved config for active profile |
| `nia honcho enable` / `disable` | Toggle Honcho for active profile |
| `nia honcho mode <mode>` | Change recall or observation mode |
| `nia honcho peer --user <name>` | Update user peer name |
| `nia honcho peer --ai <name>` | Update AI peer name |
| `nia honcho tokens --context <N>` | Set context token budget |
| `nia honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nia honcho map <name>` | Map current directory to a session name |
| `nia honcho sync` | Create host blocks for all NIA profiles |
