...
Handles: nia honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all NIA profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'nia honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'nia honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `nia update` -- no output, no exceptions.
    """
    try:
        from nia_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current NIA profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $NIA_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_nia_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives NIA persistent cross-session memory.")
    ...
    nia_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'nia honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = nia_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        nia_host["peerName"] = new_peer

    current_ai = nia_host.get("aiPeer") or cfg.get("aiPeer", "nia")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        nia_host["aiPeer"] = new_ai

    current_workspace = nia_host.get("workspace") or cfg.get("workspace", "nia")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        nia_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = nia_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        nia_host["observationMode"] = new_obs
    else:
        nia_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(nia_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        nia_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        nia_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = nia_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        nia_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = nia_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in ("none", "uncapped", "no limit"):
        nia_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                nia_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(nia_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            nia_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        nia_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        nia_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in ("minimal", "low", "medium", "high", "max"):
        nia_host["dialecticReasoningLevel"] = new_reasoning
    else:
        nia_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = nia_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        nia_host["sessionStrategy"] = new_strat

    nia_host["enabled"] = True
    nia_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        nia_config = load_config()
        nia_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(nia_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: nia config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    nia honcho status     -- show full config")
    print("    nia honcho mode       -- change recall/observation mode")
    print("    nia honcho tokens     -- tune context and dialectic budgets")
    print("    nia honcho peer       -- update peer names")
    print("    nia honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active NIA profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: nia honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'nia honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: nia honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        nia = hosts.get(_host_key(), {})
        user = nia.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = nia.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = nia.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = nia.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    NIA' identity in Honcho. Seed with 'nia honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: nia honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: nia honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = nia.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = nia.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = nia.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. NIA asks Honcho's AI peer a question")
        ...
        print("\n  Set with: nia honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'nia honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'nia honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    nia honcho identity --show        — show both peer representations")
        print("    nia honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → NIA + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → NIA\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives NIA persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  nia honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'nia honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'nia honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  nia honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'nia'")
        print("  with Honcho configured and no prior session history.")
        print("  (NIA calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    nia honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'nia honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In NIA, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'nia honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    nia honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  nia honcho identity <path/to/SOUL.md>")
    ...
    print("    NIA:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    NIA:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    NIA:   Honcho observes every message and updates representations")
    ...
    print("    NIA:   per-session by default — each run gets its own session")
    print("              Map a custom name:  nia honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. nia honcho setup              — configure API key (required)")
        print("  2. nia honcho migrate            — re-run this walkthrough")
    else:
        print("  1. nia honcho status             — verify Honcho connection")
        print("  2. nia                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. nia honcho identity --show    — verify AI peer representation")
        print("  4. nia honcho tokens             — tune context and dialectic budgets")
        print("  5. nia honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'nia memory setup'...\n")
        from nia_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``nia honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``nia honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to nia memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to NIA Honcho",
    )
