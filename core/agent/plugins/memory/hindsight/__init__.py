...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: nia)
...
Or via $NIA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so NIA can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $NIA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_nia_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "nia": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "nia"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this NIA config."""
    profile = config.get("profile", "nia")
    return str(profile or "nia")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active NIA profile name (from agent_identity)
      {workspace} — NIA workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``nia-{user}`` with no user becomes ``nia``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between NIA and the User"
    ...
    def save_config(self, values, nia_home):
        """Write config to $NIA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(nia_home) / "hindsight"
    ...
    def post_setup(self, nia_home: str, config: dict) -> None:
        ...
        from nia_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "nia")
        ...
        self.save_config(provider_config, nia_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(nia_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(nia_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "nia"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: nia-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between NIA and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "nia"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "nia"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "nia", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "nia")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between NIA and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When NIA created/used that client on the shared async loop,
