...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with nia-agent)
2. User-installed providers: ``$NIA_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$NIA_HOME/plugins/`` or None if unavailable."""
    try:
        from nia_constants import get_nia_home
        d = get_nia_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$NIA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_nia_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_nia_user_memory.{active_provider}.cli"
    try:
