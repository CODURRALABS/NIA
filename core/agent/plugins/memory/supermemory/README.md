...
Config file: `$NIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `nia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nia-{identity}` → `nia-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per NIA profile:

```json
{
  "container_tag": "nia-{identity}"
}
```

For a profile named `coder`, this resolves to `nia-coder`. The default profile resolves to `nia-default`. Without `{identity}`, all profiles share the same container.
