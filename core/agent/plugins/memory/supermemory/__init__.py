...
def _load_supermemory_config(nia_home: str) -> dict:
    config = _default_config()
    config_path = Path(nia_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, nia_home: str) -> None:
    config_path = Path(nia_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nia memory setup`.
        # All other options are documented for $NIA_HOME/supermemory.json
    ...
    def save_config(self, values, nia_home):
        ...
        _save_supermemory_config(sanitized, nia_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from nia_constants import get_nia_home
        self._nia_home = kwargs.get("nia_home") or str(get_nia_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nia_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "nia", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "nia_memory", "target": target, "type": "explicit_memory"},
