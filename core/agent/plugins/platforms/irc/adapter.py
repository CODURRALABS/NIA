"""
IRC Platform Adapter for NIA.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the NIA.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "nia-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :NIA")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :NIA shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: nia_, nia_1, nia_2...
...
def check_requirements() -> bool:
    ...
    # nia setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `nia gateway setup` flow for the IRC platform.

    Lazy-imports ``nia_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect NIA to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #nia — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.nia/.env")
    print_info("Restart the gateway for changes to take effect: nia gateway restart")
