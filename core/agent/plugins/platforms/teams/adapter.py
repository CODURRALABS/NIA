"""
Microsoft Teams platform adapter for NIA.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "NIA"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        nia_action = data.get("nia_action", "")
        session_key = data.get("session_key", "")

        if not nia_action or not session_key:
        ...
        choice = choice_map.get(nia_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="nia_approve",
                    data={**btn_data_base, "nia_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="nia_approve",
                    data={**btn_data_base, "nia_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="nia_approve",
                    data={**btn_data_base, "nia_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="nia_approve",
                    data={**btn_data_base, "nia_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    from nia_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_info,
        print_success,
    ...
    print_info("  teams app create --name \"NIA\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.nia/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       nia gateway restart")
