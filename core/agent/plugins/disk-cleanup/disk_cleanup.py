"""disk_cleanup — ephemeral file cleanup for NIA.
...
Rules:
  ...
  - empty dirs    → always delete (under NIA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NIA_HOME and /tmp/nia-*
Never touches: ~/.nia/logs/ or any system directory.
"""
...
try:
    ...
    def get_nia_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NIA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nia").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NIA_HOME/logs/``."""
    return get_nia_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NIA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NIA_HOME or ``/tmp/nia-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nia_home = get_nia_home()
    try:
        path.resolve().relative_to(nia_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nia-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nia-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NIA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NIA_HOME (but leave NIA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "nia-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(nia_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == nia_home:
                continue
            try:
                rel_parts = dirpath.relative_to(nia_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nia_home = get_nia_home()
    try:
        rel = path.resolve().relative_to(nia_home)
        ...
        # Path isn't under NIA_HOME (e.g. /tmp/nia-*) — fall through.
        pass
