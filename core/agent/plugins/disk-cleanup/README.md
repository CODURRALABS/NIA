...
Auto-tracks and cleans up ephemeral files created during NIA
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NIA_HOME` and `/tmp/nia-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NIA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NIA_HOME` or `/tmp/nia-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NIA_HOME/disk-cleanup/` is itself excluded
- `$NIA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
