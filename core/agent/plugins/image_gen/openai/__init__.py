...
three virtual model IDs so the ``nia tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `nia tools` → Image "
                    "Generation → OpenAI to configure, or `nia setup` "
                    "to add the key."
                ),
