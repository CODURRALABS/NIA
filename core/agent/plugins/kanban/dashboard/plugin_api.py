...
``nia_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
bridge in ``nia_cli/web_server.py``. If you run the dashboard with
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from nia_cli import web_server as _ws
    except Exception:
...
def get_board(
    ...
    through to the active board (``NIA_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from nia_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``nia kanban
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.nia/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.nia/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in nia_cli/web_server.py.
