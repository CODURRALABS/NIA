...
nia meet install                 # pip deps + Chromium (transcribe only)
nia meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
nia meet auth                    # optional; skips guest-lobby wait
nia meet setup                   # preflight checks
```

`nia meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or NIA_MEET_REALTIME_KEY in ~/.nia/.env
```
...
pip install playwright websockets && python -m playwright install chromium
nia plugins enable google_meet
nia meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
nia meet node approve my-mac ws://<mac-ip>:18789 <token>
nia meet node ping my-mac                   # confirm reachable
```

Run `nia meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A NIA bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `nia meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `NIA_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `nia_meet_src`). |
...
Local:
```
$NIA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$NIA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `nia meet node approve`.
