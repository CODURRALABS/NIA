"""`nia meet node ...` subcommand tree.

Wired into the existing ``nia meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``nia meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="nia-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             nia meet node approve <name> ws://<host>:{args.port} {token}")
        try:
