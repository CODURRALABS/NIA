...
Launched by ``nia meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$NIA_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``nia meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_nia_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "nia-meet-node",
    ) -> None:
        self.host = host
        self.port = port
