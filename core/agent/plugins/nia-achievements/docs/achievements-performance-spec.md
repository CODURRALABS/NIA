# NIA Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nia-achievements plugin
...
### `GET /api/plugins/nia-achievements/achievements`
...
### `POST /api/plugins/nia-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nia-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nia-achievements/overview`
...
- Persist plugin data under:
  - `~/.nia/plugins/nia-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nia/plugins/nia-achievements/scan_checkpoint.json`
