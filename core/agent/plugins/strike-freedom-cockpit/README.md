...
1. **Theme** — copy the theme YAML into your NIA home:

   ```
   cp theme/strike-freedom.yaml ~/.nia/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.nia/plugins/`:

   ```
   cp -r . ~/.nia/plugins/strike-freedom-cockpit
   ```
