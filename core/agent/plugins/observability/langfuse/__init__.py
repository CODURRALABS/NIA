"""langfuse — NIA plugin for Langfuse observability.

Traces NIA conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the NIA plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``nia plugins enable
observability/langfuse`` or ``nia tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``nia tools`` or ~/.nia/.env):
  NIA_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  NIA_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  NIA_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  NIA_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  NIA_LANGFUSE_RELEASE     - release/version tag
  NIA_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  NIA_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  NIA_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("NIA_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the NIA plugin system —
    ...
    public_key = _env("NIA_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("NIA_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("NIA_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("NIA_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("NIA_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("NIA_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid NIA_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("NIA_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older NIA branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current NIA also has a turn-scoped
    ...
    # messages is a list only for legacy NIA branches that fired
    # pre_llm_call with API messages directly. Current NIA fires
...
def register(ctx) -> None:
    ...
    # NIA versions.  pre_api_request / post_api_request fire per API
