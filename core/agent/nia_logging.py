"""Centralized logging setup for NIA.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nia/logs/`` (profile-aware via ``get_nia_home()``).
...
from nia_constants import get_config_path, get_nia_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nia_session_injector", False):
    ...
    _session_record_factory._nia_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``nia logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the NIA logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nia_home
        Override for the NIA home directory.  Falls back to
        ``get_nia_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nia_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nia_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the nia group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from nia_cli.config import is_managed
