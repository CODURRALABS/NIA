...
# (nia-* prefixed), and scenario toolsets.
#
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.nia/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'nia auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (nia_cli/config.py load_config()) so that
