#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the NIA Skills Hub.
...
Used by nia_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            nia_meta = metadata.get("nia", {})
            if isinstance(nia_meta, dict):
                tags = nia_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Anonymousinsaan & Codurra Labs) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.nia/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# NIA centralized index source
# ---------------------------------------------------------------------------

NIA_INDEX_URL = "Local Sovereign Hub/docs/api/skills-index.json"
NIA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "nia-index.json"
NIA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_nia_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for NIA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if NIA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - NIA_INDEX_CACHE_FILE.stat().st_mtime
            if age < NIA_INDEX_TTL:
                return json.loads(NIA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(NIA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("NIA index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("NIA index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        NIA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        NIA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class NIAIndexSource(SkillSource):
    """Skill source backed by the centralized NIA Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nia-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nia-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        NIAIndexSource(auth=auth), # Centralized index (search + resolved install paths)
