...
This module provides the glue:
    - ``NIATokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "NIA"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses NIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``NIA_HOME/mcp-tokens/``
    """
    try:
        from nia_constants import get_nia_home
        base = Path(get_nia_home())
    except ImportError:
        base = Path(os.environ.get("NIA_HOME", str(Path.home() / ".nia")))
...
# ---------------------------------------------------------------------------
# NIATokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------
...
class NIATokenStorage:
    ...
    File layout::
        ...
        NIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        NIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # NIA records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to NIA.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "NIA")
    scope = cfg.get("scope")
