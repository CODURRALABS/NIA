...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NIA_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $NIA_HOME.
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bnia\s+gateway\s+(stop|restart)\b', "stop/restart nia gateway (kills running agents)"),
    (r'\bnia\s+update\b', "nia update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nia-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nia-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nia|gateway|cli\.py)\b', "kill nia/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nia` but not
    # `kill -9 $(pgrep -f nia)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("NIA_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("NIA_INTERACTIVE")
    is_gateway = os.getenv("NIA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("NIA_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("NIA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("NIA_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("NIA_INTERACTIVE")
    is_gateway = os.getenv("NIA_GATEWAY_SESSION")
    is_ask = os.getenv("NIA_EXEC_ASK")
