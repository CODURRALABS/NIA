...
`mcp_tool.py`, and `nia_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# NIAMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_nia_provider_class() -> Optional[type]:
    ...
    class NIAMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._nia_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`NIATokenStorage` persisting an absolute
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._nia_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return NIAMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_NIA_PROVIDER_CLS: Optional[type] = _make_nia_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`NIAMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``nia mcp remove <name>`` and (indirectly) by
        ``nia mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
