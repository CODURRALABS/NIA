...
Backend is selected during ``nia tools`` setup (web.backend in config.yaml).
When available, NIA can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nia/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nia tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "run `nia tools` and select Nous Subscription as the web provider."
        )
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=nia-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "nia-agent"
    return _exa_client
