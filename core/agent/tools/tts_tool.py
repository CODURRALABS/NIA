...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.nia/config.yaml``. NIA
...
Configuration is loaded from ~/.nia/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``nia_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from nia_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import nia_xai_user_agent
...
def _get_default_output_dir() -> str:
    from nia_constants import get_nia_dir
    return str(get_nia_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.nia/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.nia/config.yaml.
    ...
    try:
        ...
        logger.debug("nia_cli.config not available, using default TTS config")
        return {}
...
# custom voice-cloning scripts, etc.) into NIA without any Python code
...
#           output_format: wav
#
# NIA writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__NIA_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when NIA actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where NIA caches Piper voice models.

    Resolves to ``~/.nia/cache/piper-voices/`` under the active
    NIA_HOME so voice downloads follow profile boundaries.
    """
    from nia_constants import get_nia_dir
    root = Path(get_nia_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.nia/config.yaml (tts: section).
    ...
    platform = get_session_env("NIA_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'nia-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run nia setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'nia setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'nia tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_nia_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
