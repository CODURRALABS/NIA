...
LOCAL_STT_COMMAND_ENV = "NIA_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "NIA_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set NIA_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="nia-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("NIA_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
