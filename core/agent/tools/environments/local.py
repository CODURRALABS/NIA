...
# NIA-internal env vars that should NOT leak into terminal subprocesses.
_NIA_PROVIDER_ENV_FORCE_PREFIX = "_NIA_FORCE_"
...
_NIA_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter NIA-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_NIA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _NIA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_NIA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_NIA_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _NIA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("NIA_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. NIA requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set NIA_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_NIA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_NIA_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _NIA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {NIA_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, NIA trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._nia_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_nia_pgid", None)
                    if pgid is None:
                        raise
