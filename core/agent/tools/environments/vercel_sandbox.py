...
Uses the Vercel Python SDK to run commands in cloud sandboxes through NIA'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``NIA_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_nia_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.nia"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_nia = (
            "/.nia"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.nia"
        )
        archive_member = remote_nia.lstrip("/")
        remote_tar = f"/tmp/.nia_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
