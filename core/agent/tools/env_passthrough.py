...
def _is_nia_provider_credential(name: str) -> bool:
    """True if ``name`` is a NIA-managed provider credential (API key,
    token, or similar) per ``_NIA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-NIA API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _NIA_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _NIA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are NIA-managed provider credentials (from
    ``_NIA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a NIA-managed
    ...
    Non-NIA third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_nia_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register NIA provider "
                "credential %r (blocked by _NIA_PROVIDER_ENV_BLOCKLIST). "
