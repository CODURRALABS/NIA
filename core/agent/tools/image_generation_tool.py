...
selectable via ``nia tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `nia tools` → Image Generation."
            ) from exc
        raise
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping nia with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `nia plugins list` to see "
