...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.nia",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``NIA_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside NIA_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    nia_home = _resolve_nia_home()

    # Reject absolute paths — they bypass the NIA_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to NIA_HOME)",
            relative_path,
        )
        return False

    host_path = nia_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape NIA_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, nia_home)
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="nia-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching nia_constants.get_nia_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_nia_dir()`` for backward compatibility with old directory layouts.
    """
    from nia_constants import get_nia_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_nia_dir(new_subpath, old_name)
        if host_dir.is_dir():
