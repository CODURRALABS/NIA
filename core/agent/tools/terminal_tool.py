...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or NIA exit
...
# display_nia_home imported lazily at call site (stale-module safety during nia update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of nia directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "nia-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("NIA_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("NIA_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (NIA_INTERACTIVE=1).
    ...
    try:
        os.environ["NIA_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "NIA_SPINNER_PAUSE" in os.environ:
            del os.environ["NIA_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (NIA_INTERACTIVE=1):
    ...
    # interactive NIA password prompt or the sudo -S password-pipe path.
    ...
    # silently without NIA prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and os.getenv("NIA_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so NIA can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, NIASweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.nia/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `nia model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so NIA can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("NIA_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("NIA_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("NIA_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("NIA_SESSION_USER_ID", "")
                        _gw_user_name = _gse("NIA_SESSION_USER_NAME", "")
            ...
            try:
                from nia_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `nia model` "
