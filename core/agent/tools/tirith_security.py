...
it is automatically downloaded from GitHub releases to $NIA_HOME/bin/tirith.
...
def _get_nia_home() -> str:
    """Return the NIA home directory, respecting NIA_HOME env var."""
    return str(get_nia_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nia_home(), ".tirith-install-failed")
...
def _nia_bin_dir() -> str:
    """Return $NIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nia_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_nia_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NIA_HOME/bin/tirith
    ...
    nia_bin = os.path.join(_nia_bin_dir(), "tirith")
    if os.path.isfile(nia_bin) and os.access(nia_bin, os.X_OK):
        _resolved_path = nia_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nia_bin
