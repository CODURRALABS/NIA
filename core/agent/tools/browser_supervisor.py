...
One ``CDPSupervisor`` runs per NIA ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "nia-dialog-bridge.invalid"
...
(() => {
  if (window.__niaDialogBridgeInstalled) return;
  window.__niaDialogBridgeInstalled = true;
  const ENDPOINT = "http://nia-dialog-bridge.invalid/";
