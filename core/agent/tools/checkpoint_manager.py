...
Architecture:
    ~/.nia/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        NIA_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_nia_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("NIA_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal NIA infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "nia@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "NIA Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "NIA_WORKDIR").write_text(
...
#   * orphan:  the ``NIA_WORKDIR`` path no longer exists on disk
...
def _read_workdir_marker(shadow_repo: Path) -> Optional[str]:
    """Read ``NIA_WORKDIR`` from a shadow repo, or None if missing/unreadable."""
    try:
        return (shadow_repo / "NIA_WORKDIR").read_text(encoding="utf-8").strip()
...
def prune_checkpoints(
    ...
    * ``delete_orphans=True`` and its ``NIA_WORKDIR`` path no longer
...
def maybe_auto_prune_checkpoints(
    ...
    bounded regardless of how many times nia is invoked per day.
