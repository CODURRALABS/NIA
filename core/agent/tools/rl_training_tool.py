...
# Path to tinker-atropos submodule (relative to nia-agent root)
NIA_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = NIA_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = get_nia_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.nia/.env",
        }, indent=2)
