...
discovers their tools, and registers them into the nia-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.nia/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nia/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            nia_home = os.path.expanduser(
                os.getenv(
                    "NIA_HOME", os.path.join(os.path.expanduser("~"), ".nia")
                )
            )
            candidates = [
                os.path.join(nia_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `nia mcp login {server_name}` (or delete the tokens "
            f"file under ~/.nia/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the NIA config file.
    ...
    ``os.environ`` (which includes ``~/.nia/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves NIA's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the NIA registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``nia tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the NIA registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple nia
