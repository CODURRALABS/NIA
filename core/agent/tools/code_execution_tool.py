...
Lets the LLM write a Python script that calls NIA tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `nia_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `nia_tools.py` with file-based RPC stubs
...
def generate_nia_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the nia_tools.py stub module.
...
"""Auto-generated NIA tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["NIA_RPC_SOCKET"])
...
"""Auto-generated NIA tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("NIA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "nia_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated nia_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/nia_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_nia_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/nia_tools.py", tools_src)
        ...
        env_prefix = (
            f"NIA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("NIA_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with nia_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="nia_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"nia_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated nia_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_nia_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "nia_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["NIA_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the nia-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from nia_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _nia_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _nia_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # NIA_TIMEZONE is an internal NIA setting and must not leak
        # into child processes.
        _tz_name = os.getenv("NIA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("NIA_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {NIA_HOME}/home/ when that directory exists.
        from nia_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.nia/.env')).
...
def _load_config() -> dict:
    ...
    ``nia --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (nia-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``nia tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and nia-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.nia/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call NIA tools programmatically. "
        ...
        f"Available via `from nia_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into nia_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from nia_tools import {import_str}` "
