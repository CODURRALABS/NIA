...
running under the dispatcher (env var ``NIA_KANBAN_TASK`` set). A
normal ``nia chat`` session sees **zero** kanban tools in its schema.

Why tools instead of just shelling out to ``nia kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``nia kanban complete …``
   inside the container, where ``nia`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.nia/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``nia kanban …``), the dashboard
(``nia dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``NIA_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``nia chat`` without the kanban toolset see zero
    kanban tools. Workers spawned by the kanban dispatcher (gateway-
    embedded by default) and orchestrator profiles with the kanban
    toolset enabled see all seven.
    """
    if os.environ.get("NIA_KANBAN_TASK"):
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("NIA_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``NIA_KANBAN_TASK`` set
    ...
    ``NIA_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set NIA_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = args.get("author") or os.environ.get("NIA_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("NIA_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("NIA_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to NIA_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to NIA_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
