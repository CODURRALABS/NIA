...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    nia_node_bin = str(nia_home / "node" / "bin")
    return [nia_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-nia_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed nia processes — this way every clean nia exit sweeps
    ...
    # Sweep orphans from other crashed nia processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live nia processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current nia PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live nia process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning nia process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced nia PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent nia instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-nia_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live nia process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, NIA-managed
    ...
    # Build an extended search PATH including NIA-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this nia PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = nia_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_nia_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/nia-agent:latest")
                else:
