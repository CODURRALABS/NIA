...
~/.nia/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `nia config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.nia/skills/ (single source of truth)
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``nia curator unpin <name>`` (or edit it directly by hand).  This
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`nia curator unpin {name}` if they want the change."
            )
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.nia/skills/) first, then any
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_nia_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "pointing the user to `nia curator unpin <name>`. Don't try to route "
