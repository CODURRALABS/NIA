...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "message": {
                "type": "string",
                "description": "The message text to send. To send an image or file, include MEDIA:<local_path> (e.g. 'MEDIA:/tmp/nia/cache/img_xxx.jpg') in the message — the platform will deliver it as a native media attachment."
            }
        },
        "required": []
    }
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nia/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nia/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: nia config set {platform_name.upper()}_HOME_CHANNEL <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("NIA_SESSION_PLATFORM", "cli")
                user_id = get_session_env("NIA_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("NIA_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("NIA_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("NIA_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"nia_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'nia-agent[feishu]'"}
