"""
NIA MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (NIA-specific extra)

Usage:
    nia mcp serve
    nia mcp serve --verbose
...
logger = logging.getLogger("nia.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NIA_HOME."""
    try:
        from nia_constants import get_nia_home
        return get_nia_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NIA_HOME", Path.home() / ".nia")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nia_constants import get_nia_home
        directory_file = get_nia_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NIA_HOME", Path.home() / ".nia")
...
class EventBridge:
    ...
    This is the NIA equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from nia_constants import get_nia_home
            db_file = get_nia_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NIA_HOME", Path.home() / ".nia")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the NIA MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nia",
        instructions=(
            "NIA messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the NIA MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
