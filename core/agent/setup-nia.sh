...
# 5. Symlinks the 'nia' CLI command into a user-facing bin dir
...
# ============================================================================
# PATH setup — symlink nia into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nia command..."

NIA_BIN="$SCRIPT_DIR/venv/bin/nia"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NIA_BIN" "$COMMAND_LINK_DIR/nia"
echo -e "${GREEN}✓${NC} Symlinked nia → $COMMAND_LINK_DISPLAY_DIR/nia"
...
# ============================================================================
# Seed bundled skills into ~/.nia/skills/
# ============================================================================

NIA_SKILLS_DIR="${NIA_HOME:-$HOME/.nia}/skills"
mkdir -p "$NIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nia/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NIA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nia status        # Check configuration"
if is_termux; then
    echo "  nia gateway       # Run gateway in foreground"
else
    echo "  nia gateway install # Install gateway service (messaging + cron)"
fi
echo "  nia cron list     # View scheduled jobs"
echo "  nia doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nia_cli.main setup
fi
