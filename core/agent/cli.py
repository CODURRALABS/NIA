#!/usr/bin/env python3
"""
NIA CLI - Interactive Terminal Interface

A beautiful command-line interface for the NIA, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills nia-agent-dev,github-auth
...
os.environ["NIA_QUIET"] = "1"  # Our own modules
...
from nia_cli.banner import _format_context_length, format_banner_version_label
...
from nia_cli.browser_connect import (
...
load_nia_dotenv(nia_home=_nia_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.nia/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If NIA_IGNORE_USER_CONFIG=1 is set (via ``nia chat --ignore-user-config``),
    the user config at ``~/.nia/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({NIA_HOME}/config.yaml)
    user_config_path = _nia_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("NIA_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain NIA, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me NIA - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and NIACLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `nia model`).
    ...
    # Normalize config key: the new config system (nia_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && nia` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _NIA_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_NIA_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["NIA_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.nia/logs/.
...
try:
    from nia_cli.skin_engine import init_skin_from_config
...
from nia_cli.banner import build_welcome_banner
from nia_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from nia_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run nia -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"nia-{short_id}"
    branch_name = f"nia/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`nia_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from nia_constants import get_nia_home as _get_nia_home
        _nia_home_maint = _get_nia_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_nia_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`nia_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``nia/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("nia-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``nia/nia-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``nia -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("nia/nia-") or b.startswith("pr-"))
    ]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from nia_cli.skin_engine import get_active_skin
...
from nia_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``NIACLI._busy_command()``.
        """
        yield self

# ASCII Art - nia-agent logo (full width, single line - requires ~95 char terminal)
NIA_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - NIA Caduceus (compact, fits in left panel)
NIA_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS NIA - AI Agent Framework"
        tiny_line = "⚕ NOUS NIA"
    else:
        agent_name = _skin.get_branding("agent_name", "NIA") if _skin else "NIA"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Anonymousinsaan & Codurra Labs[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from nia_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.nia/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.nia/config.yaml on first use)
...
class NIACLI:
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from nia_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("NIA_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("NIA_MAX_ITERATIONS"):
            self.max_turns = int(os.getenv("NIA_MAX_ITERATIONS"))
        else:
            self.max_turns = 90
        ...
        # by `nia chat --ignore-rules` in nia_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("NIA_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("NIA_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all NIA processes for this NIA_HOME.
        ...
        self._history_file = _nia_home / ".nia_history"
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'NIA'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from nia_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from nia_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from nia_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from nia_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ NIA")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: nia setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: nia setup")
            return False
        ...
        # `nia chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `nia auth add openai-codex`
        # without `nia model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (nia sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   NIA needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous NIA LLM (not agentic)
        from nia_cli.model_switch import is_nous_nia_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_nia_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Anonymousinsaan & Codurra Labs NIA 3 & 4 models are NOT agentic and are not "
                "designed for use with NIA.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ NIA: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ NIA: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.nia/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from nia_cli.clipboard import save_clipboard_image

        img_dir = get_nia_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: nia --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for NIA config/state.
        ...
        if subcmd in ("list", "ls"):
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_nia_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from nia_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run nia chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'nia setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from nia_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from nia_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with NIA!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in nia_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from nia_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("NIA_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `nia sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from nia_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `nia sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.nia/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``nia --resume <id>``
        ...
        saved_dir = get_nia_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"nia_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: nia --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from nia_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `nia model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from nia_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from nia_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from nia_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        custom_provs = None
        try:
            from nia_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from nia_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to nia_cli.curator so the CLI and the `nia curator`
        ...
        try:
            from nia_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to nia_cli.skills_hub."""
        from nia_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_nia_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_nia_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in nia_cli/commands.py instead of touching every dispatch site.
        from nia_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit"):
            ...
            from nia_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_nia_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from nia_cli.commands import COMMANDS
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"NIA keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from nia_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_nia_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from nia_cli.config import load_config
        from nia_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("NIA_YOLO_MODE"))
        if current:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while NIA is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while NIA is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while NIA is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from nia_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from nia_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'nia_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from nia_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.nia/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "nia_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _nia_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  nia --resume {self.session_id}")
            if session_title:
                print(f"  nia -c \"{session_title}\"")
            print()
            ...
            try:
                from nia_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from nia_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from nia_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        # Only show when the user explicitly configured Honcho for NIA
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to NIA! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to NIA! Type your message or /help for commands."
            _welcome_color = "#FFF8DC"
        self._console_print(f"[{_welcome_color}]{_welcome_text}[/]")
        # First-time OpenClaw-residue banner — fires once if ~/.openclaw/ exists
        # after an OpenClaw→NIA migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from nia_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from nia_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "NIA")
        ...
        # Dynamic prompt: shows NIA symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("NIA needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'NIA needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("NIA needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = NIACLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``NIA_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("NIA_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: nia setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    NIA CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting NIA Gateway (messaging platforms)...")
    ...
    # Default to nia-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside NIACLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  NIA_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "NIA" box, no tool-gen
