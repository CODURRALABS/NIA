...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nia-agent = self'.packages.default;
      niaVenv = nia-agent.niaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nia-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${niaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nia-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "nia-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nia-agent}/bin/nia || (echo "FAIL: nia binary missing"; exit 1)
          test -x ${nia-agent}/bin/nia-agent || (echo "FAIL: nia-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nia-agent}/bin/nia version 2>&1 | grep -qi "nia" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nia-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nia nia-agent nia-acp; do
            test -x ${nia-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nia-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nia --help ==="
          ${nia-agent}/bin/nia --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nia-agent}/bin/nia --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nia-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nia-agent}/share/nia-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${nia-agent}/share/nia-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NIA_BUNDLED_SKILLS" ${nia-agent}/bin/nia || \
            (echo "FAIL: NIA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NIA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nia-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nia-agent}/share/nia-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nia-agent}/share/nia-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NIA_BUNDLED_PLUGINS" ${nia-agent}/bin/nia || \
            (echo "FAIL: NIA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NIA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "nia-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nia-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nia-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${nia-agent}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "NIA_TUI_DIR" ${nia-agent}/bin/nia || \
            (echo "FAIL: NIA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NIA_TUI_DIR set in wrapper"
        ...
        # Verify NIA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        nia-node = pkgs.runCommand "nia-node-version" { } ''
          set -e
          echo "=== Checking NIA_NODE in wrapper ==="
          grep -q "NIA_NODE" ${nia-agent}/bin/nia || \
            (echo "FAIL: NIA_NODE not set in wrapper"; exit 1)
          echo "PASS: NIA_NODE present in wrapper"

          NIA_NODE=$(sed -n "s/^export NIA_NODE='\(.*\)'/\1/p" ${nia-agent}/bin/nia)
          test -x "$NIA_NODE" || (echo "FAIL: NIA_NODE=$NIA_NODE not executable"; exit 1)
          echo "PASS: NIA_NODE executable at $NIA_NODE"

          NODE_MAJOR=$("$NIA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All NIA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NIA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nia-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NIA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NIA_MANAGED guards ==="
          check_blocked "config set" ${nia-agent}/bin/nia config set model foo
          check_blocked "config edit" ${nia-agent}/bin/nia config edit
        ...
        extra-python-packages = let
          ...
          niaWithExtra = nia-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nia-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${niaWithExtra}/bin/nia || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${niaWithExtra}/bin/nia || \
          ...
          if grep -q "PYTHONPATH" ${nia-agent}/bin/nia; then
        ...
        in pkgs.runCommand "nia-config-roundtrip" {
          ...
          merge_and_load() {
            local nia_home="$1"
            export NIA_HOME="$nia_home"
            ${configMergeScript} ${nixSettings} "$nia_home/config.yaml"
            ${niaVenv}/bin/python3 -c '
import json, sys
