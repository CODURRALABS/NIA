# nix/nixosModules.nix — NixOS module for nia-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nia runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NIA_HOME/.env
# and read by nia at startup — no container recreation needed for env changes.
#
# Tool resolution: the nia wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nia-agent;
    effectivePackage = if cfg.extraPythonPackages == [ ] then cfg.package
      else cfg.package.override { inherit (cfg) extraPythonPackages; };
    nia-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nia-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nia-config-attrs";
      description = "NIA YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "nia-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "nia-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NIA_DOC_EOF'\n${value}\nNIA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nia-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nia";
    ...
    # nia user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nia-container-entrypoint" ''
      set -eu

      NIA_UID="''${NIA_UID:?NIA_UID must be set}"
      NIA_GID="''${NIA_GID:?NIA_GID must be set}"

      # ── Group: ensure a group with GID=$NIA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NIA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nia"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NIA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NIA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NIA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NIA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nia"
        TARGET_HOME="/home/nia"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NIA_UID" -g "$NIA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NIA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NIA_UID:$NIA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NIA_HOME is owned by the target user
      if [ -n "''${NIA_HOME:-}" ] && [ -d "$NIA_HOME" ]; then
        chown -R "$NIA_UID:$NIA_GID" "$NIA_HOME"
      fi
      ...
      if [ ! -f /var/lib/nia-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nia-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nia ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nia
        chmod 0440 /etc/sudoers.d/nia
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NIA_UID" --regid="$NIA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NIA_HOME/.env.
    ...
    # Default: /var/lib/nia/workspace → /data/workspace.
  ...
  in {
    options.services.nia-agent = with lib; {
      enable = mkEnableOption "NIA gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nia-agent;
        description = "The nia-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nia";
        description = "State directory. Contains .nia/ subdir (NIA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative NIA config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NIA_HOME/.env at activation time.
          NIA reads this file on every startup via load_nia_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NIA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NIA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nia gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nia user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nia plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. NIA discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nia_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nia (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nia CLI to environment.systemPackages and export
          NIA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nia symlink to the service
            stateDir. These users are automatically added to the nia group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nia-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nia CLI to system PATH and export NIA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nia/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NIA_HOME = "${cfg.stateDir}/.nia";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nia-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the nia user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nia-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nia
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nia is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nia        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nia/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nia/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nia/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nia/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nia/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nia-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nia
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nia ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nia ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nia -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nia/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nia/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nia/$_subdir"
            find "${cfg.stateDir}/.nia/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.nia/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nia/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nia/config.yaml
            chmod 0640 ${cfg.stateDir}/.nia/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nia/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nia/.managed
          chmod 0644 ${cfg.stateDir}/.nia/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nia/.container-mode <<'NIA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nia_bin=${containerDataDir}/current-package/bin/nia
    NIA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nia/.container-mode
            chmod 0644 ${cfg.stateDir}/.nia/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nia/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nia";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nia" ]; then
                  rm -f "${symlinkPath}"
                  echo "nia-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nia -> stateDir/.nia for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nia";
                target = "${cfg.stateDir}/.nia";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nia-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nia/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nia/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nia/auth.json
              fi
            ''}
          ''}
          ...
          # NIA reads $NIA_HOME/.env at startup via load_nia_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nia/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NIA_NIX_ENV_EOF'
    ${envFileContent}
    NIA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nia/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nia/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nia/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nia-agent = {
          ...
          serviceConfig = {
            ...
            # $NIA_HOME/.env by the activation script. load_nia_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nia"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nia group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nia-agent = {
          description = "NIA Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NIA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NIA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NIA_UID="$NIA_UID" \
                --env NIA_GID="$NIA_GID" \
                --env NIA_HOME=${containerDataDir}/.nia \
                --env NIA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nia gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
