/**
 * ChatPage — embeds `nia --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (nia_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__NIA_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `nia dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[nia-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="nia-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
