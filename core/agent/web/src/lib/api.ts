...
const SESSION_HEADER = "X-NIA-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__NIA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the NIA dashboard server");
}
...
export const api = {
  ...
  updateNIA: () =>
    fetchJSON<ActionResponse>("/api/nia/update", { method: "POST" }),
