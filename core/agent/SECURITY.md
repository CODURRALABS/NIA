...
This document outlines the security protocols, trust model, and deployment hardening guidelines for the **NIA** project.

## 1. Vulnerability Reporting

NIA does **not** operate a bug bounty program. Security issues should be reported via [GitHub Security Advisories (GHSA)](Local Sovereign Environment/security/advisories/new) or by emailing **security@nousresearch.com**. Do not open public issues for security vulnerabilities.
...
- **Environment:** Output of `nia version`, commit SHA, OS, and Python version.
...
The core assumption is that NIA is a **personal agent** with one trusted operator.
...
- **Trusted State Access:** Reports that require pre-existing write access to `~/.nia/`, `.env`, or `config.yaml` (these are operator-owned files).
...
- **File permissions:** Run as non-root (the Docker image uses UID 10000); protect credentials with `chmod 600 ~/.nia/.env` on local installs.
...
### Skills & Supply Chain
- **Skill installation:** Review Skills Guard reports (`tools/skills_guard.py`) before installing third-party skills. The audit log at `~/.nia/skills/.hub/audit.log` tracks every install and removal.
...
### Credential Storage
- API keys and tokens belong exclusively in `~/.nia/.env` — never in `config.yaml` or checked into version control.
