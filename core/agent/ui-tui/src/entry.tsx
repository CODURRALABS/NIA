...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `nia-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`nia-tui ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    resetTerminalModes()
    process.stderr.write(`nia-tui: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write('nia-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  onHigh: (snap, dump) => process.stderr.write(dumpNotice(snap, dump))
})

if (process.env.NIA_HEAPDUMP_ON_START === '1') {
