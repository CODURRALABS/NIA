...
// NIA_TUI_THEME=light (or NIA_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('NIA')
...
describe('detectLightMode', () => {
  ...
  it('honors NIA_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NIA_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ NIA_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ NIA_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ NIA_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ NIA_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets NIA_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', NIA_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors NIA_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NIA_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ NIA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', NIA_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', NIA_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses NIA_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ NIA_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ NIA_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ NIA_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // NIA_TUI_THEME=light would flip the base palette and make these
