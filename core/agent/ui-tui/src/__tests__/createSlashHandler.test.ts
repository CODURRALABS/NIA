...
describe('createSlashHandler', () => {
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'nia-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/nia-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: nia-agent-dev')
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/nia_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/nia_conversation_test.json')
    })
  })
