...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'nia-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith(
      "💾 Self-improvement review: Skill 'nia-release' patched"
    )
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.nia/config.yaml'
      },
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `nia model` to select a provider, or run `nia setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
