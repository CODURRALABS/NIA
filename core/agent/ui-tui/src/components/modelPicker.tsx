import { Box, Text, useInput, useStdout } from '@nia/ink'
            ...
            if (r?.disconnected) {
              ...
              setProviders(prev =>
                prev.map(p => p.slug === provider.slug
                  ? { ...p, authenticated: false, models: [], total_models: 0, warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `nia model` to configure' }
                  : p
                )
              )
            }
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run nia model)
          return
        }
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.nia/.env)
        </Text>
