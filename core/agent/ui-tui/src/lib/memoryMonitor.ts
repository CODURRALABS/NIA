...
// Deferred @nia/ink import: loading `@nia/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@nia/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@nia/ink` off the cold-start critical path;
