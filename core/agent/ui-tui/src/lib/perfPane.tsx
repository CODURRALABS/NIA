...
//
// Both gate on NIA_DEV_PERF=1 and dump JSON-lines (default ~/.nia/perf.log,
// override NIA_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// NIA_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@nia/ink'
import { scrollFastPathStats } from '@nia/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.NIA_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.NIA_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.NIA_DEV_PERF_LOG?.trim() || join(homedir(), '.nia', 'perf.log')
