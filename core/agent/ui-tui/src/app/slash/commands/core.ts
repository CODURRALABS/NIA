    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try NIA_TUI_FORCE_OSC52=1 to force the escape sequence; NIA_TUI_DEBUG_CLIPBOARD=1 for details'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `NIA #${i + 1}`
