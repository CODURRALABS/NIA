import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@nia/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'NIA')
  ...
  const cwd = ui.info?.cwd || process.env.NIA_CWD || process.cwd()
