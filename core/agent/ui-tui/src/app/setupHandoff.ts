import type { RunExternalProcess } from '@nia/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nia ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nia: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nia ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
