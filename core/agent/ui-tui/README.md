...
React + Ink terminal UI for NIA. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NIA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.nia/.nia_history` or under `NIA_HOME`.
...
```text
ui-tui/
  packages/nia-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      nia-ink.d.ts    type declarations for @nia/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NIACLI subprocess for slash commands
```
