...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_nia_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.nia/config.yaml (primary user-facing config)
    3. ~/.nia/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_nia_home()
    gw_data: dict = {}
