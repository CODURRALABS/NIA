...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_nia_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /nia prefix). A single regex
            ...
            # manifest (see `nia slack manifest`). In Socket Mode, Slack
            ...
            from nia_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/nia$")

            @self._app.command(_slash_pattern)
            async def handle_nia_command(ack, command):
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own NIA session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nia_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "nia_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "nia_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nia_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "nia_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nia_confirm_once": "once",
            "nia_confirm_always": "always",
            "nia_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nia_approve_once": "once",
            "nia_approve_session": "session",
            "nia_approve_always": "always",
            "nia_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/nia <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/nia
        ...
        if slash_name in ("nia", ""):
            # Legacy /nia <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from nia_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_nia_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/nia <question>" must produce public replies so
