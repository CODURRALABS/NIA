...
# Uses get_nia_home() so each profile gets its own Matrix store.
from nia_constants import get_nia_dir as _get_nia_dir

_STORE_DIR = _get_nia_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("NIA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("NIA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "NIA" become "Agent".
        """
        if not body:
            return ""
