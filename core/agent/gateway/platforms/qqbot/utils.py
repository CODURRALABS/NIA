...
def _get_nia_version() -> str:
    """Return the nia-agent package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("nia-agent")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; NIA/<nia_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; NIA/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    nia_version = _get_nia_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; NIA/{nia_version})"
