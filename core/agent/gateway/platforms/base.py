...
# Audio file extensions NIA recognizes for native audio delivery.
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.nia/.env manually."
)
...
# Default location: {NIA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_nia_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_nia_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_nia_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_nia_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?nia\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+nia[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from nia_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          NIA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          NIA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          NIA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("NIA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("NIA_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("NIA_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _nia_run_generation is set on
