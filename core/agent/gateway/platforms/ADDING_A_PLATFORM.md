...
There are two ways to add a platform to the NIA gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nia/plugins/` with a `PLUGIN.yaml` and
...
requires **zero changes to core NIA code**.
...
Checklist for integrating a platform directly into the NIA core.
...
And add it to the `nia-gateway` composite:

```python
"nia-gateway": {
    "includes": [..., "nia-your-platform"]
}
```
...
## 12. Status Display (`nia_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nia_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nia_cli/ toolsets.py \
  --include="*.py" -l | sort -u
