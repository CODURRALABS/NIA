# OpenAI-Compatible API Server for NIA
...
SSE streaming. By exposing this endpoint, nia-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  nia-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.nia/.env`
2. Run `nia gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with nia-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns nia-agent as a model) |
...
Authorization: Bearer nia-api-key-here
...
But nia-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "nia-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured nia-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of nia-agent's core prompt
...
Once implemented, nia-agent works as a drop-in backend for:
