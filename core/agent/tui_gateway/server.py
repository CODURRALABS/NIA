...
load_nia_dotenv(
    nia_home=_nia_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.nia/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_nia_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("NIA_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(2, int(os.environ.get("NIA_TUI_RPC_POOL_WORKERS") or "4"))
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent NIACLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `nia` shows the prompt before constructing AIAgent; the TUI used
...
def resolve_skin() -> dict:
    try:
        from nia_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("NIA_MODEL", "")
        or os.environ.get("NIA_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("NIA_TUI_PROVIDER", "").strip()
    ...
    try:
        from nia_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("NIA_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("NIA_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] NIA_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from nia_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown NIA_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in NIA_TUI_TOOLSETS "
        ...
        fallback_notice = "[tui] no valid NIA_TUI_TOOLSETS entries; using configured CLI toolsets"
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid NIA_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from nia_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["NIA_MODEL"] = result.new_model
    os.environ["NIA_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # NIA_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["NIA_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["NIA_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _sync_session_key_after_compress(sid: str, session: dict) -> None:
    ...
    Mirrors NIACLI._manual_compress's session_id sync.
    """
    agent = session.get("agent")
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from nia_cli.banner import get_available_skills
    ...
    try:
        ...
        info["mcp_servers"] = []
    try:
        from nia_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from nia_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from nia_cli.config import load_config as _load_full_cfg
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `NIA_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"nia_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $NIA_HOME/spawn-trees/<session_id>/<timestamp>.json
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from nia_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from nia_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: nia_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from nia_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _nia_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from nia_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_nia_home), "display": display_nia_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from nia_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.nia/.env`` into the gateway process via
    ``nia_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `nia` is interactive — use `/nia chat -q …` or run `nia` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`nia setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`nia gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`nia sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`nia config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m nia_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "nia_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nia_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `nia model` and
        ...
        from nia_cli.auth import PROVIDER_REGISTRY as _auth_reg
        ordered: list = []
        for entry in CANONICAL_PROVIDERS:
            if entry.slug in authed_map:
                ...
                if auth_type == "api_key" and key_env:
                    ...
                    warning = f"run `nia model` to configure ({auth_type})"
                ordered.append({
                    "slug": entry.slug,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nia_cli.config import is_managed, save_env_value
        from nia_cli.model_switch import list_authenticated_providers
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `nia model` to configure"
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.nia/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from nia_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from nia_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("NIA_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("NIA_VOICE_TTS", "").strip() == "1"
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        if not enabled:
            ...
            try:
                from nia_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from nia_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from nia_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from nia_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from nia_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("NIA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("NIA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_nia_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nia_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from nia_cli.skills_hub import do_install
        ...
        if action == "browse":
            from nia_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from nia_cli.skills_hub import inspect_skill
