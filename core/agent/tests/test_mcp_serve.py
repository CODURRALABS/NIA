"""
Tests for mcp_serve — NIA MCP server.
...
def _isolate_nia_home(tmp_path, monkeypatch):
    """Redirect NIA_HOME to a temp directory."""
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    try:
        import nia_constants
        monkeypatch.setattr(nia_constants, "get_nia_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking nia_state schema."""
