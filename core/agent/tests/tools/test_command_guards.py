...
def _clean_state():
    ...
    saved = {}
    for k in ("NIA_INTERACTIVE", "NIA_GATEWAY_SESSION", "NIA_EXEC_ASK", "NIA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["NIA_INTERACTIVE"] = "1"
        session_key = os.getenv("NIA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No NIA_INTERACTIVE or NIA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
