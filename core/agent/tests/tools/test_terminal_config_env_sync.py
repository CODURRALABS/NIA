...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. nia_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``nia config set …``)
...
dicts) keeps the test independent of the user's ~/.nia/config.yaml and
mirrors the pattern used in tests/nia_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``nia config set foo bar``."""
    from nia_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``nia config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`nia config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in nia_cli/config.py:set_config_value."
    )
