...
def _restore_tool_modules():
    original_nia_home = os.environ.get("NIA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_nia_home is None:
            os.environ.pop("NIA_HOME", None)
        else:
            os.environ["NIA_HOME"] = original_nia_home
        _reset_modules(("tools", "nia_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    nia_cli = types.ModuleType("nia_cli")
    nia_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["nia_cli"] = nia_cli
    nia_home = tmp_path / "nia-home"
    os.environ["NIA_HOME"] = str(nia_home)
    sys.modules["nia_cli.config"] = types.SimpleNamespace(
        get_nia_home=lambda: nia_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="nia-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": nia_home / "modal_snapshots.json",
