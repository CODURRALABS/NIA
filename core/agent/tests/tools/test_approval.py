...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("nia_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("nia_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"NIA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_nia_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.nia/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_nia_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $NIA_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_nia_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$NIA_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_nia_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $NIA_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.nia/nia-agent && source venv/bin/activate && python -m nia_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m nia_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m nia_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "nia_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart nia-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_nia_detected(self):
        """pkill targeting nia/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_nia_detected(self):
        cmd = "killall nia"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep nia) bypasses the pkill/killall name-matching
    ...
    def test_pkill_nia_still_detected(self):
