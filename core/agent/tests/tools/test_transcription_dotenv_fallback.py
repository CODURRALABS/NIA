...
``~/.nia/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.nia/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while nia_cli.config.get_env_value is patched must
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``nia_cli.config.load_env`` to simulate
    ``~/.nia/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("nia_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
