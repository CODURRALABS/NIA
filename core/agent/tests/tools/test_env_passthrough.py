...
class TestTerminalIntegration:
    ...
    def test_blocklisted_var_blocked_by_default(self):
        from tools.environments.local import _sanitize_subprocess_env, _NIA_PROVIDER_ENV_BLOCKLIST

        # Pick a var we know is in the blocklist
        blocked_var = next(iter(_NIA_PROVIDER_ENV_BLOCKLIST))
    ...
    def test_passthrough_cannot_override_provider_blocklist(self):
        ...
        NIA provider credentials — that was the bypass where a skill
    ...
    def test_non_nia_api_key_still_registerable(self):
        """Third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
        NIA provider credentials and must still pass through — skills
