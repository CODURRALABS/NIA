...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nia_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_nia_bin(self):
        """After _INSTALL_FAILED, manual install in NIA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        nia_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(nia_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(nia_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._nia_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == nia_bin
            assert _tirith_mod._resolved_path == nia_bin
            mock_clear.assert_called_once()
...
class TestNIAHomeIsolation:
    def test_nia_bin_dir_respects_nia_home(self):
        """_nia_bin_dir must use NIA_HOME, not hardcoded ~/.nia."""
        from tools.tirith_security import _nia_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"NIA_HOME": tmpdir}):
            result = _nia_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_nia_home(self):
        """_failure_marker_path must use NIA_HOME, not hardcoded ~/.nia."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"NIA_HOME": "/custom/nia"}):
            result = _failure_marker_path()
        assert result == "/custom/nia/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets NIA_HOME; verify it's active."""
        nia_home = os.getenv("NIA_HOME")
        assert nia_home is not None, "NIA_HOME should be set by conftest"
        assert "nia_test" in nia_home, "Should point to test temp dir"

    def test_get_nia_home_fallback(self):
        """Without NIA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_nia_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove NIA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".nia")
            result = _get_nia_home()
