...
Verifies that NIA-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed NIA secrets.

See: Local Sovereign Environment/issues/1002
See: Local Sovereign Environment/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _NIA_PROVIDER_ENV_BLOCKLIST,
    _NIA_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.nia/.env must not leak."""
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _NIA_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_NIA_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_NIA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_NIA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_NIA_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_NIA_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                assert var in _NIA_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _NIA_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_NIA_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _NIA_PROVIDER_ENV_BLOCKLIST, (
