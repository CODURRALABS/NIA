...
1. ``NIATokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``NIAMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# NIATokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestSetTokensAbsoluteExpiry:
    def test_set_tokens_persists_absolute_expires_at(self, tmp_path, monkeypatch):
        ...
        from tools.mcp_oauth import NIATokenStorage
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("NIA_HOME", str(tmp_path))
        from tools.mcp_oauth import NIATokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# NIAMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import NIATokenStorage
    from tools.mcp_oauth_manager import _NIA_PROVIDER_CLS, reset_manager_for_tests
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = NIATokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="NIA",
    )
