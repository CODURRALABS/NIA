...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_nia_home_override(self, tmp_path):
        """Auth lookup should read from NIA_HOME/auth.json, not ~/.nia/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".nia").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``nia tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
