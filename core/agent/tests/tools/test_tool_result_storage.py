...
class TestHeredocMarker:
    ...
    def test_uuid_marker_on_collision(self):
        ...
        assert marker.startswith("NIA_PERSIST_")
...
class TestWriteToSandbox:
    def test_success(self):
        ...
        result = _write_to_sandbox("hello world", "/tmp/nia-results/abc.txt", env)
    ...
    def test_failure_returns_false(self):
        ...
        result = _write_to_sandbox("content", "/tmp/nia-results/abc.txt", env)
    ...
    def test_heredoc_collision_uses_uuid_marker(self):
        ...
        _write_to_sandbox(content, "/tmp/nia-results/abc.txt", env)
        cmd = env.execute.call_args[0][0]
    ...
    def test_uses_parent_dir_of_remote_path(self):
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        remote_path = "/data/data/com.termux/files/usr/tmp/nia-results/abc.txt"
        _write_to_sandbox("content", remote_path, env)
        cmd = env.execute.call_args[0][0]
        assert "mkdir -p /data/data/com.termux/files/usr/tmp/nia-results" in cmd

    def test_path_with_spaces_is_quoted(self):
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        remote_path = "/tmp/nia results/abc file.txt"
        _write_to_sandbox("content", remote_path, env)
        cmd = env.execute.call_args[0][0]
        assert "'/tmp/nia results'" in cmd
        assert "'/tmp/nia results/abc file.txt'" in cmd

    def test_shell_metacharacters_neutralized(self):
        """Paths with shell metacharacters must be quoted to prevent injection."""
        env = MagicMock()
        env.execute.return_value = {"output": "", "returncode": 0}
        malicious_path = "/tmp/nia-results/$(whoami).txt"
        _write_to_sandbox("content", malicious_path, env)
        cmd = env.execute.call_args[0][0]
        # The $() must not appear unquoted — shlex.quote wraps it
        assert "'/tmp/nia-results/$(whoami).txt'" in cmd
...
class TestResolveStorageDir:
    ...
    def test_uses_env_temp_dir_when_available(self):
        ...
        assert _resolve_storage_dir(env) == "/data/data/com.termux/files/usr/tmp/nia-results"
...
class TestBuildPersistedMessage:
    def test_structure(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/nia-results/test123.txt",
        )
        ...
        assert "/tmp/nia-results/test123.txt" in msg
    ...
    def test_no_ellipsis_when_complete(self):
        msg = _build_persisted_message(
            ...
            has_more=False,
            original_size=16,
            file_path="/tmp/nia-results/x.txt",
        )
    ...
    def test_large_size_shows_mb(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/nia-results/big.txt",
        )
        assert "MB" in msg
...
class TestMaybePersistToolResult:
    ...
    def test_env_temp_dir_changes_persisted_path(self):
        ...
        assert "/data/data/com.termux/files/usr/tmp/nia-results/tc_termux.txt" in result
        cmd = env.execute.call_args[0][0]
        assert "mkdir -p /data/data/com.termux/files/usr/tmp/nia-results" in cmd
