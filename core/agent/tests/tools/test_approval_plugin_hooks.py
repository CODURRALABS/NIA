...
def isolated_session(monkeypatch):
    ...
    monkeypatch.setenv("NIA_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: NIA_INTERACTIVE is set, the
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("nia_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("nia_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: NIA_GATEWAY_SESSION is set and a
    ...
    def test_pre_and_post_fire_on_gateway_surface(
        ...
        monkeypatch.setenv("NIA_GATEWAY_SESSION", "1")
        monkeypatch.delenv("NIA_EXEC_ASK", raising=False)
        monkeypatch.setattr(approval_module, "_get_approval_mode", lambda: "manual")
