...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning nia PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_non_nia_dirs_are_ignored(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which nia process owns a daemon so that
    concurrent nia processes don't reap each other's active browser
