...
def test_nia_provider_subclass_exists():
    """NIAMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _NIA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _NIA_PROVIDER_CLS is not None
    assert issubclass(_NIA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_nia_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns NIAMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _NIA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    ...
    assert _NIA_PROVIDER_CLS is not None
    assert isinstance(provider, _NIA_PROVIDER_CLS)
    assert provider._nia_server_name == "srv"
