...
class TestWriteDenyExactPaths:
    ...
    def test_nia_env(self):
        # ``.env`` under the active NIA_HOME (profile-aware, not just
        # ``~/.nia``) must be write-denied. The hermetic test conftest
        # points NIA_HOME at a tempdir — resolve via get_nia_home()
        # to match the denylist.
        from nia_constants import get_nia_home
        path = str(get_nia_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_nia_config_not_env(self):
        path = os.path.join(str(Path.home()), ".nia", "config.yaml")
