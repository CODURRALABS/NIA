...
Verifies:
  - Tools are gated on NIA_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `nia chat` sessions (no NIA_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("NIA_KANBAN_TASK", raising=False)
    home = tmp_path / ".nia"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("nia-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    """Worker sessions (NIA_KANBAN_TASK set) must have all 7 tools."""
    monkeypatch.setenv("NIA_KANBAN_TASK", "t_fake")
    home = tmp_path / ".nia"
    home.mkdir()
    monkeypatch.setenv("NIA_HOME", str(home))
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: NIA_HOME isolated, NIA_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".nia"
    home.mkdir()
    monkeypatch.setenv("NIA_HOME", str(home))
    monkeypatch.setenv("NIA_PROFILE", "test-worker")
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("NIA_KANBAN_TASK", tid)
    return tid
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to NIA_PROFILE env we set in the fixture
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no NIA_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (NIA_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has NIA_KANBAN_TASK set to its own task id. The
...
#
# Orchestrator profiles (no NIA_KANBAN_TASK in env) are intentionally
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no NIA_KANBAN_TASK) can still complete
