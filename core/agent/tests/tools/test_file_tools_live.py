...
class TestReadFile:
    ...
    def test_tilde_expansion(self, ops):
        test_path = Path.home() / ".nia_test_tilde_9f8a7b"
        try:
            test_path.write_text("TILDE_EXPANSION_OK\n")
            result = ops.read_file("~/.nia_test_tilde_9f8a7b")
            assert result.error is None
...
class TestExpandPath:
    ...
    def test_tilde_injection_blocked(self, ops):
        ...
        malicious = "~; echo PWNED > /tmp/_nia_injection_test"
        ...
        import os
        assert not os.path.exists("/tmp/_nia_injection_test")
