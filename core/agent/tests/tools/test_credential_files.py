...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        nia_home.mkdir()
        (nia_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"NIA_HOME": str(nia_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(nia_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.nia/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.nia/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        nia_home = tmp_path / ".nia"
        (nia_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"NIA_HOME": str(nia_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.nia")

        assert mounts[0]["container_path"] == "/home/user/.nia/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.nia/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.nia/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape NIA_HOME."""
        monkeypatch.setenv("NIA_HOME", str(tmp_path / ".nia"))
        (tmp_path / ".nia").mkdir()

        # Create a sensitive file one level above nia_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside nia_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside NIA_HOME must still be registered."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        (nia_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_nia_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of NIA_HOME must be allowed."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        subdir = nia_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside NIA_HOME pointing outside must be rejected."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))

        # Create a sensitive file outside nia_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside nia_home pointing outside
        symlink = nia_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes NIA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, nia_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape NIA_HOME."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        ...
        sensitive.write_text("{}")
        self._write_config(nia_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(nia_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside NIA_HOME via config must still mount."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))

        (nia_home / "oauth.json").write_text("{}")
        self._write_config(nia_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "cache" / "documents").mkdir(parents=True)
        (nia_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("NIA_HOME", str(nia_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.nia/cache/documents" in paths
        assert "/root/.nia/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.nia/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        # Use legacy dir name — get_nia_dir prefers old if it exists
        (nia_home / "document_cache").mkdir()
        (nia_home / "image_cache").mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(nia_home / "document_cache") in host_paths
        assert str(nia_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.nia/cache/documents" in container_paths
        assert "/root/.nia/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        doc_dir = nia_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        ss_dir = nia_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.nia/cache/screenshots/session_abc/screen1.png"
