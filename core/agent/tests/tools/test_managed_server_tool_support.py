...
Validates that:
1. The installed atroposlib API still matches NIA's expectations
2. NIA's parser registry remains compatible with ManagedServer parsing
3. NIAAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between nia-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what nia-agent expects."""
...
class TestParserCompatibility:
    """Test that nia-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that nia_base_env.py's tool-parser wiring matches the current API."""

    def test_nia_base_env_sets_server_manager_tool_parser(self):
        """NIA wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "nia_base_env.py"
        ...
        assert found_assignment, (
            "nia_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_nia_base_env_uses_config_tool_call_parser(self):
        """Verify nia_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "nia_base_env.py"
        source = base_env_path.read_text()
