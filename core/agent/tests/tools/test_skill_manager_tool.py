...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.nia/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("nia_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("nia_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.nia/skills/ as a
    workaround.
    """
...
# The user unpins via `nia curator unpin <name>`.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_edit_refuses_pinned(self, tmp_path):
        ...
        assert "nia curator unpin my-skill" in result["error"]
