...
- Signal forwarding runs through the init so ``docker stop`` triggers
  nia's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, nia accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, nia accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into nia,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, nia "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    assert "ui-tui/packages/nia-ink/package-lock.json" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    assert any(
        "ui-tui" in step
        and "node_modules/@nia/ink" in step
        and "packages/nia-ink" in step
        and "rm -rf packages/nia-ink/node_modules" in step
        and "npm install --omit=dev" in step
        and "--prefix node_modules/@nia/ink" in step
        and "rm -rf node_modules/@nia/ink/node_modules/react" in step
        and "await import('@nia/ink')" in step
        for step in _run_steps(dockerfile_text)
    )
