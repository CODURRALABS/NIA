...
def _make_voice_cli(**overrides):
    """Create a minimal NIACLI with only voice-related attrs initialized.
...
class TestDisableVoiceModeStopsTTS:
    ...
    def test_disable_voice_mode_calls_stop_playback(self):
        ...
        source = inspect.getsource(NIACLI._disable_voice_mode)
...
class TestEnableVoiceModeReal:
    ...
    @patch("cli._cprint")
    @patch("nia_cli.config.load_config", return_value={"voice": {}})
    ...
    @patch("cli._cprint")
    @patch("nia_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("cli._cprint")
    @patch("nia_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("nia_cli.config.load_config", return_value={"voice": {"beep_enabled": False}})
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("nia_cli.config.load_config", return_value={"stt": {}})
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("nia_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("tools.voice_mode.transcribe_recording",
