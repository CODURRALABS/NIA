...
def test_resolve_stdio_command_falls_back_to_nia_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"NIA_HOME": str(tmp_path)}, clear=False):
...
def test_run_stdio_uses_resolved_command_and_prepended_path(tmp_path):
    ...
    async def _test():
        with patch("tools.mcp_tool.shutil.which", return_value=None), \
             patch.dict("os.environ", {"NIA_HOME": str(tmp_path), "PATH": "/usr/bin", "HOME": str(tmp_path)}, clear=False), \
