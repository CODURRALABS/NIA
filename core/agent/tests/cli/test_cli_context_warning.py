...
def _isolate(tmp_path, monkeypatch):
    """Isolate NIA_HOME so tests don't touch real config."""
    home = tmp_path / ".nia"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal NIACLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = NIACLI.__new__(NIACLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nia-core"]
        obj.compact = False
