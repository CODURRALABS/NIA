...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("nia/nia-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned nia/* and pr-* branches."""

    def test_prunes_orphaned_nia_branch(self, git_repo):
        """nia/nia-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "nia/nia-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "nia/nia-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "nia/nia-deadbeef" in result.stdout
        ...
        assert "nia/nia-deadbeef" in orphaned
        ...
        assert "nia/nia-deadbeef" not in result.stdout
