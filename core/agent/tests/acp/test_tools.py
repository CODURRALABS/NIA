...
COMMON_NIA_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_nia_tools_have_kind(self):
        """Every common nia tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_NIA_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        ...
        assert title == "python: from nia_tools import terminal"

    def test_skill_manage_title_includes_action_and_target(self):
        title = build_tool_title(
            "skill_manage",
            {"action": "patch", "name": "nia-agent-operations", "file_path": "references/acp.md"},
        )
        assert title == "skill patch: nia-agent-operations/references/acp.md"
...
class TestBuildToolStart:
    ...
    def test_build_tool_start_for_skill_manage_patch_shows_diff(self):
        ...
        assert result.kind == "edit"
        assert result.title == "skill patch: nia-agent-operations/references/acp.md"
        assert isinstance(result.content[0], FileEditToolCallContent)
        assert result.content[0].path == "skills/nia-agent-operations/references/acp.md"
...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_skill_manage_summarizes_without_raw_json(self):
        result = build_tool_complete(
            "tc-skill-manage",
            "skill_manage",
            '{"success":true,"message":"Patched references/nia-acp-zed-rendering.md in skill \'nia-agent-operations\' (1 replacement)."}',
            function_args={
                "action": "patch",
                "name": "nia-agent-operations",
                "file_path": "references/nia-acp-zed-rendering.md",
            },
        )
        text = result.content[0].content.text
        assert "**✅ Skill updated**" in text
        assert "`patch`" in text
        assert "`nia-agent-operations`" in text
        assert "references/nia-acp-zed-rendering.md" in text
    ...
    def test_build_tool_complete_for_write_file_uses_snapshot_diff(self, tmp_path):
        ...
        target.write_text("hello from nia\n", encoding="utf-8")

        result = build_tool_complete(
            "tc-wf1",
            "write_file",
            '{"bytes_written": 18, "dirs_created": false}',
            function_args={"path": str(target), "content": "hello from nia\n"},
            snapshot=snapshot,
        )
        assert isinstance(result, ToolCallProgress)
        assert len(result.content) == 1
        ...
        assert diff_item.new_text == "hello from nia"
