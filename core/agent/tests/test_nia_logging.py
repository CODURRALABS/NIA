"""Tests for nia_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    nia_logging._logging_initialized = False
    ...
    nia_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    nia_logging._logging_initialized = False
    nia_logging.clear_session_context()


@pytest.fixture
def nia_home(tmp_path, monkeypatch):
    """Provide an isolated NIA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_nia_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["NIA_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, nia_home):
        log_dir = nia_logging.setup_logging(nia_home=nia_home)
        assert log_dir == nia_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        nia_logging.setup_logging(nia_home=nia_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        nia_logging.setup_logging(nia_home=nia_home, force=True)
    ...
    def test_custom_log_level(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, nia_home):
        nia_logging.setup_logging(
            nia_home=nia_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
    ...
    def test_writes_to_agent_log(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)

        test_logger = logging.getLogger("test_nia_logging.write_test")
        ...
        agent_log = nia_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)

        test_logger = logging.getLogger("test_nia_logging.warning_test")
        ...
        agent_log = nia_home / "logs" / "agent.log"
        errors_log = nia_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)

        test_logger = logging.getLogger("test_nia_logging.info_test")
    ...
    def test_explicit_params_override_config(self, nia_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (nia_home / "config.yaml").write_text(yaml.dump(config))

        nia_logging.setup_logging(nia_home=nia_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, nia_home):
        """The custom record factory injects session_tag on all records."""
        nia_logging.setup_logging(nia_home=nia_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_nia_session_injector", False), (
            "Record factory should have _nia_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, nia_home):
        ...
        gw_log = nia_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, nia_home):
    ...
    def test_gateway_log_receives_gateway_records(self, nia_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, nia_home):
    ...
    def test_agent_log_still_receives_all(self, nia_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, nia_home):
        """When session context is set, log lines include [session_id]."""
        nia_logging.setup_logging(nia_home=nia_home)
        nia_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, nia_home):
    ...
    def test_clear_session_context(self, nia_home):
        """After clearing, session tag disappears."""
        nia_logging.setup_logging(nia_home=nia_home)
        nia_logging.set_session_context("xyz789")
        nia_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, nia_home):
        ...
        def thread_a():
            nia_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            nia_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        nia_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        nia_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = nia_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = nia_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in nia_logging.COMPONENT_PREFIXES
        assert ("gateway",) == nia_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = nia_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == nia_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = nia_logging.COMPONENT_PREFIXES["cli"]
        assert "nia_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == nia_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        nia_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_nia_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, nia_home):
        nia_logging.setup_logging(nia_home=nia_home)
        nia_logging.setup_verbose_logging()
        nia_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        nia_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = nia_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        nia_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("nia_cli.config.is_managed", return_value=True):
                nia_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, nia_home):
        level, max_size, backup = nia_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, nia_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, nia_home):
        import yaml
        config = {"model": "test"}
        (nia_home / "config.yaml").write_text(yaml.dump(config))
