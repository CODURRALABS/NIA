...
class TestToolsetConsistency:
    ...
    def test_nia_platforms_share_core_tools(self):
        """All nia-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nia-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nia-cli", "nia-telegram", "nia-discord", "nia-whatsapp", "nia-slack", "nia-signal", "nia-homeassistant"]
