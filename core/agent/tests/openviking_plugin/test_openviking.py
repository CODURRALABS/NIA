...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nia/.overview.md") == "viking://user/nia"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nia/memories/profile.md") == "viking://user/nia/memories/profile.md"
...
class TestOpenVikingRead:
    def test_overview_read_normalizes_uri_and_unwraps_result(self):
        ...
        result = json.loads(provider._tool_read({"uri": "viking://user/nia/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/nia/.overview.md"
        assert result["resolved_uri"] == "viking://user/nia"
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/nia/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/nia/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/nia/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/nia/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/nia/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/nia"}),
        ]
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/nia/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/nia/.overview.md", "level": "overview"})
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/nia/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/nia/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/nia"}))

        assert result["path"] == "viking://user/nia"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/nia/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/nia/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/nia"},
