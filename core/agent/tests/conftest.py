"""Shared fixtures for the nia-agent test suite.
...
2. **Isolated NIA_HOME.** NIA_HOME points to a per-test tempdir so
   code reading ``~/.nia/*`` via ``get_nia_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".nia"`` instead of the canonical
   ``get_nia_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NIA_SESSION_* inheritance** — the agent's current gateway
...
# NIA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NIA_BEHAVIORAL_VARS = frozenset({
    "NIA_YOLO_MODE",
    "NIA_INTERACTIVE",
    "NIA_QUIET",
    "NIA_TOOL_PROGRESS",
    ...
    "NIA_MAX_ITERATIONS",
    "NIA_SESSION_PLATFORM",
    "NIA_SESSION_CHAT_ID",
    "NIA_SESSION_CHAT_NAME",
    "NIA_SESSION_THREAD_ID",
    "NIA_SESSION_SOURCE",
    "NIA_SESSION_KEY",
    "NIA_GATEWAY_SESSION",
    "NIA_PLATFORM",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NIA_HOME to per-test tempdirs so code that
    reads ``~/.nia/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral NIA_* vars that could change test semantics.
    for name in _NIA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect NIA_HOME to a per-test tempdir. Code that reads
    #    ``~/.nia/*`` via ``get_nia_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.nia/*`` via
    #    ``Path.home() / ".nia"`` instead of ``get_nia_home()``
    #    is a bug to fix at the callsite.
    fake_nia_home = tmp_path / "nia_test"
    fake_nia_home.mkdir()
    (fake_nia_home / "sessions").mkdir()
    (fake_nia_home / "cron").mkdir()
    (fake_nia_home / "memories").mkdir()
    (fake_nia_home / "skills").mkdir()
    monkeypatch.setenv("NIA_HOME", str(fake_nia_home))
    ...
    #    ~/.nia/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import nia_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_nia_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "nia_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal nia config dict suitable for unit tests."""
    return {
