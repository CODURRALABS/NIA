...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves nia_cli.kanban_db on its own
  ...
  - worker logs are captured to NIA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "NIA_KANBAN_TASK": task.id,
            "NIA_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="nia_e2e_")
    os.environ["NIA_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from nia_cli import kanban_db as kb

    # Point the `nia` CLI child processes will run at the worktree
    # nia_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "nia")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m nia_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/nia_e2e_*/worker_*.log
    import glob
