...
normal tests assume away:
  ...
  - Filesystem: NIA_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own NIA_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"nia_atyp_{name}_")
            os.environ["NIA_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("nia_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from nia_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "NIA_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from nia_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["NIA_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE nia_home: {resolved}")
...
@scenario("nia_home_with_spaces")
def _(home, kb):
    """NIA_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="nia with spaces ")
    os.environ["NIA_HOME"] = weird
    ...
    try:
        ...
        print(f"  NIA_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("nia_home_with_unicode")
def _(home, kb):
    """NIA_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/nia_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  NIA_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("nia_home_via_symlink")
def _(home, kb):
    """NIA_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="nia_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["NIA_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same NIA_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from nia_cli import web_server as ws  # noqa
    except Exception:
        pass
