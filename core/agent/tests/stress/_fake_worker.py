...
Reads NIA_KANBAN_TASK from env, heartbeats periodically, does short
work, completes via the CLI. Designed to be spawned by the dispatcher
exactly the way `nia chat -q` would be, minus the LLM cost.
"""
...
def main():
    tid = os.environ["NIA_KANBAN_TASK"]
    workspace = os.environ.get("NIA_KANBAN_WORKSPACE", "")

    # Announce via CLI (goes through real argparse + init_db + etc)
    subprocess.run(
        ["nia", "kanban", "heartbeat", tid, "--note", "started"],
    ...
    for i in range(3):
        time.sleep(0.3)
        subprocess.run(
            ["nia", "kanban", "heartbeat", tid, "--note", f"progress {i+1}/3"],
            check=True, capture_output=True,
        )
