...
"""
    ...
    assert "Tyler Williams > Active Projects: NIA" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `nia cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # nia_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.nia/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "nia_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``NIA``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "NIA prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told NIA to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "NIA config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "NIA uses tools well"
    # All-lowercase matches → lowercase ``nia``; this preserves the
    # real filesystem path ``~/.nia`` (NIA home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "nia should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "NIA remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "nia prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "NIA was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "nia uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "NIA said to ask NIA about NIA settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.nia`` (the real NIA home), not the broken
    ``.NIA``.
    ...
    rewritten to ``~/.NIA/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.nia/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .nia directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.nia'"
