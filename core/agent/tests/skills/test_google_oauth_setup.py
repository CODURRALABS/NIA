...
class TestNIAConstantsFallback:
    """Tests for _nia_home.py fallback when nia_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_nia_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _nia_home.py with nia_constants blocked."""
        monkeypatch.setitem(sys.modules, "nia_constants", None)
        spec = importlib.util.spec_from_file_location("_nia_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_nia_home_env_var(self, monkeypatch, tmp_path):
        """When nia_constants is missing, NIA_HOME comes from env var."""
        monkeypatch.setenv("NIA_HOME", str(tmp_path / "custom-nia"))
        module = self._load_helper(monkeypatch)
        assert module.get_nia_home() == tmp_path / "custom-nia"

    def test_fallback_defaults_to_dot_nia(self, monkeypatch):
        """When nia_constants is missing and NIA_HOME unset, default to ~/.nia."""
        monkeypatch.delenv("NIA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_nia_home() == Path.home() / ".nia"

    def test_fallback_ignores_empty_nia_home(self, monkeypatch):
        """Empty/whitespace NIA_HOME is treated as unset."""
        monkeypatch.setenv("NIA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_nia_home() == Path.home() / ".nia"

    def test_fallback_display_nia_home_shortens_path(self, monkeypatch):
        """Fallback display_nia_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("NIA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_nia_home() == "~/.nia"

    def test_fallback_display_nia_home_profile_path(self, monkeypatch):
        """Fallback display_nia_home() handles profile paths under ~/."""
        monkeypatch.setenv("NIA_HOME", str(Path.home() / ".nia/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_nia_home() == "~/.nia/profiles/coder"

    def test_fallback_display_nia_home_custom_path(self, monkeypatch):
        """Fallback display_nia_home() returns full path for non-home locations."""
        monkeypatch.setenv("NIA_HOME", "/opt/nia-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_nia_home() == "/opt/nia-custom"

    def test_delegates_to_nia_constants_when_available(self):
        """When nia_constants IS importable, _nia_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_nia_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import nia_constants
        assert module.get_nia_home is nia_constants.get_nia_home
        assert module.display_nia_home is nia_constants.display_nia_home
