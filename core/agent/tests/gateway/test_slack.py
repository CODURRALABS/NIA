...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.nia."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /nia, /btw, /stop,
        ...
        for expected in ("/nia", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /nia subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /nia btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_nia_prefix_still_works(self, adapter):
        """Backward compat: /nia btw foo must still route to /btw foo.

        Old workspace manifests only declared /nia as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_nia_freeform_question(self, adapter):
        """/nia <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/nia",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_nia_slash_stashes_context(self, adapter):
        """Legacy /nia <subcommand> also stashes context."""
        command = {
            "command": "/nia",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_nia_question_does_not_stash_context(self, adapter):
        """Free-form /nia <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/nia",
