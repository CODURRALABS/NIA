...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("NIA_MANAGED", "homebrew")

        result = await runner._handle_update_command(event)

        assert "managed by Homebrew" in result
        assert "brew upgrade nia-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _nia_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._nia_home", tmp_path), \
        ...
        with patch("gateway.run._nia_home", tmp_path):
            ...
            # project_root will be the real nia-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_nia_binary(self, tmp_path):
        """Returns error when nia is not on PATH and nia_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "nia update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m nia_cli.main when nia not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._nia_home", nia_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting NIA update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m nia_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "nia_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_nia_bin_prefers_which(self, tmp_path):
        """_resolve_nia_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_nia_bin

        with patch("shutil.which", return_value="/custom/path/nia"):
            result = _resolve_nia_bin()

        assert result == ["/custom/path/nia"]

    @pytest.mark.asyncio
    async def test_resolve_nia_bin_fallback(self):
        """_resolve_nia_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_nia_bin
        ...
        assert result == [sys.executable, "-m", "nia_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_nia_bin_returns_none_when_both_fail(self):
        """_resolve_nia_bin returns None when both strategies fail."""
        from gateway.run import _resolve_nia_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._nia_home", nia_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/nia" if x == "nia" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = nia_home / ".update_pending.json"
        assert pending_path.exists()
        data = json.loads(pending_path.read_text())
        assert data["platform"] == "telegram"
        assert data["chat_id"] == "99999"
        assert "timestamp" in data
        assert not (nia_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((nia_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (nia_home / ".update_pending.json").exists()
        assert not (nia_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (nia_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = nia_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (nia_home / ".update_output.txt").write_text("done")
        (nia_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (nia_home / ".update_pending.json").write_text(json.dumps(pending))
        (nia_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (nia_home / ".update_pending.json").write_text(json.dumps(pending))
        (nia_home / ".update_output.txt").write_text("x" * 5000)
        (nia_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (nia_home / ".update_pending.json").write_text(json.dumps(pending))
        (nia_home / ".update_output.txt").write_text("Traceback: boom")
        (nia_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = nia_home / ".update_pending.json"
        output_path = nia_home / ".update_output.txt"
