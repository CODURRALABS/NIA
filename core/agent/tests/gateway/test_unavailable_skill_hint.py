...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`nia_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + NIA_HOME so the real user config is untouched."""
    home = tmp_path / ".nia"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `nia skills config` writes).
    monkeypatch.setattr(
    ...
    assert "nia skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "nia_constants.get_optional_skills_dir",
