...
A gateway that survives ``nia update`` keeps pre-update modules cached
...
def test_compute_repo_mtime_returns_newest(tmp_path):
    ...
    newer = time.time()
    os.utime(repo / "nia_cli/config.py", (newer, newer))
...
def test_compute_repo_mtime_partial_files_still_works(tmp_path):
    ...
    (tmp_path / "nia_cli").mkdir()
    target = tmp_path / "nia_cli" / "config.py"
...
def test_detect_stale_code_true_after_update(tmp_path):
    ...
    # Simulate nia update touching config.py
    new_mtime = time.time()
    os.utime(repo / "nia_cli/config.py", (new_mtime, new_mtime))
...
def test_detect_stale_code_ignores_subsecond_drift(tmp_path):
    ...
    os.utime(repo / "nia_cli/config.py", (baseline + 1.0, baseline + 1.0))
    assert runner._detect_stale_code() is False

    # Touch 5s newer → stale
    os.utime(repo / "nia_cli/config.py", (baseline + 5.0, baseline + 5.0))
    assert runner._detect_stale_code() is True
