...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 nia-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["nia-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * nia-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "nia"
        assert adapter.channel == "#nia-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 nia :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * nia :Nickname in use")
        assert adapter._current_nick == "nia_"
        sent = writer.write.call_args[0][0]
        assert b"NICK nia_" in sent
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :nia: hello there")
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG nia :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":nia!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG nia :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :nia: hello")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :nia: hello")
        assert len(dispatched) == 0
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * nia :Nickname in use")
        assert adapter._current_nick == "nia_"
        await adapter._handle_line(":server 433 * nia_ :Nickname in use")
        assert adapter._current_nick == "nia_1"
        await adapter._handle_line(":server 433 * nia_1 :Nickname in use")
        assert adapter._current_nick == "nia_2"
