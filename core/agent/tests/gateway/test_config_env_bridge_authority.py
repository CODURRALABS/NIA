...
Guards against the 60-vs-500 bug where a stale `.env NIA_MAX_ITERATIONS=60`
...
def _run_gateway_import(nia_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "NIA_MAX_ITERATIONS",
            "NIA_AGENT_TIMEOUT",
            "NIA_AGENT_TIMEOUT_WARNING",
            "NIA_GATEWAY_BUSY_INPUT_MODE",
            "NIA_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
...
@pytest.fixture
def nia_home(tmp_path: Path) -> Path:
    home = tmp_path / ".nia"
    home.mkdir()
    return home


def test_config_max_turns_wins_over_stale_env(nia_home: Path) -> None:
    """Regression: config.yaml:agent.max_turns=500 must beat .env=60."""
    _write_config(nia_home, agent_cfg={"max_turns": 500})
    _write_env(nia_home, {"NIA_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(nia_home, initial_env={})

    assert env.get("NIA_MAX_ITERATIONS") == "500", (
        f"expected config.yaml max_turns=500 to win; got {env.get('NIA_MAX_ITERATIONS')!r}. "
        "Stale .env value is shadowing config — the bridge lost its override."
    )


def test_config_gateway_timeout_wins_over_stale_env(nia_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(nia_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(nia_home, {
        "NIA_AGENT_TIMEOUT": "60",
        "NIA_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(nia_home, initial_env={})

    assert env.get("NIA_AGENT_TIMEOUT") == "1800"
    assert env.get("NIA_AGENT_TIMEOUT_WARNING") == "900"


def test_config_display_busy_input_mode_wins_over_stale_env(nia_home: Path) -> None:
    _write_config(nia_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(nia_home, {"NIA_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(nia_home, initial_env={})

    assert env.get("NIA_GATEWAY_BUSY_INPUT_MODE") == "interrupt"


def test_config_timezone_wins_over_stale_env(nia_home: Path) -> None:
    _write_config(nia_home, timezone="America/Los_Angeles")
    _write_env(nia_home, {"NIA_TIMEZONE": "UTC"})

    env = _run_gateway_import(nia_home, initial_env={})

    assert env.get("NIA_TIMEZONE") == "America/Los_Angeles"


def test_env_value_survives_when_config_omits_key(nia_home: Path) -> None:
    ...
    """
    _write_config(nia_home, agent_cfg={})  # no max_turns
    _write_env(nia_home, {"NIA_MAX_ITERATIONS": "123"})

    env = _run_gateway_import(nia_home, initial_env={})

    assert env.get("NIA_MAX_ITERATIONS") == "123"
