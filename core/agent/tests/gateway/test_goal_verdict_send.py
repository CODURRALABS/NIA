...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(nia_home):
    ...
    with patch("nia_cli.goals.judge_goal", return_value=("done", "the feature shipped")):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(nia_home):
    ...
    with patch("nia_cli.goals.judge_goal", return_value=("continue", "still needs work")):
        runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(nia_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from nia_cli.goals import GoalManager, save_goal
    ...
    with patch("nia_cli.goals.judge_goal", return_value=("continue", "keep going")):
        runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(nia_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(nia_home):
    ...
    with patch("nia_cli.goals.judge_goal", return_value=("done", "ok")):
        # must not raise
        runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
