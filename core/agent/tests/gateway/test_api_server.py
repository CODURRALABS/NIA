...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_config_reasoning_effort(self, monkeypatch):
        ...
        monkeypatch.setattr("nia_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["platform"] == "nia-agent"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_nia_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "nia-agent"
            assert data["data"][0]["owned_by"] == "nia"
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "nia.api_server.capabilities"
            assert data["platform"] == "nia-agent"
            assert data["model"] == "nia-agent"
            assert data["auth"]["type"] == "bearer"
            assert data["auth"]["required"] is False
            assert data["features"]["chat_completions"] is True
            assert data["features"]["run_status"] is True
            assert data["features"]["run_events_sse"] is True
            assert data["features"]["session_continuity_header"] == "X-NIA-Session-Id"
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: nia.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``nia.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: nia.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: nia.tool.progress":
                    continue
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "What is 1+1?"},
                )
    ...
    async def test_previous_response_id_preserves_session(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "Hello"},
                )
            assert resp1.status == 200
    ...
    async def test_stream_true_returns_responses_sse(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "hi", "stream": True},
                )
                assert resp.status == 200
                assert "text/event-stream" in resp.headers.get("Content-Type", "")
                body = await resp.text()
    ...
    async def test_stream_emits_function_call_and_output_items(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "read the file", "stream": True},
                )
                assert resp.status == 200
                body = await resp.text()
    ...
    async def test_streamed_response_is_stored_for_get(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "store this", "stream": True},
                )
                body = await resp.text()
                response_id = None
                for line in body.splitlines():
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nia-agent", "input": "What is 6*7?"},
                )
...
# ---------------------------------------------------------------------------
# X-NIA-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_new_session_response_includes_session_id_header(self, adapter):
        """Without X-NIA-Session-Id, a new session is created and returned in the header."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "nia-agent", "messages": [{"role": "user", "content": "Hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-NIA-Session-Id") is not None

    @pytest.mark.asyncio
    async def test_provided_session_id_is_used_and_echoed(self, auth_adapter):
        """When X-NIA-Session-Id is provided, it's passed to the agent and echoed in the response."""
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-NIA-Session-Id": "my-session-123", "Authorization": "Bearer sk-secret"},
                    json={"model": "nia-agent", "messages": [{"role": "user", "content": "Continue"}]},
                )

            assert resp.status == 200
            assert resp.headers.get("X-NIA-Session-Id") == "my-session-123"
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs["session_id"] == "my-session-123"

    @pytest.mark.asyncio
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-NIA-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-NIA-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run, \
                 patch("nia_state.SessionDB", side_effect=Exception("DB unavailable")):
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})

                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-NIA-Session-Id": "some-session", "Authorization": "Bearer sk-secret"},
                    json={"model": "nia-agent", "messages": [{"role": "user", "content": "Hi"}]},
                )
