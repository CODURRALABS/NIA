...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @nia:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("nia can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("NIA can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@nia:example.org">NIA</a> help'
        assert self.adapter._is_bot_mentioned("NIA help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "niabot" should not match word-boundary check for "nia"
        assert not self.adapter._is_bot_mentioned("niabot is here")
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@nia:example.org help me")
        assert result == "help me"

    def test_localpart_preserved(self):
        """Bare localpart (no @) is preserved — avoids false positives in paths."""
        result = self.adapter._strip_mention("nia help me")
        assert result == "nia help me"

    def test_localpart_in_path_preserved(self):
        """Localpart inside a file path must not be damaged."""
        result = self.adapter._strip_mention("read /home/nia/config.yaml")
        assert result == "read /home/nia/config.yaml"
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "NIA" should not be mutated.
        result = self.adapter._strip_mention("NIA")
        assert result == "NIA"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@nia:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@nia:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@nia:example.org">NIA</a> help'
    event = _make_event("NIA help", formatted_body=formatted)
...
async def test_dm_preserves_localpart_in_body(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("nia help me")

    await adapter._on_room_message(event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "nia help me"
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@nia:example.org")
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@nia:example.org help me"
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@nia:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@nia:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@nia:example.org help", event_id="$dm1")
