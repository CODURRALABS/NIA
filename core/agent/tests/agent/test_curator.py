...
def curator_env(tmp_path, monkeypatch):
    """Isolated NIA_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".nia"
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `nia curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """nia curator unpin must refuse bundled/hub skills too (matches pin)."""
    from nia_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `nia model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from nia_cli.main import _AUX_TASKS
    from nia_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. nia_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. nia_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
