...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    nia_home = str(tmp_path / ".nia")
    os.makedirs(nia_home, exist_ok=True)
    monkeypatch.setenv("NIA_HOME", nia_home)
...
class TestIsGenuineNousRateLimit:
    ...
    Nous Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, NIA).
