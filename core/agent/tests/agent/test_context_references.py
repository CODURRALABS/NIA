...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "NIA Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_nia_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("NIA_HOME", str(tmp_path / ".nia"))

    nia_env = tmp_path / ".nia" / ".env"
    nia_env.parent.mkdir(parents=True)
    nia_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.nia/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
