...
def backup_env(monkeypatch, tmp_path):
    """Isolate NIA_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".nia"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_nia_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd nia_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under NIA_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level NIA_DIR picks up the tmp HOME."""
    import nia_constants
    importlib.reload(nia_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.nia/cron/jobs.json
