...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "nia" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "nia" / "auth.json").read_text())
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.nia/.env when
    ...
    # User edited ~/.nia/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.nia/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (nia_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_nia_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("nia_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:nia_pkce", "nia_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NIA_HOME", str(tmp_path / "nia"))
    (tmp_path / "nia").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "nia" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_load_pool_does_not_seed_claude_code_when_anthropic_not_configured(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.auth.is_provider_explicitly_configured",
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `nia auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `nia model` to reauth, but the pool
    ...
    # Simulate `nia model` / `nia auth` refreshing the tokens.
