...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("NIA_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"NIA_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"NIA_PLATFORM": "discord"}):
...
"""
    ...
    def test_gateway_still_loads_skill_but_returns_setup_guidance(
        ...
        with patch.dict(
            os.environ, {"NIA_SESSION_PLATFORM": "telegram"}, clear=False
        ):
            with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
                _make_skill(
                    tmp_path,
...
class TestTemplateVarSubstitution:
    """``${NIA_SKILL_DIR}`` and ``${NIA_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${NIA_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${NIA_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${NIA_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${NIA_SKILL_DIR}/scripts/foo.js" in msg
