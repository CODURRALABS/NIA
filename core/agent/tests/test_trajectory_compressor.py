...
def test_import_loads_env_from_nia_home(tmp_path, monkeypatch):
    home = tmp_path / ".nia"
    home.mkdir()
    (home / ".env").write_text("OPENROUTER_API_KEY=from-nia-home\n", encoding="utf-8")
    ...
    assert os.getenv("OPENROUTER_API_KEY") == "from-nia-home"
