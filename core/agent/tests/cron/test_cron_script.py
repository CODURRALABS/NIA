...
- Path resolution (absolute, relative to NIA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp NIA_HOME."""
    nia_home = tmp_path / ".nia"
    nia_home.mkdir()
    (nia_home / "cron").mkdir()
    (nia_home / "cron" / "output").mkdir()
    (nia_home / "scripts").mkdir()
    monkeypatch.setenv("NIA_HOME", str(nia_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "NIA_DIR", nia_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", nia_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", nia_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", nia_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.nia/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("NIA_SESSION_PLATFORM") is None
        assert os.environ.get("NIA_SESSION_CHAT_ID") is None
        assert os.environ.get("NIA_SESSION_CHAT_NAME") is None
