...
from nia_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from nia_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("NIA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_nia_ca_bundle_env_falls_back(self, monkeypatch):
        from nia_cli.auth import _resolve_verify

        monkeypatch.setenv("NIA_CA_BUNDLE", "/nonexistent/nia-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (nia_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from nia_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    nia_home = tmp_path / "nia"
    _setup_nous_auth(nia_home, access_token="at-123")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
    monkeypatch.setattr(
        "nia_cli.auth.resolve_nous_runtime_credentials",
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    nia_home = tmp_path / "nia"
    _setup_nous_auth(nia_home, access_token="at-fresh")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    nia_home = tmp_path / "nia"
    _setup_nous_auth(nia_home, refresh_token="refresh-old")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
    ...
    monkeypatch.setattr("nia_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("nia_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `nia model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = nia_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return nia_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import nia_cli.auth as auth_mod
        import nia_cli.models as models_mod
        import nia_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from nia_cli.auth import PROVIDER_REGISTRY, _login_nous

        nia_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `nia auth add nous`
    ...
    """
    from nia_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "NIA is not logged into Nous Portal").
    ...
    """
    from nia_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "NIA is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `nia auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", NIA must surface an
    ...
    re-authenticate" string led users to report this as a NIA persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from nia_cli.auth import _refresh_access_token
    ...
    assert "nia auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect NIA_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("NIA_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without NIA_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.nia/shared/`` across CI runs.
    """
    from nia_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("NIA_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """NIA_SHARED_AUTH_DIR must redirect the path."""
    from nia_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("NIA_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from nia_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from nia_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from nia_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `nia auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from nia_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    """
    from nia_cli import auth as auth_mod
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "nia-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different NIA_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("NIA_HOME", str(profile_a))
    ...
    # Profile B: fresh NIA_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("NIA_HOME", str(profile_b))
