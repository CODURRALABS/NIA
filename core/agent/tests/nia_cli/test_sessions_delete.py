...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(nia_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def test_sessions_delete_reports_not_found_when_prefix_is_unknown(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "sessions", "delete", "missing-prefix", "--yes"],
    )
...
def test_sessions_delete_handles_eoferror_on_confirm(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["nia", "sessions", "delete", "20260315_092437_c9a6"],
    )
