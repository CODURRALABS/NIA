"""Tests for the `nia memory reset` CLI command.
...
- Profile-scoped reset (uses NIA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake NIA_HOME with memory files."""
    nia_home = tmp_path / ".nia"
    memories = nia_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("NIA_HOME", str(nia_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nNIA repo is at ~/.nia/nia-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `nia memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        (nia_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("NIA_HOME", str(nia_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's NIA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("NIA_HOME", str(nia_home))

        # The memories dir won't exist; get_nia_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
