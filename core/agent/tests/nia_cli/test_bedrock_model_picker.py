...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (NIA_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from nia_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via NIA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in NIA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from nia_cli.providers import NIA_OVERLAYS
        assert "bedrock" in NIA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from nia_cli.providers import NIA_OVERLAYS
        assert NIA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from nia_cli.providers import NIA_OVERLAYS
        assert NIA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
