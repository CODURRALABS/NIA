"""Tests for nia_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("nia_cli.copilot_auth._try_gh_cli_token", return_value="gho_from_cli"):
            token, source = resolve_copilot_token()
    ...
    def test_gh_cli_classic_pat_raises(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("nia_cli.copilot_auth._try_gh_cli_token", return_value="ghp_classic"):
    ...
    def test_no_token_returns_empty(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("nia_cli.copilot_auth._try_gh_cli_token", return_value=None):
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
...
class TestRequestHeaders:
    ...
    def test_default_headers_include_openai_intent(self):
        ...
        assert headers["User-Agent"] == "NIAAgent/1.0"
