"""Tests for the multi-board kanban layer (``nia kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``NIA_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``nia kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``NIA_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated NIA_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test NIA_HOME plus a path-init cache reset
    so each test sees a truly empty board set.
    """
    home = tmp_path / "nia_home"
    home.mkdir()
    monkeypatch.setenv("NIA_HOME", str(home))
    for var in (
        "NIA_KANBAN_DB",
        "NIA_KANBAN_WORKSPACES_ROOT",
        "NIA_KANBAN_HOME",
        "NIA_KANBAN_BOARD",
    ):
        monkeypatch.delenv(var, raising=False)
    # Also reset nia_constants cache so get_default_nia_root() re-reads.
    try:
        import nia_constants
        nia_constants._cached_default_nia_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "nia-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``NIA_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("NIA_KANBAN_DB", str(forced))
        assert kb.kanban_db_path() == forced
        assert kb.kanban_db_path(board="ignored") == forced

    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("NIA_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("NIA_KANBAN_BOARD", "envboard")
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("NIA_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``NIA_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["NIA_KANBAN_BOARD"] == "default"
        assert env["NIA_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``nia kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "nia_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
