"""Tests for `nia update --yes / -y` — assume yes for interactive prompts.
...
class TestUpdateYesConfigMigration:
    ...
    @patch("nia_cli.config.migrate_config")
    @patch("nia_cli.config.check_config_version", return_value=(1, 2))
    @patch("nia_cli.config.get_missing_config_fields", return_value=[])
    @patch("nia_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
...
class TestUpdateYesStashRestore:
    ...
    @patch("nia_cli.main._restore_stashed_changes")
    @patch(
        "nia_cli.main._stash_local_changes_if_needed",
        return_value="stash@{0}",
    )
    @patch("nia_cli.config.check_config_version", return_value=(1, 1))
    @patch("nia_cli.config.get_missing_config_fields", return_value=[])
    @patch("nia_cli.config.get_missing_env_vars", return_value=[])
    @patch("shutil.which", return_value=None)
    @patch("subprocess.run")
