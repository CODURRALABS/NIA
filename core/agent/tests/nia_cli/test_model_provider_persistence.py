"""Tests that provider selection via `nia model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated NIA_HOME with a minimal string-format config."""
    home = tmp_path / "nia"
    home.mkdir()
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from nia_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from nia_cli.auth import _update_config_for_provider
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from nia_cli.main import _model_flow_api_key_provider
        ...
        with patch("nia_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("nia_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from nia_cli.main import _model_flow_copilot
        from nia_cli.config import load_config

        with patch(
            "nia_cli.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "nia_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "nia_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "nia_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from nia_cli.main import _model_flow_copilot_acp
        from nia_cli.config import load_config

        with patch(
            "nia_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "nia_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("nia_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("nia_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("nia_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("nia_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("nia_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("nia_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_lmstudio_provider_saved_when_selected(self, config_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "nia_cli.models.fetch_lmstudio_models",
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from nia_cli.main import _model_flow_api_key_provider
        from nia_cli.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("nia_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("nia_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.nia/.env"):
    ...
    def test_stepfun_provider_saved_with_selected_region(self, config_home, monkeypatch):
        from nia_cli.main import _model_flow_stepfun
        from nia_cli.config import load_config, get_env_value

        monkeypatch.setenv("STEPFUN_API_KEY", "stepfun-test-key")

        with patch(
            "nia_cli.main._prompt_provider_choice",
            return_value=1,
        ), patch(
            "nia_cli.models.fetch_api_models",
