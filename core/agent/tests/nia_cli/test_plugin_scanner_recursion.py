...
from nia_cli.plugins import PluginManager, PluginManifest
...
def _enable(nia_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<nia_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        nia_home = Path(os.environ["NIA_HOME"])  # set by hermetic conftest fixture
        user_plugins = nia_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(nia_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        nia_home = Path(os.environ["NIA_HOME"])  # set by hermetic conftest fixture
        _write_plugin(nia_home / "plugins", ["p1"])
        _enable(nia_home, "p1")
...
class TestBackendGate:
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(nia_home, "image_gen/fancy")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(nia_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(nia_home, "bad-img-plugin")
