...
Covers:
 - Normal path (tokens already in NIA auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — NIA
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `nia auth openai-codex`. The old
...
@pytest.fixture()
def nia_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in NIA auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(nia_auth_only_env):
    """openai-codex appears when tokens are already in NIA auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or NIA
    auth store."""
    nia_home = tmp_path / ".nia"
    nia_home.mkdir()
