"""Tests for IRC gateway configuration via `nia setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh NIA install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new NIA install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nia")
            monkeypatch.setenv("IRC_NICKNAME", "nia-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nia setup gateway` experience with IRC present."""
