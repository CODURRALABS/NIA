"""Tests for nia_cli/goals.py — persistent cross-turn goals."""
...
def nia_home(tmp_path, monkeypatch):
    """Isolated NIA_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves NIA_HOME.
    from nia_cli import goals
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from nia_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from nia_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, nia_home):
    ...
    def test_set_then_status(self, nia_home):
        from nia_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, nia_home):
        from nia_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, nia_home):
        from nia_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, nia_home):
    ...
    def test_evaluate_after_turn_done(self, nia_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, nia_home):
        from nia_cli import goals
        from nia_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, nia_home):
    ...
    def test_evaluate_after_turn_inactive(self, nia_home):
    ...
    def test_continuation_prompt_shape(self, nia_home):
        ...
        mgr.set("port goal command to nia")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to nia" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from nia_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
