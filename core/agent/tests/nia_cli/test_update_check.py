"""Tests for the update check mechanism in nia_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from nia_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("nia_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    with patch("nia_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import nia_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "nia_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when NIA_HOME has no git repo."""
    import nia_cli.banner as banner
    ...
    # Point NIA_HOME at a temp dir with no nia-agent/.git
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    with patch("nia_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from nia_cli.main import _invalidate_update_cache

    # Build a fake ~/.nia with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"NIA_HOME": str(default_home)}):
