"""Tests for nia_cli configuration management."""
...
class TestGetNIAHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_HOME", None)
            home = get_nia_home()
            assert home == Path.home() / ".nia"

    def test_env_override(self):
        with patch.dict(os.environ, {"NIA_HOME": "/custom/path"}):
            home = get_nia_home()
            assert home == Path("/custom/path")


class TestEnsureNIAHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"NIA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIA_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"NIA_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from nia_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
