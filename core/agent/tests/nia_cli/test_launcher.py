"""Tests for the top-level `./nia` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nia` should use `nia_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nia"
    called = []

    fake_main_module = types.ModuleType("nia_cli.main")

    def fake_main():
        called.append("nia_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nia_cli.main", fake_main_module)
