"""Tests for Qwen OAuth provider authentication (nia_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_refresh_qwen_cli_tokens_success(qwen_env):
    ...
    with patch("nia_cli.auth.httpx") as mock_httpx:
...
def test_resolve_qwen_runtime_credentials_triggers_refresh(qwen_env):
    ...
    with patch(
        "nia_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
...
def test_resolve_qwen_runtime_credentials_base_url_env_override(qwen_env, monkeypatch):
    ...
    _write_qwen_creds(qwen_env, tokens)
    monkeypatch.setenv("NIA_QWEN_BASE_URL", "https://custom.qwen.ai/v1")
