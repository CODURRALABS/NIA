...
from nia_cli import setup as setup_mod
from nia_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("nia_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import nia_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(nia_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("nia_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("nia_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "nia_cli.main._model_flow_named_custom",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("nia_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("nia_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("nia_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("nia_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "nia_cli.setup.get_nous_subscription_features",
    ...
    from nia_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("nia_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("nia_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "nia_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_offer_launch_chat_relaunches_via_bin(monkeypatch):
    from nia_cli import setup as setup_mod
    from nia_cli import relaunch as relaunch_mod

    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(relaunch_mod, "resolve_nia_bin", lambda: "/usr/local/bin/nia")
    ...
    assert exec_calls == [("/usr/local/bin/nia", ["/usr/local/bin/nia", "chat"])]
...
def test_offer_launch_chat_falls_back_to_module(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(relaunch_mod, "resolve_nia_bin", lambda: None)
    ...
    assert exec_calls == [(sys.executable, [sys.executable, "-m", "nia_cli.main", "chat"])]
