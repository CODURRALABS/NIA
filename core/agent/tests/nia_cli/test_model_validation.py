"""Tests for provider-aware `/model` validation in nia_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("nia_cli.models.fetch_api_models", return_value=api_models), \
         patch("nia_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:nia-3", "openrouter")
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("nia_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("nia_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_falls_back_to_curated_defaults_without_stale_opus(self):
        with patch("nia_cli.models._resolve_copilot_catalog_api_key", return_value="gh-token"), \
             patch("nia_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("nia_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("nia_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("nia_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("nia_cli.models.urllib.request.urlopen", return_value=_Resp()):
...
class TestValidateApiFallback:
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "nia_cli.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("nia_cli.models.provider_model_ids", return_value=[]):
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("nia_cli.models.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_fetch_lmstudio_models_raises_auth_error_on_401(self):
        ...
        with patch("nia_cli.models.urllib.request.urlopen", side_effect=http_error):
    ...
    def test_fetch_lmstudio_models_returns_empty_on_network_error(self):
        with patch(
            "nia_cli.models.urllib.request.urlopen",
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("nia_cli.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a NIA User-Agent.
    ...
    def test_probe_sends_nia_user_agent(self):
        ...
        assert ua.startswith("nia-cli/"), (
            f"User-Agent must advertise nia-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("nia-cli/")
