...
from nia_cli import config as nia_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nia_main.subprocess, "run", fake_run)

    stash_ref = nia_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M nia_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert nia_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = nia_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = nia_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    nia_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes nia unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="nia_cli/main.py\n", stderr="", returncode=0)
    ...
    result = nia_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    assert result is False
    out = capsys.readouterr().out
    assert "Conflicted files:" in out
    assert "nia_cli/main.py" in out
...
def test_restore_stashed_changes_auto_resets_non_interactive(monkeypatch, tmp_path, capsys):
    ...
    result = nia_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        nia_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(nia_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(nia_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(nia_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(nia_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(nia_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(nia_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(nia_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(nia_main, "_load_installable_optional_extras", lambda: ["matrix", "mcp"])
    ...
    nia_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(nia_main.subprocess, "run", side_effect)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        nia_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        nia_main, "_restore_stashed_changes",
