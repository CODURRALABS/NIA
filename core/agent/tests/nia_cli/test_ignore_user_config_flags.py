"""Tests for --ignore-user-config and --ignore-rules flags on `nia chat`.
...
files. In NIA the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.nia/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → NIACLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("NIA_IGNORE_USER_CONFIG", "NIA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``NIA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<nia_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._nia_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_nia_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("NIA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``NIACLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting NIA_IGNORE_RULES=1 flips NIACLI.ignore_rules True."""
        monkeypatch.setenv("NIA_IGNORE_RULES", "1")

        # Import NIACLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of NIACLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.NIACLI)
        # Replicate the exact logic from cli.py NIACLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("NIA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("NIA_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.NIACLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``nia_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("NIA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("NIA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("NIA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("NIA_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("NIA_IGNORE_USER_CONFIG") == "1"
        assert "NIA_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "NIA_IGNORE_USER_CONFIG" not in os.environ
        assert "NIA_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="nia")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real nia parser accepts both flags."""
        ...
        import inspect
        import nia_cli.main as hm
        src = inspect.getsource(hm)
        assert "NIA_IGNORE_USER_CONFIG" in src
        assert "NIA_IGNORE_RULES" in src
