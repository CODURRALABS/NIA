...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["nia", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(sys.modules, "nia_cli.plugins", types.SimpleNamespace(discover_plugins=lambda: None))
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["nia", "-z", "hello", "--toolsets", "web,terminal"])
    monkeypatch.setitem(sys.modules, "nia_cli.plugins", types.SimpleNamespace(discover_plugins=lambda: None))
    monkeypatch.setitem(sys.modules, "tools.mcp_tool", types.SimpleNamespace(discover_mcp_tools=lambda: None))
    monkeypatch.setattr(config_mod, "load_config", lambda: {})
    monkeypatch.setattr(config_mod, "get_container_exec_info", lambda: None)
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from nia_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "nia -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["NIA_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(model="nous/nia-test", provider="nous", toolsets="web, terminal")

    env = captured["env"]
    assert env["NIA_MODEL"] == "nous/nia-test"
    assert env["NIA_INFERENCE_MODEL"] == "nous/nia-test"
    assert env["NIA_TUI_PROVIDER"] == "nous"
    assert env["NIA_INFERENCE_PROVIDER"] == "nous"
    assert env["NIA_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["NIA_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("nia-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import nia_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "nia_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "nia --tui --resume 20260409_000001_abc123" in out
    assert 'nia --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "nia --tui --resume actual_session" in out
