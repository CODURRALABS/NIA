...
import nia_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "nia-gateway.service"
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `nia gateway restart`)."""
        unit_path = tmp_path / "nia-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nia gateway install" in out
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("NIA_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("NIA_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("NIA_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.nia.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_nia_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: nia gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup nia gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.nia.gateway"])
            ),
        )
...
class TestSystemUnitNIAHome:
    """NIA_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("NIA_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'NIA_HOME=/home/alice/.nia' in unit
        assert '/root/.nia' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: NIA_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("NIA_HOME", "/root/.nia/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'NIA_HOME=/home/alice/.nia/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_nia_home(self, monkeypatch):
        # Custom NIA_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("NIA_HOME", "/opt/nia-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'NIA_HOME=/opt/nia-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's NIA_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        nia_home = str(gateway_cli.get_nia_home().resolve())
        assert f'NIA_HOME={nia_home}' in unit


class TestNIAHomeForTargetUser:
    """Unit tests for _nia_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("NIA_HOME", raising=False)

        result = gateway_cli._nia_home_for_target_user("/home/alice")
        assert result == "/home/alice/.nia"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._nia_home_for_target_user("/home/alice")
        assert result == "/home/alice/.nia/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("NIA_HOME", "/opt/nia")

        result = gateway_cli._nia_home_for_target_user("/home/alice")
        assert result == "/opt/nia"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "nia gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_nia_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.nia should not produce a --profile flag."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        result = gateway_cli._profile_arg(str(nia_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.nia/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".nia" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile NIA_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "nia"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.nia/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".nia" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".nia" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_nia_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".nia" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.nia.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".nia" / "nia-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".nia" / "nia-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/nia", str(tmp_path / "alice"))
        assert result == "/opt/nia"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".nia" / "nia-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".nia" / "nia-agent"
        ...
        monkeypatch.setenv("NIA_HOME", str(root_home / ".nia"))
        monkeypatch.setattr(gateway_cli, "get_nia_home", lambda: root_home / ".nia")
        ...
        assert "WorkingDirectory=/home/alice/.nia/nia-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "nia-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "nia-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'nia gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'nia gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'nia gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "nia gateway run" in out


class TestLegacyNIAUnitDetection:
    """Tests for _find_legacy_nia_units / has_legacy_nia_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``nia.service`` unit behind when the service was
    renamed to ``nia-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``nia.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``nia-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=NIA Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m nia_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_nia_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "nia.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_nia_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "nia.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_nia_units() is True

    def test_detects_legacy_nia_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "nia.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_nia_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``nia-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "nia-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "nia-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "nia-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_nia_units() is False

    def test_ignores_unrelated_nia_service(self, tmp_path, monkeypatch):
        """Third-party ``nia.service`` that isn't ours stays untouched.

        If a user has some other package named ``nia`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "nia.service").write_text(
            "[Unit]\nDescription=Some Other NIA\n[Service]\n"
            "ExecStart=/opt/other-nia/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_nia_units() == []
        assert gateway_cli.has_legacy_nia_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "nia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "nia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m nia_cli.main gateway run
          - python path/to/nia_cli/main.py gateway run
          - nia gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m nia_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nia/nia_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/nia gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nia/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"nia.service" if i == 0 else f"nia.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nia.service" in out
        assert "nia gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "nia.service"
...
class TestRemoveLegacyNIAUnits:
    """Tests for remove_legacy_nia_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_nia_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_nia_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop nia.service" in c for c in cmds_joined)
        assert any("--user disable nia.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo nia gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop nia.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable nia.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "nia.service"
        system_legacy = system_dir / "nia.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (nia-gateway-coder.service)
        ...
        profile_unit = user_dir / "nia-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "nia-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_nia_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `nia gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import nia_cli.main as cli_main
        ...
        # public, just confirm that `nia gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import nia_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "nia_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_nia_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nia_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_nia_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_nia_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_nia_units", fake_remove)
