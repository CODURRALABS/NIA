...
`NIA_REDACT_SECRETS` at module-import time. `nia_cli/main.py` at
...
in .env), the toggle was silently ignored in both `nia chat` and
`nia gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `NIA_REDACT_SECRETS`
env var in `nia_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports nia_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("NIA_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import nia_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('NIA_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["NIA_HOME"] = str(nia_home)
    env.pop("NIA_REDACT_SECRETS", None)
...
def test_redact_secrets_default_false_when_unset(tmp_path):
    ...
    `security.redact_secrets: true` explicitly (or NIA_REDACT_SECRETS=true).
    """
    nia_home = tmp_path / ".nia"
    nia_home.mkdir()
    (nia_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env NIA_REDACT_SECRETS takes precedence over config.yaml."""
    nia_home = tmp_path / ".nia"
    nia_home.mkdir()
    (nia_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (nia_home / ".env").write_text("NIA_REDACT_SECRETS=true\n")
