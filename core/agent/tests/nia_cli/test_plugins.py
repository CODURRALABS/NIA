"""Tests for the NIA plugin system (nia_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<nia_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<nia_home>/plugins/``; we derive
    ``<nia_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <NIA_HOME>/config.yaml.
        # Config is always read from NIA_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        nia_home_str = os.environ.get("NIA_HOME")
        if nia_home_str:
            nia_home = Path(nia_home_str)
        else:
            nia_home = base.parent
        nia_home.mkdir(parents=True, exist_ok=True)
        cfg_path = nia_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.nia/plugins/ are discovered."""
        plugins_dir = tmp_path / "nia_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("NIA_HOME", str(tmp_path / "nia_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".nia" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under nia_plugins.<name>."""
        plugins_dir = tmp_path / "nia_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("NIA_HOME", str(tmp_path / "nia_test"))

        # Clean up any prior namespace module
        sys.modules.pop("nia_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "nia_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nia_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import nia_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("nia_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "nia_test" / "plugins"
        # Set NIA_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("nia_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("nia_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("nia_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("nia_cli.plugins.PluginContext.dispatch_tool.__module__", "nia_cli.plugins"):
