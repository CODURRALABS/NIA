...
Complements tests/nia_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/nia_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_max_runtime_terminates_overrun_worker(kanban_home):
    ...
    import nia_cli.kanban_db as _kb
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    """list_profiles_on_disk returns directories under ~/.nia/profiles/
    that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("NIA_HOME", raising=False)
    profiles = tmp_path / ".nia" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom NIA_HOME root."""
...
def test_parse_duration_accepts_formats():
    from nia_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`nia kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: nia_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "nia_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty NIA_HOME must not error with
    ...
    env = {**os.environ, "NIA_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "nia_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh NIA_HOME must create the
...
def test_cli_show_json_carries_runs(kanban_home):
    """nia kanban show --json must include runs[] so scripts that
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    nia subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("NIA_KANBAN_TASK") == tid
    assert env.get("NIA_PROFILE") == "some-profile"
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`nia kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`nia kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from nia_cli import kanban as kb_cli
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "nia gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "nia gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "nia gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "nia gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`nia kanban daemon` (no --force) is a deprecation stub."""
    from nia_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "nia gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `nia kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import nia_cli.config as _cfg_mod
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """NIA_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("NIA_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("NIA_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
