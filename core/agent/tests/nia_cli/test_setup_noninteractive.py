...
import pytest
from nia_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from nia_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("nia_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `nia setup` should reach setup.py, which prints headless guidance."""
        from nia_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("nia_cli.setup.ensure_nia_home"),
            patch("nia_cli.setup.load_config", return_value={}),
            patch("nia_cli.setup.get_nia_home", return_value="/tmp/.nia"),
            patch("nia_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "nia config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `nia` should not prompt for input when no provider exists and stdin is headless."""
        from nia_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("nia_cli.main._has_any_provider_configured", return_value=False),
            patch("nia_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`nia setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["nia", "setup", "tts"])
