...
def test_resolved_api_call_timeout_priority(monkeypatch, tmp_path):
          ...
          openrouter:
            ...
            models:
              openai/gpt-4o-mini:
                timeout_seconds: 42
        """)
    monkeypatch.setenv("NIA_API_TIMEOUT", "999")
    ...
    import importlib
    from nia_cli import config as cfg_mod
    importlib.reload(cfg_mod)
    from nia_cli import timeouts as to_mod
    ...
    monkeypatch.delenv("NIA_API_TIMEOUT", raising=False)
...
def test_resolved_api_call_stale_timeout_priority(monkeypatch, tmp_path):
          ...
          openai-codex:
            ...
            models:
              gpt-5.4:
                stale_timeout_seconds: 1800
        """)
    monkeypatch.setenv("NIA_API_CALL_STALE_TIMEOUT", "999")
    ...
    monkeypatch.delenv("NIA_API_CALL_STALE_TIMEOUT", raising=False)
...
def test_explicit_non_stream_stale_timeout_is_honored_for_local_endpoints(monkeypatch, tmp_path):
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
    (tmp_path / ".env").write_text("", encoding="utf-8")
    monkeypatch.setenv("NIA_API_CALL_STALE_TIMEOUT", "300")
