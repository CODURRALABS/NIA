"""Tests for Codex auth — tokens stored in NIA auth store (~/.nia/auth.json)."""
...
def _setup_nia_auth(nia_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the NIA auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    nia_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (nia_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("NIA_HOME", str(nia_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    nia_home = tmp_path / "nia"
    _setup_nia_auth(nia_home, access_token="")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_nia_auth(nia_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
    ...
    monkeypatch.setattr("nia_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    nia_home = tmp_path / "nia"
    _setup_nia_auth(nia_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("NIA_HOME", str(nia_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to NIA auth store, not ~/.codex/."""
    nia_home = tmp_path / "nia"
    codex_home = tmp_path / "codex-cli"
    nia_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "nia-at", "refresh_token": "nia-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches NIA store
    assert not (codex_home / "auth.json").exists()

    # NIA auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "nia-at"


def test_resolve_returns_nia_auth_store_source(tmp_path, monkeypatch):
    nia_home = tmp_path / "nia"
    _setup_nia_auth(nia_home)
    monkeypatch.setenv("NIA_HOME", str(nia_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "nia-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("nia_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "nia_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("nia_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("nia_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
