"""Tests for ``nia debug`` CLI command and debug utilities."""
...
@pytest.fixture
def nia_home(tmp_path, monkeypatch):
    """Set up an isolated NIA_HOME with minimal logs."""
    home = tmp_path / ".nia"
    home.mkdir()
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from nia_cli.debug import _upload_paste_rs
        ...
        with patch("nia_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "nia_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from nia_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from nia_cli.debug import upload_to_pastebin

        with patch("nia_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("nia_cli.debug._upload_paste_rs",
             ...
             patch("nia_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, nia_home):
        from nia_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, nia_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (nia_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, nia_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = nia_home / "logs" / "agent.log"
        from nia_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, nia_home):
        ...
        (nia_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, nia_home):
    ...
    def test_unknown_log_returns_none(self, nia_home):
        from nia_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, nia_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from nia_cli.debug import _capture_log_snapshot

        logs_dir = nia_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, nia_home):
    ...
    def test_falls_back_when_primary_empty(self, nia_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of NIA_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def nia_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated NIA_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".nia"
        home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(home))
        ...
        monkeypatch.delenv("NIA_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, nia_home_with_secret):
        from nia_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, nia_home_with_secret):
        from nia_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_overrides_unset_env_var(self, nia_home_with_secret):
        ...
        input unchanged when NIA_REDACT_SECRETS is unset, and the feature
        ...
        assert os.environ.get("NIA_REDACT_SECRETS", "") == ""
    ...
    def test_capture_default_log_snapshots_threads_redact(
        self, nia_home_with_secret
    ):
        from nia_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, nia_home):
        from nia_cli.debug import collect_debug_report

        with patch("nia_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- nia dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- nia dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, nia_home):
        from nia_cli.debug import collect_debug_report

        with patch("nia_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, nia_home):
        from nia_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, nia_home):
        from nia_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, nia_home, capsys):
        ...
        with patch("nia_cli.dump.run_dump"), \
             patch("nia_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("nia_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, nia_home, capsys):
        ...
        with patch("nia_cli.dump.run_dump"), \
             patch(
                 "nia_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, nia_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, nia_home, capsys):
        ...
        with patch("nia_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- nia dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- nia dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- nia dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, nia_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from nia_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("nia_cli.dump.run_dump"), \
             patch("nia_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("nia_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, nia_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, nia_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, nia_home_with_secret, capsys
    ):
        ...
        with patch("nia_cli.dump.run_dump"), \
             patch("nia_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("nia_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "nia debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, nia_home):
        from nia_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from nia_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from nia_cli.debug import delete_paste
        ...
        with patch("nia_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.nia/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``nia debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, nia_home):
        ...
        import ast
        import inspect
        from nia_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, nia_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from nia_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, nia_home):
    ...
    def test_merges_with_existing_pending(self, nia_home):
        """Subsequent calls merge into existing pending.json."""
        from nia_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, nia_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, nia_home):
    ...
    def test_sweep_deletes_expired_entries(self, nia_home):
        ...
        with patch("nia_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, nia_home):
        from nia_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("nia_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, nia_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, nia_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, nia_home):
        from nia_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("nia_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, nia_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: nia debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from nia_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("nia_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("nia_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, nia_home, capsys):
        from nia_cli.debug import run_debug_share
        ...
        with patch("nia_cli.dump.run_dump"), \
             ...
             patch("nia_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, nia_home, capsys):
        from nia_cli.debug import run_debug_share
        ...
        with patch("nia_cli.dump.run_dump"), \
             patch("nia_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("nia_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, nia_home, capsys):
        from nia_cli.debug import run_debug_share
