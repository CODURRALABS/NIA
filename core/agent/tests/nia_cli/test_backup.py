"""Tests for nia backup and import commands."""
...
def _make_nia_tree(root: Path) -> None:
    """Create a realistic ~/.nia directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "nia_state.db").write_bytes(b"fake-state")
    ...
    # nia-agent repo (should be EXCLUDED)
    (root / "nia-agent").mkdir(exist_ok=True)
    (root / "nia-agent" / "run_agent.py").write_text("# big file\n")
    (root / "nia-agent" / ".git").mkdir()
    (root / "nia-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_nia_agent(self):
        from nia_cli.backup import _should_exclude
        assert _should_exclude(Path("nia-agent/run_agent.py"))
        assert _should_exclude(Path("nia-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        _make_nia_tree(nia_home)

        monkeypatch.setenv("NIA_HOME", str(nia_home))
        # get_default_nia_root needs this
    ...
    def test_excludes_nia_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include nia-agent/ directory."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        _make_nia_tree(nia_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "nia-agent" in n]
            assert agent_files == [], f"nia-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/nia-backup-*.zip."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("nia-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid NIA backup."""
        from nia_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only nia_state.db (old wrong name) is rejected."""
        from nia_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["nia_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into nia home."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (nia_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (nia_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (nia_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (nia_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_nia_prefix(self, tmp_path, monkeypatch):
        """Import strips .nia/ prefix if all entries share it."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".nia/config.yaml": "model: test\n",
            ".nia/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (nia_home / "config.yaml").read_text() == "model: test\n"
        assert (nia_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_nia_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a nia backup."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (nia_home / "config.yaml").exists()
        # traversal file should NOT exist outside nia home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (nia_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (nia_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (nia_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (nia_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".nia"
        src_home.mkdir(parents=True)
        _make_nia_tree(src_home)

        monkeypatch.setenv("NIA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from nia_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".nia"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("NIA_HOME", str(dst_home))
        ...
        # nia-agent should NOT be present
        assert not (dst_home / "nia-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from nia_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without nia markers fails validation."""
        import io
        from nia_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_nia(self):
        """Detects .nia/ prefix wrapping all entries."""
        import io
        from nia_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".nia/config.yaml", "test")
            zf.writestr(".nia/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".nia/"
...
class TestBackupEdgeCases:
    def test_nonexistent_nia_home(self, tmp_path, monkeypatch):
        """Backup exits when nia home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".nia"
        monkeypatch.setenv("NIA_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("nia-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_nia_home(self, tmp_path, monkeypatch):
        """Backup handles empty nia home (no files to back up)."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        # Only excluded dirs, no actual files
        (nia_home / "__pycache__").mkdir()
        (nia_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_skips_output_zip_inside_nia(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside nia root."""
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside nia home
        out_zip = nia_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        (nia_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (nia_home / "config.yaml").exists()
        assert (nia_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (nia_home / "profiles" / "coder" / "config.yaml").exists()
        assert (nia_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "nia -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import nia_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "nia_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from nia_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def nia_home(self, tmp_path):
        """Create a fake NIA_HOME with critical state files."""
        home = tmp_path / ".nia"
        home.mkdir()
    ...
    def test_creates_snapshot(self, nia_home):
        from nia_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)
        assert snap_id is not None
        snap_dir = nia_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, nia_home):
        from nia_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", nia_home=nia_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, nia_home):
        from nia_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)
        db_copy = nia_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, nia_home):
        from nia_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)
        assert (nia_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, nia_home):
        from nia_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)
        with open(nia_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(nia_home=empty) is None

    def test_list_snapshots(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", nia_home=nia_home)
        id2 = create_quick_snapshot(label="second", nia_home=nia_home)

        snaps = list_quick_snapshots(nia_home=nia_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", nia_home=nia_home)
        snaps = list_quick_snapshots(limit=3, nia_home=nia_home)
        assert len(snaps) == 3

    def test_restore_config(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)

        (nia_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (nia_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, nia_home=nia_home)
        assert result is True
        assert "openrouter" in (nia_home / "config.yaml").read_text()

    def test_restore_state_db(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(nia_home=nia_home)

        conn = sqlite3.connect(str(nia_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, nia_home=nia_home)
    ...
    def test_restore_nonexistent(self, nia_home):
        from nia_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", nia_home=nia_home) is False

    def test_auto_prune(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", nia_home=nia_home)
        snaps = list_quick_snapshots(limit=100, nia_home=nia_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, nia_home):
        from nia_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", nia_home=nia_home)
        deleted = prune_quick_snapshots(keep=3, nia_home=nia_home)
        assert deleted == 7
        assert len(list_quick_snapshots(nia_home=nia_home)) == 3

    def test_snapshot_includes_pairing_directories(self, nia_home):
        ...
        (nia_home / "platforms" / "pairing").mkdir(parents=True)
        (nia_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (nia_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (nia_home / "pairing").mkdir()
        (nia_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (nia_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, nia_home):
        ...
        assert restore_quick_snapshot(snap_id, nia_home=nia_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, nia_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (nia update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``nia update``
    ...
    def test_creates_backup_under_backups_dir(self, nia_home):
        from nia_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(nia_home=nia_home)
        assert out is not None
        assert out.exists()
        assert out.parent == nia_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, nia_home):
        """Pre-update backup should include the same user data that
        ``nia backup`` would, and should exclude the same directories."""
        from nia_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(nia_home=nia_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # nia-agent repo excluded
        assert not any(n.startswith("nia-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, nia_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from nia_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(nia_home=nia_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(nia_home=nia_home)
    ...
    def test_rotation_keeps_only_n(self, nia_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(nia_home=nia_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (nia_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, nia_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from nia_cli.backup import create_pre_update_backup

        (nia_home / "backups").mkdir(exist_ok=True)
        manual = nia_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(nia_home=nia_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from nia_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(nia_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, nia_home):
        ...
        """
        from nia_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(nia_home=nia_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, nia_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, nia_home):
        ...
        first = create_pre_update_backup(nia_home=nia_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(nia_home=nia_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(nia_home=nia_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def nia_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".nia"
        root.mkdir()
        _make_nia_tree(root)
        # Point NIA_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("NIA_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for nia_cli.config + nia_constants so they pick up NIA_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("nia_cli.config") or mod == "nia_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, nia_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from nia_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "nia import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((nia_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, nia_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (nia_home / "backups").exists() or not list(
            (nia_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, nia_home, capsys):
        from nia_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, nia_home, capsys):
        ...
        import yaml
        (nia_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("nia_cli.config"):
    ...
    def test_config_disabled_is_silent(self, nia_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((nia_home / "backups").glob("pre-update-*.zip")) \
            if (nia_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, nia_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (nia claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``nia claw migrate`` runs before mutating ~/.nia/."""
    ...
    def test_creates_backup_under_backups_dir(self, nia_home):
        from nia_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(nia_home=nia_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `nia
        # import` and the update-backup listing both pick them up.
        assert out.parent == nia_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, nia_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``nia backup`` / ``create_pre_update_backup`` — no drift."""
        from nia_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(nia_home=nia_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_nia_import(self, nia_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid NIA
        backup — `nia import` should accept it."""
        from nia_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(nia_home=nia_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, nia_home):
        from nia_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(nia_home=nia_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(nia_home=nia_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, nia_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(nia_home=nia_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((nia_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_nia_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.nia yet — nothing to back up."""
        from nia_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(nia_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, nia_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from nia_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(nia_home=nia_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(nia_home=nia_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
