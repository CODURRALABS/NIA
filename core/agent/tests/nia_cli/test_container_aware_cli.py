...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NIA_HOME with .container-mode file."""
    nia_home = tmp_path / ".nia"
    nia_home.mkdir()
    monkeypatch.setenv("NIA_HOME", str(nia_home))
    monkeypatch.delenv("NIA_DEV", raising=False)

    container_mode = nia_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nia-agent\n"
        "exec_user=nia\n"
        "nia_bin=/data/current-package/bin/nia\n"
    )
    return nia_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nia_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nia-agent"
    assert info["exec_user"] == "nia"
    assert info["nia_bin"] == "/data/current-package/bin/nia"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("nia_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_nia_dev(container_env, monkeypatch):
    """Returns None when NIA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NIA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_nia_dev_zero(container_env, monkeypatch):
    """NIA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NIA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        nia_home = Path(tmpdir) / ".nia"
        nia_home.mkdir()
        (nia_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("nia_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_nia_home": lambda: nia_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=nia-custom\n"
        "exec_user=myuser\n"
        "nia_bin=/opt/nia/bin/nia\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "nia-custom"
    assert info["exec_user"] == "myuser"
    assert info["nia_bin"] == "/opt/nia/bin/nia"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nia_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nia"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nia-agent" in cmd
    assert "/data/current-package/bin/nia" in cmd
    assert "chat" in cmd
