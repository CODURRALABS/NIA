"""Tests for nia_cli/completion.py — shell completion script generation."""
...
from nia_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real nia structure."""
    p = argparse.ArgumentParser(prog="nia")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_nia_completion()" in out
        assert "complete -F _nia_completion nia" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c nia -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from nia_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_nia_profiles()" in out
        assert 'profiles_dir="$HOME/.nia/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'nia profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _nia_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_nia_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _nia_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_nia_profiles()" in out
        assert "$HOME/.nia/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__nia_profiles" in out
        count = out.count("(__nia_profiles)")
