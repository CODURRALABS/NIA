...
Ensures ``nia update`` correctly detects running gateways managed by
...
from nia_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """nia_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units nia-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="nia-gateway.service loaded active running NIA Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>NIA_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: nia gateway run'."""
        ...
        assert "Restart manually: nia gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: nia gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.nia.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".nia" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted nia-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="nia-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "nia_cli.gateway._graceful_restart_via_sigusr1",
...
class TestServicePidExclusion:
    ...
    where ``nia update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.nia.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="nia-gateway.service loaded inactive dead NIA Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.nia/nia-agent/venv/bin/python -m nia_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.nia/nia-agent/venv/bin/python -m nia_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda nia_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``nia update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point NIA_HOME at a temp dir so the marker file lands there
        nia_home = tmp_path / ".nia"
        nia_home.mkdir()
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        import nia_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_nia_home", lambda: nia_home)
        # Also patch the module-level ref used by cmd_update
        import nia_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_nia_home", lambda: nia_home)
        ...
        exit_code_path = nia_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy nia.service warning printed by `nia update`.

    Users who installed NIA before the service rename often have a
    dormant ``nia.service`` that starts flap-fighting the current
    ``nia-gateway.service`` after PR #5646. Every ``nia update``
    should remind them to run ``nia gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "nia.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy NIA gateway unit(s) detected" in captured
        assert "nia.service" in captured
        assert "nia gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy NIA gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (nia-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``nia.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "nia-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "nia-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy NIA gateway" not in captured
        assert "nia-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy NIA gateway" in captured
