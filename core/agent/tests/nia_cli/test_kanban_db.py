"""Tests for the Kanban DB layer (nia_cli.kanban_db)."""
...
from nia_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated NIA_HOME with an empty kanban DB."""
    home = tmp_path / ".nia"
    home.mkdir()
...
def test_scratch_workspace_created_under_nia_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `nia -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's NIA_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's NIA_HOME."""

    def _set_home(self, monkeypatch, tmp_path, nia_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(nia_home))
        monkeypatch.delenv("NIA_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_nia(
        self, tmp_path, monkeypatch
    ):
        # Standard install: NIA_HOME == ~/.nia, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.nia/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.nia/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.nia/kanban.db.
        default_home = tmp_path / ".nia"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # NIA_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `nia -p coder`).
        monkeypatch.setenv("NIA_HOME", str(profile_home))
    ...
    def test_docker_custom_nia_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: NIA_HOME points outside ~/.nia.
        # `get_default_nia_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".nia"`.
        custom_root = tmp_path / "opt" / "nia"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: NIA_HOME=/opt/nia/profiles/coder;
        # `get_default_nia_root()` walks up to /opt/nia because
    ...
    def test_explicit_override_via_nia_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: NIA_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".nia"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(profile_home))
        monkeypatch.setenv("NIA_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIA_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # NIA_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile NIA_HOME and reads.
        monkeypatch.setenv("NIA_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_nia_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # NIA_KANBAN_DB pins the file path directly and beats both
        # NIA_KANBAN_HOME and the `get_default_nia_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(default_home))
        monkeypatch.setenv("NIA_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("NIA_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows NIA_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_nia_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # NIA_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".nia"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("NIA_HOME", str(default_home))
        monkeypatch.setenv("NIA_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("NIA_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows NIA_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(default_home))
        monkeypatch.setenv("NIA_KANBAN_DB", "   ")
        monkeypatch.setenv("NIA_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject NIA_KANBAN_DB
        # and NIA_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites NIA_HOME.
        default_home = tmp_path / ".nia"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["NIA_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["NIA_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["NIA_KANBAN_TASK"] == "t_dispatch_env"
