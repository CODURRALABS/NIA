"""
Tests for --yes / --force flag separation in `nia skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_y_alias(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "skills", "install", "test/skill", "-y"],
    )
...
def test_cli_skills_install_force_sets_force(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "skills", "install", "test/skill", "--force"],
    )
...
def test_cli_skills_install_force_and_yes_together(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nia", "skills", "install", "test/skill", "--force", "--yes"],
    )
