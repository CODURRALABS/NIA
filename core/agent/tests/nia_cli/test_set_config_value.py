...
from nia_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_nia_home(tmp_path):
    """Point NIA_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"NIA_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_nia_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_nia_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_nia_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_nia_home):
        set_config_value(key, "secret-456")
        env_content = _read_env(_isolated_nia_home)
        assert f"{key}=secret-456" in env_content
        assert key not in _read_config(_isolated_nia_home)

    def test_case_insensitive(self, _isolated_nia_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_nia_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_nia_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_nia_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_nia_home)

    def test_nested_key(self, _isolated_nia_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_nia_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_nia_home)

    def test_terminal_image_goes_to_config(self, _isolated_nia_home):
    ...
    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_nia_home):
    ...
    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_nia_home):
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_nia_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_nia_home):
    ...
    def test_zero_routes_to_config(self, _isolated_nia_home):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_nia_home):
...
class TestListNavigation:
    """nia config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_nia_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_nia_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_nia_home))
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_nia_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_nia_home):
