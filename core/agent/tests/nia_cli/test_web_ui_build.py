...
Critical invariant: the Vite build outputs to nia_cli/web_dist/
(vite.config.ts: outDir: "../nia_cli/web_dist"), NOT web/dist/.
...
from nia_cli.main import _web_ui_build_needed, _build_web_ui
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "nia_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in nia_cli/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_skips_npm_when_dist_is_fresh(self, tmp_path):
        ...
        with patch("nia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nia_cli.main.subprocess.run") as mock_run:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("nia_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nia_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
            result = _build_web_ui(web_dir)
