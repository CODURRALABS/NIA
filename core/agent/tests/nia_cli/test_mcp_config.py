"""
Tests for nia_cli.mcp_config — ``nia mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.config.get_nia_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "nia_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "nia_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from nia_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from nia_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_nia_home in the mcp_config module namespace
        monkeypatch.setattr(
            "nia_cli.mcp_config.get_nia_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    def test_add_no_transport(self, capsys):
        ...
        from nia_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "nia_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_connection_failure_save_disabled(
        ...
        monkeypatch.setattr(
            "nia_cli.mcp_config._probe_single_server", mock_probe_fail
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "nia_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    def test_test_not_found(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from nia_cli.mcp_config import cmd_mcp_test
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from nia_cli.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_remove_mcp_server(self, tmp_path):
        from nia_cli.mcp_config import (
            _save_mcp_server,
            _remove_mcp_server,
            _get_mcp_servers,
        )
    ...
    def test_remove_nonexistent(self, tmp_path):
        from nia_cli.mcp_config import _remove_mcp_server

        assert _remove_mcp_server("ghost") is False

    def test_env_key_for_server(self):
        from nia_cli.mcp_config import _env_key_for_server
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from nia_cli.mcp_config import cmd_mcp_login
