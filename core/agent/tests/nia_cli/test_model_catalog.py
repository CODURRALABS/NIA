"""Tests for nia_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate NIA_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".nia"
    home.mkdir()
    ...
    import importlib
    from nia_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from nia_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in nia_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from nia_cli import model_catalog
        from nia_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from nia_cli import model_catalog
        from nia_cli.models import get_curated_nous_model_ids
