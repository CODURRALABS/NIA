"""Tests for nia_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _NIA_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `nia tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("nia_cli.tools_config.save_config"):
...
def test_toolset_has_keys_for_vision_accepts_codex_auth(tmp_path, monkeypatch):
    monkeypatch.setenv("NIA_HOME", str(tmp_path))
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for Local Sovereign Environment/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (nia-cli, nia-telegram, etc.) must NOT
    ...
    ``nia tools``, but nia-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "nia-cli",  # <-- the culprit
    ...
    # nia-cli must NOT survive — it's a platform default, not an MCP server
    assert "nia-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_nia_telegram():
    """Same bug for Telegram — nia-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "nia-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "nia-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "nia-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("nia_cli.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "nia_cli.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("nia_cli.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("nia_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "nia_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config: configured.append(ts_key),
    )
    monkeypatch.setattr("nia_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("nia_cli.tools_config.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setattr("nia_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "nia_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "nia_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "nia_cli.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "nia_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from nia_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """nia-gateway includes list should cover all messaging platforms."""
        from nia_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["nia-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from nia_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from nia_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for Local Sovereign Environment/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from nia_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from nia_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from nia_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("nia_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("nia_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("nia_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`nia tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["nia-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "nia-_test_platform"},
    }

    with mock_patch("nia_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'nia-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `nia tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from nia_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `nia tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from nia_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `nia tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from nia_cli.tools_config import _get_effective_configurable_toolsets
