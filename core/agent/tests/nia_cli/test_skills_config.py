"""Tests for nia_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("nia_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from nia_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("nia_cli.config.load_config")
    ...
    @patch("nia_cli.config.load_config")
    @patch.dict("os.environ", {"NIA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NIA_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """NIA_SESSION_PLATFORM should be used when NIA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_nia_platform_takes_precedence(self, tmp_path, monkeypatch):
        """NIA_PLATFORM should win over NIA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("NIA_HOME", str(tmp_path))
        monkeypatch.setenv("NIA_PLATFORM", "telegram")
        monkeypatch.setenv("NIA_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIA_HOME", str(tmp_path))
        monkeypatch.setenv("NIA_PLATFORM", "telegram")
        monkeypatch.setenv("NIA_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from nia_cli.skills_config import _get_categories
        skills = [
