"""Comprehensive tests for nia_cli.profiles module.
...
from nia_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_nia_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NIA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.nia/profiles)
    * NIA_HOME  -> tmp_path/.nia  (so get_nia_home() agrees)
    * Creates the bare-minimum ~/.nia directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".nia"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("NIA_HOME", str(default_home))
    return tmp_path
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.nia must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".nia"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("nia_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".nia" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_nia_home_returns_default(self, profile_env):
        # NIA_HOME points to tmp_path/.nia which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".nia" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom NIA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("NIA_HOME", str(custom))
        # With Docker-aware roots, a custom NIA_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("nia")
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "nia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nia.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nia.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "nia.ssi_health": {"workspace": "nia", "enabled": True}
            }
        }))
        ...
        assert "nia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nia.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nia.heimdall"]["workspace"] == "nia"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "nia.ssi_health": {"aiPeer": "ssi_health"},
                "nia.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["nia.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nia.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("nia-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".nia_history",
        ...
        excluded_prefixes = [
            "default/nia-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.nia_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_nia_profiles_function(self):
        script = generate_bash_completion()
        assert "_nia_profiles" in script

    def test_zsh_completion_has_nia_function(self):
        script = generate_zsh_completion()
        assert "_nia" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultNIAHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_nia_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".nia" / "profiles"

    def test_default_nia_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_nia_home()
        assert home == tmp_path / ".nia"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (NIA_HOME outside ~/.nia), profiles go under NIA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_nia_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_nia_home() returns NIA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (NIA_HOME under ~/.nia), profiles root is still ~/.nia/profiles."""
        native = tmp_path / ".nia"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under NIA_HOME."""
        from nia_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under NIA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIA_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".nia" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from nia_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".nia"
