...
subparser, placing the flag BEFORE the subcommand (e.g. 'nia --yolo chat')
...
def _build_parser():
    """Build the nia argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets NIA_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("NIA_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `nia gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `nia <argv>` must exit 0 (help) rather than
