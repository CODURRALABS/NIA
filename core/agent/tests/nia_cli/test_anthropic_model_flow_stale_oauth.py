"""Tests for Bug #12905 fix — stale OAuth token detection in nia model flow.

Bug 3: `nia model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.nia/.env but no valid
...
from nia_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from nia_cli.main import _model_flow_anthropic
        cfg = {}
