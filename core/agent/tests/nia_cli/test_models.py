"""Tests for the nia_cli models module."""
...
import nia_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("nia_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with patch("nia_cli.models.urllib.request.urlopen", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        nia-agent is tool-calling-first — surfacing a non-tool model leads to
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from nia_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from nia_cli.models import _find_openrouter_slug
        with patch("nia_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestCheckNousFreeTierCache:
    ...
    @patch("nia_cli.models.fetch_nous_account_tier")
    @patch("nia_cli.models.is_nous_free_tier", return_value=True)
    def test_result_is_cached(self, mock_is_free, mock_fetch):
        ...
        with patch("nia_cli.auth.get_provider_auth_state", return_value={"access_token": "tok"}), \
             patch("nia_cli.auth.resolve_nous_runtime_credentials"):
    ...
    @patch("nia_cli.models.fetch_nous_account_tier")
    @patch("nia_cli.models.is_nous_free_tier", return_value=False)
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from nia_cli.models import fetch_nous_recommended_models
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        ...
        with patch(
            "nia_cli.models.fetch_nous_recommended_models",
    ...
    def test_get_aux_model_returns_none_on_empty_payload(self):
        from nia_cli.models import get_nous_recommended_aux_model
        with patch("nia_cli.models.fetch_nous_recommended_models", return_value={}):
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("nia_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        ...
        with (
            patch("nia_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("nia_cli.models.check_nous_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("nia_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("nia_cli.models.check_nous_free_tier", return_value=False),
        ):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("nia_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("nia_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "paid-model"
