"""Unit tests for nia_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf nia-ok"])
        try:
            output = _read_until(bridge, b"nia-ok")
            assert b"nia-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$NIA_PTY_TEST\""],
            env={**os.environ, "NIA_PTY_TEST": "pty-env-works"},
        )
        try:
