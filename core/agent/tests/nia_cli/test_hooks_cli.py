"""Tests for the ``nia hooks`` CLI subcommand."""
...
from nia_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("NIA_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("NIA_ACCEPT_HOOKS", raising=False)
...
class TestHooksList:
    def test_empty_config(self, tmp_path):
        with patch("nia_cli.config.load_config", return_value={}):
    ...
    def test_shows_configured_and_consent_status(self, tmp_path):
        ...
        with patch("nia_cli.config.load_config", return_value=cfg):
            out = _run(SimpleNamespace(hooks_action="list"))
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `nia hooks doctor` used to run every
