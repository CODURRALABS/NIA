...
from nia_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_skips_migration_prompt(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "nia_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch("nia_cli.config.get_missing_config_fields", return_value=[]), patch(
            "nia_cli.config.check_config_version", return_value=(1, 2)
        ), patch("nia_cli.main.sys") as mock_sys:
