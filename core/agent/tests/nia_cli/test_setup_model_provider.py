...
from nia_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from nia_cli.config import load_config, save_config, save_env_value
from nia_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from nia_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import nia_cli.setup as _setup_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("nia_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", fake_load_pool)
    monkeypatch.setattr("nia_cli.auth_commands.auth_add_command", fake_auth_add_command)
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("nia_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("nia_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("nia_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nia_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("nia_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("nia_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'nia setup' to configure" not in output
