...
- Bare `nia setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `nia setup --quick` runs the narrower "fill in missing items" flow.
- `nia setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("nia_cli.setup.ensure_nia_home", {}),
        ("nia_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("nia_cli.config.is_managed", {"return_value": False}),
        ("nia_cli.setup.load_config", {"return_value": {}}),
        ("nia_cli.setup.save_config", {}),
        ("nia_cli.setup.get_env_value", {"return_value": None}),
        ("nia_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("nia_cli.setup._print_setup_summary", {}),
        ("nia_cli.setup._offer_launch_chat", {}),
        ("nia_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("nia_cli.setup.load_config", {"return_value": {}}),
        ("nia_cli.setup.save_config", {}),
        ("nia_cli.auth.get_active_provider", {"return_value": None}),
        ("nia_cli.setup.get_env_value", {"return_value": None}),
        ("nia_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `nia setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="nia_cli.setup.prompt_choice",
                quick="nia_cli.setup._run_quick_setup",
                model="nia_cli.setup.setup_model_provider",
                terminal="nia_cli.setup.setup_terminal_backend",
                agent="nia_cli.setup.setup_agent_settings",
                gateway="nia_cli.setup.setup_gateway",
                tools="nia_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`nia setup --reconfigure` behaves the same as bare `nia setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("nia_cli.setup.prompt_choice", {"return_value": 0}),
                first="nia_cli.setup._run_first_time_quick_setup",
            )
            from nia_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from nia_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "nia_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["nia", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["nia", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["nia", "setup"])
        try:
            main()
        except SystemExit:
            pass
