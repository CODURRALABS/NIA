...
import nia_cli.doctor as doctor
import nia_cli.gateway as gateway_cli
from nia_cli import doctor as doctor_mod
from nia_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `nia doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "NIA_HOME", nia_home)
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("NIA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_nia_home(self, tmp_path, provider=""):
        """Create a minimal NIA_HOME with config.yaml."""
        home = tmp_path / ".nia"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_nia_home(tmp_path, provider)
...
def test_run_doctor_accepts_nia_provider_ids_that_catalog_aliases(
