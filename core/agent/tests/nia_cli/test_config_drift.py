...
def test_delegation_default_toolsets_removed_from_cli_config():
    ...
    the user's ~/.nia/config.yaml over the defaults (cli.py:359-366).
    A contributor who still has the legacy key set in their own config
    would cause a false failure, and NIA_HOME patching via conftest
    doesn't help because cli._nia_home is frozen at module import time
