...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "nia"
        assert config.workspace_id == "nia"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.nia/honcho.json
        monkeypatch.setenv("NIA_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"nia": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="nia-agent"
        ):
            result = config.resolve_session_name("/home/user/nia-agent/subdir")
        assert result == "nia-agent"
...
class TestResolveConfigPath:
    def test_prefers_nia_home_when_exists(self, tmp_path):
        nia_home = tmp_path / "nia"
        nia_home.mkdir()
        local_cfg = nia_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        nia_home = tmp_path / "nia"
        nia_home.mkdir()
        # No honcho.json in NIA_HOME — also isolate ~/.nia so
    ...
    def test_falls_back_to_global_without_nia_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("NIA_HOME", None)
            result = resolve_config_path()
        assert result == fake_home / ".honcho" / "config.json"
...
class TestResolveActiveHost:
    def test_default_returns_nia(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("NIA_HONCHO_HOST", None)
            os.environ.pop("NIA_HOME", None)
            assert resolve_active_host() == "nia"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"NIA_HONCHO_HOST": "nia.coder"}):
            assert resolve_active_host() == "nia.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_HONCHO_HOST", None)
            with patch("nia_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "nia.coder"

    def test_default_profile_returns_nia(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_HONCHO_HOST", None)
            with patch("nia_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "nia"

    def test_custom_profile_returns_nia(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_HONCHO_HOST", None)
            with patch("nia_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "nia"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIA_HONCHO_HOST", None)
            # Temporarily remove nia_cli.profiles to simulate import failure
            saved = sys.modules.get("nia_cli.profiles")
            sys.modules["nia_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "nia"
            finally:
                if saved is not None:
                    sys.modules["nia_cli.profiles"] = saved
                else:
                    sys.modules.pop("nia_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="nia.coder")
        assert config.host == "nia.coder"
        assert config.workspace_id == "nia"  # shared workspace
        assert config.ai_peer == "nia.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="nia")
        assert config.host == "nia"
        assert config.workspace_id == "nia"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="nia.coder", config_path=config_file,
        )
        assert config.host == "nia.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "nia.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="nia.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "nia.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"nia": {"enabled": True, "aiPeer": "nia"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"nia": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_nia_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="nia",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("nia_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("nia_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_nia_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="nia",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("nia_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
