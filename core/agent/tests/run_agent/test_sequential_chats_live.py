...
AlexKucera reported on Discord (2026-04-16) that after ``nia update`` pulled
...
Opt-in — not part of default CI:
    NIA_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.nia/.env).
"""
...
LIVE = os.environ.get("NIA_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set NIA_LIVE_TESTS=1"),
