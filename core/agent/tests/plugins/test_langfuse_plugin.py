...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing NIA_ prefixed keys.
        assert "NIA_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "NIA_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        from nia_cli import plugins as plugins_mod

        # Isolated NIA_HOME so we don't read the developer's config.yaml.
        home = tmp_path / ".nia"
        home.mkdir()
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "NIA_LANGFUSE_PUBLIC_KEY", "NIA_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("NIA_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_nia_config(self, monkeypatch):
        ...
        # Drop any cached import of nia_cli.config.
        sys.modules.pop("nia_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "nia_cli.config" not in sys.modules, (
            "langfuse plugin imported nia_cli.config — regression toward "
