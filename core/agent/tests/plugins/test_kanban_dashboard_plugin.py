...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "nia_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "nia_cli.web_server", stub)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `nia gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "nia_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
