...
def test_registry_defaults_to_nia_home(tmp_path, monkeypatch):
    from plugins.google_meet.node.registry import NodeRegistry

    # _isolate_home already set NIA_HOME to tmp_path/.nia; the
    # registry default path must live inside that tree.
    r = NodeRegistry()
    r.add("x", "ws://x", "t")
    expected = Path(tmp_path) / ".nia" / "workspace" / "meetings" / "nodes.json"
...
def test_cli_status_pings_via_node_client(capsys, monkeypatch):
    ...
    class _FakeClient:
        ...
        def ping(self):
            return {"type": "pong", "display_name": "nia-meet-node"}
