...
def server():
    with patch.dict(
        "sys.modules",
        {
            "nia_constants": MagicMock(
                get_nia_home=MagicMock(return_value="/tmp/nia_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nia-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nia-release' patched"
    }
