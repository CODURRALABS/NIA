"""Tests for MiniMax OAuth provider (nia_cli/auth.py).
...
def test_poll_token_timeout_raises():
    ...
    import nia_cli.auth as auth_module
...
def test_refresh_updates_access_token():
    ...
    with patch("httpx.Client") as mock_client_class:
        ...
        with patch("nia_cli.auth._minimax_save_auth_state"):
            result = _refresh_minimax_oauth_state(state)
...
def test_resolve_credentials_requires_login():
    ...
    with patch("nia_cli.auth.get_provider_auth_state", return_value=None):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_get_minimax_oauth_auth_status_logged_in():
    ...
    with patch("nia_cli.auth.get_provider_auth_state", return_value=state):
        status = get_minimax_oauth_auth_status()
