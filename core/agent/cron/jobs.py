...
Jobs are stored in ~/.nia/cron/jobs.json
Output is saved to ~/.nia/cron/output/{job_id}/{timestamp}.md
"""
...
NIA_DIR = get_nia_home().resolve()
CRON_DIR = NIA_DIR / "cron"
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _nia_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in NIA configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured NIA timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _nia_now().tzinfo
    if dt.tzinfo is None:
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall nia-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _nia_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _nia_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _nia_now().strftime("%Y-%m-%d_%H-%M-%S")
