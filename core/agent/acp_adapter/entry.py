"""CLI entry point for the nia-agent ACP adapter.

Loads environment variables from ``~/.nia/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from NIA_HOME (default ``~/.nia``)."""
    from nia_cli.env_loader import load_nia_dotenv

    nia_home = get_nia_home()
    loaded = load_nia_dotenv(nia_home=nia_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nia_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting nia-agent ACP adapter")
    ...
    import acp
    from .server import NIAACPAgent
