"""ACP tool-call helpers for mapping nia tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map nia tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a nia tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some NIA tools append a human hint after a JSON payload, e.g.
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # NIA read_file output is line-numbered with `|`. If we send it as raw
...
def build_tool_start(
    ...
    """Create a ToolCallStart event for the given nia tool invocation."""
