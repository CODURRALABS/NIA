"""ACP agent server — exposes NIA via the Agent Client Protocol."""
...
try:
    from nia_cli import __version__ as NIA_VERSION
except Exception:
    NIA_VERSION = "0.0.0"
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a NIA/OpenAI-compatible user content payload."""
...
class NIAACPAgent(acp.Agent):
    """ACP Agent implementation wrapping NIA AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from nia_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from nia_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  NIA estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["nia-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate NIA using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="nia-agent", version=NIA_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # NIA' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes NIA remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run NIA on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive NIA' local kawaii waiting/status
        ...
        # not the executor's. Also set NIA_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue NIA_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind NIA_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("NIA_INTERACTIVE")
            ...
            try:
                ...
                # Restore NIA_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("NIA_INTERACTIVE", None)
                else:
                    os.environ["NIA_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (NIA_SESSION_KEY in
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when NIA has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
