"""ACP permission bridging — maps ACP approval requests to nia approval callbacks."""
...
# Maps ACP PermissionOptionKind -> nia approval result strings
_KIND_TO_NIA = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a nia-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_NIA.get(opt.kind, "deny")
