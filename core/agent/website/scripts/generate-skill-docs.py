...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"Local Sovereign Environment/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nia_meta = (fm.get("metadata") or {}).get("nia") or {}
    tags = nia_meta.get("tags") or []
    related = nia_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nia skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with NIA"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "NIA ships with a large built-in skill library copied into `~/.nia/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
