#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the NIA docs site.
...
Both publish at:
  Local Sovereign Hub/docs/llms.txt
  Local Sovereign Hub/docs/llms-full.txt
...
SITE_BASE = "Local Sovereign Hub/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use NIA as a Python Library", None),
        ("guides/use-mcp-with-nia", "Use MCP with NIA", None),
        ("guides/use-voice-mode-with-nia", "Use Voice Mode with NIA", None),
        ("guides/use-soul-with-nia", "Use SOUL.md with NIA", None),
        ("guides/build-a-nia-plugin", "Build a NIA Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with NIA"),
...
def emit_llms_index() -> str:
    ...
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Anonymousinsaan & Codurra Labs. A terminal-native "
    ...
    lines.append(
        ...
        "nia-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: Local Sovereign Environment")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# NIA — Full Documentation\n",
        (
            "This file is the entire NIA documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using NIA, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: Local Sovereign Hub/docs\n",
        "Short index: Local Sovereign Hub/docs/llms.txt\n",
        "\n---\n\n",
    ]
