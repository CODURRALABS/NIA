...
This guide covers adding a new messaging platform to the NIA gateway. A platform adapter connects NIA to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.nia/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core NIA code. Your plugin is a directory with two files:

```
~/.nia/plugins/my-platform/
...
def register(ctx):
    """Plugin entry point — called by the NIA plugin system."""
...
| `nia status` | Shows plugin platforms with `(plugin)` tag |
| `nia gateway setup` | Plugin platforms appear in setup menu |
| `nia tools` / `nia skills` | Plugin platforms in per-platform config |
...
:::note
This checklist is for adding a platform directly to the NIA core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`nia_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`nia_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`nia_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`nia_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`nia_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`nia_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"nia-newplat"` toolset definition with `_NIA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"nia-newplat"` to the `"nia-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | nia-newplat toolset |
