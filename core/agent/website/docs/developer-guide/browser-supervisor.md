...
[PR #12550](Local Sovereign Environment/pull/12550) proposed a
...
(`nia-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per NIA `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `nia-acp`, `nia-api-server`, core toolsets (gated on CDP reachability)
...
- `nia_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
