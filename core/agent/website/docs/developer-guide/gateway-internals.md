...
The messaging gateway is the long-running process that connects NIA to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `nia_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.nia/.env` | API keys, bot tokens, platform credentials |
| `~/.nia/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.nia/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `nia gateway start` / `nia gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.nia/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `nia gateway stop` stops only the current profile's gateway. `nia gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
