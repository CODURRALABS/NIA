...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to NIA — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in NIA tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying NIA core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a NIA Plugin](/docs/guides/build-a-nia-plugin)
...
2. **`toolsets.py`** — add tool name to `_NIA_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `nia_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `nia_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `nia chat -q "Use the weather tool for London"`
