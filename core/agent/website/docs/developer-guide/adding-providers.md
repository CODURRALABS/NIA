...
description: "How to add a new inference provider to NIA — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

NIA can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `nia model` menu entries
...
1. `nia_cli/auth.py` decides how credentials are found.
2. `nia_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `nia_cli/models.py` and `nia_cli/main.py` make the provider show up in the CLI. (`nia_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`nia_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `nia setup`.
:::
...
- `PROVIDER_REGISTRY` in `nia_cli/auth.py`
- `_PROVIDER_LABELS` in `nia_cli/models.py`
- `_PROVIDER_ALIASES` in both `nia_cli/auth.py` and `nia_cli/models.py`
- CLI `--provider` choices in `nia_cli/main.py`
...
## Step 2: Add auth metadata in `nia_cli/auth.py`
...
- What env vars should NIA check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `nia_cli/models.py`
...
## Step 4: Resolve runtime data in `nia_cli/runtime_provider.py`
...
Be careful with API-key precedence. NIA already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `nia_cli/main.py`

A provider is not discoverable until it shows up in the interactive `nia model` flow.

Update these in `nia_cli/main.py`:
...
:::tip
`nia_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `nia model` and `nia setup` automatically.
:::
...
When you add a native provider, double-check that NIA is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m nia_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `nia_cli/auth.py`
- [ ] aliases added in `nia_cli/auth.py` and `nia_cli/models.py`
- [ ] model catalog added in `nia_cli/models.py`
- [ ] runtime branch added in `nia_cli/runtime_provider.py`
- [ ] CLI wiring added in `nia_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `nia model` but not `nia setup`
