...
description: "How NIA builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

NIA deliberately separates:
...
1. agent identity — `SOUL.md` from `NIA_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.nia/SOUL.md)
You are NIA, an AI assistant created by Anonymousinsaan & Codurra Labs.
...
`SOUL.md` lives at `~/.nia/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_nia_md(cwd_path)       # 1. .nia.md / NIA.md (walks to git root)
    ...
    # SOUL.md from NIA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.nia.md`, `NIA.md` | CWD up to git root | NIA-native project config |
...
- **YAML frontmatter stripped** — `.nia.md` frontmatter is removed (reserved for future config overrides)
...
1. `.nia.md` / `NIA.md` (walks to git root)
