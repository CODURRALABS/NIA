...
description: "How to create skills for NIA — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to NIA. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, NIA prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. NIA never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
- `prompt` (optional) — prompt text shown during `nia config migrate`; falls back to `description`
...
2. **Discovery:** `nia config migrate` scans all enabled skills, finds unconfigured settings, and prompts the user. Settings also appear in `nia config show` under "Skill Settings."

3. **Runtime injection:** When a skill loads, its config values are resolved and appended to the skill message:
   ```
   [Skill config (from ~/.nia/config.yaml):
...
4. **Manual setup:** Users can also set values directly:
   ```bash
   nia config set skills.config.myplugin.path ~/my-data
   ```

:::tip When to use which
Use `required_environment_variables` for API keys, tokens, and other **secrets** (stored in `~/.nia/.env`, never shown to the model). Use `config` for **paths, preferences, and non-sensitive settings** (stored in `config.yaml`, visible in config show).
:::
...
Each entry supports:
- `path` (required) — file path relative to `~/.nia/`
- `description` (optional) — explains what the file is and how it's created

When loaded, NIA checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
:::tip When to use which
Use `required_environment_variables` for simple API keys and tokens (strings stored in `~/.nia/.env`). Use `required_credential_files` for OAuth token files, client secrets, service account JSON, certificates, or any credential that's a file on disk.
:::
...
Prefer stdlib Python, curl, and existing NIA tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
| Token | Replaced with |
|---|---|
| `${NIA_SKILL_DIR}` | Absolute path to the skill's directory |
| `${NIA_SESSION_ID}` | The active session id (left in place if there is no session) |
...
To analyse the input, run:
    ...
    node ${NIA_SKILL_DIR}/scripts/analyse.js <input>
```
...
Git branch: !`git -C ${NIA_SKILL_DIR} rev-parse --abbrev-ref HEAD`
```
...
```bash
nia chat --toolsets skills -q "Use the X skill to do Y"
```
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo, is discoverable via `nia skills browse` (labeled "official"), and installs with builtin trust.

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a registry and share it via `nia skills install`.
...
NIA can now consume third-party skills from multiple external discovery models:
