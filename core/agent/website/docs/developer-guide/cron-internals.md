...
description: "How NIA stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nia_cli/cron.py` | CLI `nia cron` subcommands |
...
Jobs are stored in `~/.nia/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `nia cron` commands are run or during active CLI sessions.
...
```python
# ~/.nia/scripts/check_competitors.py
...
2. **Environment variable** — `NIA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.nia/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nia cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `nia cron` CLI provides direct job management:

```bash
nia cron list                    # Show all jobs
nia cron create                  # Interactive job creation (alias: add)
nia cron edit <job_id>           # Edit job configuration
nia cron pause <job_id>          # Pause a running job
nia cron resume <job_id>         # Resume a paused job
nia cron run <job_id>            # Trigger immediate execution
nia cron remove <job_id>         # Delete a job
```
