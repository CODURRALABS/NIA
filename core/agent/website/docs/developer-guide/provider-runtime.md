...
description: "How NIA resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

NIA has a shared provider runtime resolver used across:
...
- `nia_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `nia_cli/auth.py` — provider registry, `resolve_provider()`
- `nia_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
That ordering matters because NIA treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `nia model`.
...
This resolver is the main reason NIA can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.nia/.env` and run with `--provider ai-gateway`. NIA fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

NIA contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
NIA also distinguishes between:
...
When provider resolution selects `anthropic`, NIA uses:
...
- NIA preflights Anthropic credential refresh before native Messages API calls
- NIA still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, NIA resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `nia model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

NIA supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
