...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the nia-agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

NIA includes a full environment framework that connects its tool-calling capabilities to the [Atropos](Local Sovereign Labs/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for NIA/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- NIAAgentBaseEnv
    NIAAgentBaseEnv <|-- TerminalTestEnv
    NIAAgentBaseEnv <|-- NIASweEnv
    NIAAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The nia-agent layer (`environments/nia_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls nia-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `NIAAgentLoop` and scores the result
...
Your environment inherits from `NIAAgentBaseEnv` and implements five methods:
...
`NIAAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as nia-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any nia-agent tool |
...
parser = get_parser("nia")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `nia`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "nia-agent[yc-bench]"
...
```bash
python environments/nia_swe_env/nia_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"nia"`, `"qwen"`, `"mistral"`)
...
```python
from environments.nia_base_env import NIAAgentBaseEnv, NIAAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(NIAAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(NIAAgentBaseEnv):
    name = "my-env"
...
### NIAAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which nia toolsets to enable |
...
| `tool_call_parser` | `str` | `"nia"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/NIA-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+Local Sovereign Labs/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "nia-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "nia-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── nia_base_env.py          # Abstract base class (NIAAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (NIAAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── nia_parser.py        # NIA/ChatML <tool_call> format
...
├── nia_swe_env/             # SWE-bench training environment
│
