...
sidebar_position: 1
title: "Architecture"
description: "NIA internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of NIA internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nia_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NIACLI — interactive terminal UI (~11,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nia_state.py           # SQLite session/state database with FTS5
├── nia_constants.py       # NIA_HOME, profile-aware paths
...
│
├── nia_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `nia` subcommands (~10,400 lines)
...
│   ├── skills_config.py      # nia skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nia tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nia gateway start/stop
│
...
```text
User input → NIACLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .nia.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.nia/plugins/` (user), `.nia/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nia plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-nia-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes NIA as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nia -p <name>`) gets its own NIA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
