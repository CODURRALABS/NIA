...
The ACP adapter wraps NIA' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nia acp / nia-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.nia/.env
  -> configure stderr logging
  -> construct NIAACPAgent
...
- `allow_once` -> NIA `once`
- `allow_always` -> NIA `always`
- reject options -> NIA `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps NIA tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["nia-acp"])
...
Instead it reuses NIA' runtime resolver:

- `acp_adapter/auth.py`
- `nia_cli/runtime_provider.py`

So ACP advertises and uses the currently configured NIA provider/credentials.
...
- ACP sessions are persisted to the shared `~/.nia/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nia-acp` toolset definition
- `nia_cli/main.py` — `nia acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nia-acp` script
