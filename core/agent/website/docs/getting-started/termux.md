...
description: "Run NIA directly on an Android phone with Termux"
---

# NIA on Android with Termux

This is the tested path for running NIA directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop NIA from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

NIA now ships a Termux-aware installer path:
...
- links `nia` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `nia` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nia" "$PREFIX/bin/nia"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nia` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nia/.env`.
...
### `nia doctor` says ripgrep or Node is missing
