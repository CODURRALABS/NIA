...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with NIA — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working NIA setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Who this is for

- Brand new and want the shortest path to a working setup
- Switching providers and don't want to lose time to config mistakes
- Setting up NIA for a team, bot, or always-on workflow
...
|---|---|---|
| I just want NIA working on my machine | `nia setup` | Run a real chat and verify it responds |
| I already know my provider | `nia model` | Save the config, then start chatting |
| I want a bot or always-on setup | `nia gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `nia model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `nia model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if NIA cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `nia model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `nia model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `nia model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `nia model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `nia model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `nia model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `nia model` (requires `copilot` CLI + `copilot login`) |
...
NIA requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `nia model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

NIA separates secrets from normal config:

- **Secrets and tokens** → `~/.nia/.env`
- **Non-secret settings** → `~/.nia/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
nia config set model anthropic/claude-opus-4.6
nia config set terminal.backend docker
nia config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
nia            # classic CLI
nia --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
NIA ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `nia` vs `nia --tui`.
:::
...
```bash
nia --continue    # Resume the most recent session
nia -c            # Short form
```
...
```bash
nia gateway setup    # Interactive platform configuration
```
...
- `nia tools` — tune tool access per platform
- `nia skills` — browse and install reusable workflows
...
```bash
nia config set terminal.backend docker    # Docker isolation
nia config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
pip install "nia-agent[voice]"
...
|---|---|---|
| NIA opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `nia model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `nia gateway setup` and check `nia gateway status` |
| `nia --continue` can't find old session | Switched profiles or session never saved | Check `nia sessions list` and confirm you're in the right profile |
...
| `nia doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `nia tools` | Configure which tools are enabled per platform |
| `nia setup` | Full setup wizard (configures everything at once) |
| `nia doctor` | Diagnose issues |
| `nia update` | Update to latest version |
| `nia gateway` | Start the messaging gateway |
| `nia --continue` | Resume last session |
