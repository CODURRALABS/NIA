...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to NIA — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`nia claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into NIA. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
nia claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.nia/` (by default a single restore-point archive is written before apply, under `~/.nia/backups/pre-migration-*.zip`; restorable with `nia import`). |
...
| What | OpenClaw source | NIA destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.nia/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.nia/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.nia/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.nia/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | NIA destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.nia/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.nia/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.nia/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.nia/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing NIA skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | NIA destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.nia/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | NIA config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | NIA config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | NIA field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.nia/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | NIA `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | NIA path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `NIA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
| Working directory | `agents.defaults.workspace` | `.env` → `MESSAGING_CWD` | |

### Archived (no direct NIA equivalent)

These are saved to `~/.nia/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in NIA |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | NIA has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `nia cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/docs/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `nia webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `nia honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `nia skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use NIA profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to NIA manually via `nia config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.nia/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `nia status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart nia-gateway`

6. **Check session policies** — verify `nia config get session_reset` matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `nia whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `nia claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `nia config set`.

### Skills not appearing after migration

Imported skills land in `~/.nia/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `nia config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
