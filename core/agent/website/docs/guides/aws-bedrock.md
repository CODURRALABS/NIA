...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use NIA with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

NIA supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
- **boto3** — install with `pip install nia-agent[bedrock]`
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — NIA detects the instance role automatically.
:::
...
After running `nia model`, your `~/.nia/config.yaml` will contain:
...
NIA auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `nia model` picker shows these automatically, with recommended models at the top:
...
Bedrock works with all NIA gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
NIA checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. NIA automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).
