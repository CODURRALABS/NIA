---
sidebar_position: 7
title: "Use SOUL.md with NIA"
description: "How to use SOUL.md to shape NIA's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with NIA

`SOUL.md` is the **primary identity** for your NIA instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want NIA to feel like the same assistant every time you talk to it — or if you want to replace the NIA persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm NIA should be
- what NIA should avoid stylistically
- how NIA should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who NIA is and how NIA speaks
...
NIA now uses only the global SOUL file for the current instance:

```text
~/.nia/SOUL.md
```

If you run NIA with a custom home directory, it becomes:
...
NIA automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, NIA does not overwrite it
- if the file exists but is empty, NIA adds nothing from it to the prompt

## How NIA uses it

When NIA starts a session, it reads `SOUL.md` from `NIA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how NIA feels.
...
NIA already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How NIA should sound.

# Avoid
What NIA should not do.

# Defaults
How NIA should behave when ambiguity appears.
```
...
Then restart NIA or start a new session.
...
### I edited SOUL.md but NIA still sounds the same

Check:
- you edited `~/.nia/SOUL.md` or `$NIA_HOME/SOUL.md`
...
### NIA is ignoring parts of my SOUL.md
