...
title: "Build a NIA Plugin"
description: "Step-by-step guide to building a complete NIA plugin with tools, hooks, data files, and skills"
---

# Build a NIA Plugin

This guide walks through building a complete NIA plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.nia/plugins/calculator
cd ~/.nia/plugins/calculator
```
...
This tells NIA: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** NIA may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `nia my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but NIA continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.nia/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.nia/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `nia plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), NIA injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: nia-agent"}
...
- **The system prompt is NIA's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `nia <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for nia my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: nia my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for nia my-plugin."""
...
After registration, users can run `nia my-plugin status`, `nia my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `nia name` in a terminal |
...
```toml
# pyproject.toml
[project.entry-points."nia_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install nia-plugin-calculator
# Plugin auto-discovered on next nia startup
```
...
```python
# Wrong — will break if NIA passes extra context
def handler(args):
    ...
