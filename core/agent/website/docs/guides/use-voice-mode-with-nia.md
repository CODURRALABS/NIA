---
sidebar_position: 8
title: "Use Voice Mode with NIA"
description: "A practical guide to setting up and using NIA voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with NIA
...
- you want NIA sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in NIA.
...
## Step 1: make sure normal NIA works first
...
NIA supports both local and cloud speech stacks.
...
### If you use `nia setup`

If you choose NeuTTS in the setup wizard, NIA checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. NIA transcribes and responds
...
- asking NIA to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full NIA loop.

## Tuning CLI behavior

### Silence threshold

If NIA starts/stops too aggressively, tune:
...
NIA stays a normal chat bot, but can speak replies.
...
- you want NIA to function like a portable research or ops assistant
...
NIA joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- NIA detects speech boundaries
- transcripts are posted in the associated text channel
- NIA responds in text and audio
...
1. get text NIA working
2. install `nia-agent[voice]`
