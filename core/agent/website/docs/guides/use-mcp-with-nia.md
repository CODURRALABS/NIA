---
sidebar_position: 6
title: "Use MCP with NIA"
description: "A practical guide to connecting MCP servers to NIA, filtering their tools, and using them safely in real workflows"
---

# Use MCP with NIA

This guide shows how to actually use MCP with NIA in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native NIA tool
- you want NIA to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect NIA to internal APIs, databases, or company systems without modifying NIA core

Do not use MCP when:
- a built-in NIA tool already solves the job well
...
- NIA remains the agent
- MCP servers contribute tools
- NIA discovers those tools at startup or reload time
...
If you installed NIA with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- NIA banner/status should show MCP integration when configured
- ask NIA what tools it has available
...
There are two categories of MCP-exposed functionality in NIA:
...
So NIA will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want NIA to reason over a bounded workspace.
...
Now NIA can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing NIA core.
...
Because NIA now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
