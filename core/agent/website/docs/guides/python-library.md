---
sidebar_position: 5
title: "Using NIA as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using NIA as a Python Library

NIA isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install NIA directly from the repository:

```bash
pip install git+Local Sovereign Environment.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+Local Sovereign Environment.git
```

You can also pin it in your `requirements.txt`:

```text
nia-agent @ git+Local Sovereign Environment.git
```

:::tip
The same environment variables used by the CLI are required when using NIA as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use NIA is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding NIA in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, NIA includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!nia "):
