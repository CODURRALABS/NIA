...
This tutorial walks you through setting up a Telegram bot powered by NIA that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **NIA installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nia/.env`

:::tip
A $5/month VPS is plenty for running the gateway. NIA itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team NIA Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nia_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by NIA. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u nia-gateway -f
...
nia gateway start
nia gateway stop
tail -f ~/.nia/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nia gateway install` to update the plist.
:::
...
```bash
nia gateway stop && nia gateway start
```
...
nia pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nia/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nia/config.yaml`:
...
Customize how the bot communicates by editing `~/.nia/SOUL.md`:

For a full guide, see [Use SOUL.md with NIA](/docs/guides/use-soul-with-nia).
...
```bash
# From the CLI
nia cron list          # View all scheduled jobs
nia cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nia-gateway` (Linux) or `~/.nia/logs/gateway.log` (macOS) |
| Cron job output | `~/.nia/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nia/cron/jobs.json` |
| Pairing data | `~/.nia/pairing/` |
| Session history | `~/.nia/sessions/` |
