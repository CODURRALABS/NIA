...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in NIA — no API key required"
---

# MiniMax OAuth

NIA supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and NIA automatically refreshes your session.
...
# → NIA opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.nia/auth.json` and are refreshed automatically before each session.
...
```bash
nia auth add minimax-oauth --region cn
```
...
```bash
nia auth add minimax-oauth --no-browser
```

NIA will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

NIA implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. NIA generates a PKCE verifier / challenge pair and a random state value.
...
4. NIA polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.nia/auth.json` under the `minimax-oauth` key.
...
```bash
nia config set model MiniMax-M2.7
nia config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.nia/config.yaml` will contain entries similar to:
...
```bash
nia --provider minimax-oauth    # canonical
nia --provider minimax-portal   # alias
nia --provider minimax-global   # alias
nia --provider minimax_oauth    # alias (underscore form)
```
...
```bash
NIA_INFERENCE_PROVIDER=minimax-oauth nia
```
...
NIA refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, NIA marks the session as requiring re-login.

**Fix:** run `nia auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, NIA raises a timeout error.

**Fix:** re-run `nia auth add minimax-oauth` (or `nia model`). The flow starts fresh.

### State mismatch (possible CSRF)

NIA detected that the `state` value returned by the authorization server does not match what it sent.
...
If `nia` cannot open a browser window, use `--no-browser`:

```bash
nia auth add minimax-oauth --no-browser
```

NIA prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `nia model` and select MiniMax (OAuth), or run `nia auth add minimax-oauth`.
...
- [nia doctor](../reference/cli-commands.md)
