...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. NIA works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
NIA automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
NIA_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `NIA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `NIA_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `NIA_API_TIMEOUT` |
