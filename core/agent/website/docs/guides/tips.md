...
description: "Practical advice to get the most out of NIA — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---

# Tips & Best Practices

A quick-wins collection of practical tips that make you immediately more effective with NIA. Each section targets a different aspect — scan the headers and jump to what's relevant.
...
Forgot something from your last session? Run `nia -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `nia -r "my research project"`.
...
Want NIA to have a stable default voice? Edit `~/.nia/SOUL.md` (or `$NIA_HOME/SOUL.md` if you use a custom NIA home). NIA now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with NIA](/docs/guides/use-soul-with-nia).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? NIA reads those too. No need to duplicate your coding conventions — they're loaded automatically from the working directory.

### Discovery

NIA loads the top-level `AGENTS.md` from the current working directory at session start. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results — they are not loaded upfront into the system prompt.
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `nia sessions list` and resume with `nia -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `nia pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
On messaging platforms, sessions auto-reset after idle time (default: 24 hours) or daily at 4 AM. Adjust per-platform in `~/.nia/config.yaml` if you need longer sessions.
:::
...
TERMINAL_DOCKER_IMAGE=nia-sandbox:latest
```
...
NIA checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production — it exists for good reasons.
