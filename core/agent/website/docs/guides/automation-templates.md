...
Copy-paste recipes for common automation patterns. Each template uses NIA's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nia webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nia webhook subscribe`) |
...
```bash
nia cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/nia-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/nia-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
nia webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
nia cron create "0 9 * * 1" \
  "Scan the NousResearch/nia-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/nia-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (nia_cli/commands.py, nia_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (nia_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
nia cron create "0 6 * * *" \
  "Run a dependency security audit on the nia-agent project.

1. cd ~/.nia/nia-agent && source .venv/bin/activate
...
```bash
nia webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
nia webhook subscribe alert-triage \
...
```python title="~/.nia/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "NIA-Monitor/1.0"})
...
nia cron create "every 30m" \
  ...
  --script ~/.nia/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
nia cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to NIA development." \
...
```bash
nia webhook subscribe github-issues \
  --events "issues" \
...
```bash
nia webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
nia webhook subscribe stripe-payments \
...
```bash
nia cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the nia-agent codebase.
