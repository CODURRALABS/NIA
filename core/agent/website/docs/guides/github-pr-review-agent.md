...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  NIA  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to NIA instantly when PRs are opened or updated.
:::
...
- **NIA installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  nia gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.nia/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure NIA can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/nia-agent --state open --limit 3
```
...
Still in the chat, ask NIA to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/nia-agent
```
...
A skill gives NIA consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.nia/skills/code-review
```

Create `~/.nia/skills/code-review/SKILL.md`:
...
Verify it loaded — start `nia` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach NIA your team's standards:
...
2. Teach NIA your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
nia gateway status    # Is the gateway running?
nia cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-nia-plugin)** — wrap the review logic into a shareable plugin
