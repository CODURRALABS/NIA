...
NIA is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, NIA checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.nia/config.yaml`:
...
1. **CLI flag**: Start a session with `nia --yolo` or `nia chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `NIA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `NIA_YOLO_MODE` environment variable which is checked before every command execution.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that NIA refuses to run them **regardless** of:
...
Configure the timeout in `~/.nia/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.nia/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.nia/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` nia/gateway | Self-termination prevention |
...
The `NIA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.nia/config.yaml`:
...
:::tip
Use `nia config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, NIA controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.nia/.env`:
...
For more flexible authorization, NIA includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `nia pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.nia/config.yaml`:
...
nia pairing approve telegram ABC12DEF

# Revoke a user's access
nia pairing revoke telegram 123456789

# Clear all pending codes
nia pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.nia/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, NIA applies strict security hardening to every container.
...
Container resources are configurable in `~/.nia/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.nia/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `NIA_HOME`. Skills declare these in frontmatter:
...
When loaded, NIA checks if these files exist in the active profile's `NIA_HOME` and registers them for mounting:
...
Paths are relative to `~/.nia/`. Files are mounted to `/root/.nia/` inside the container.
...
| **terminal** (local) | Blocks explicit NIA infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- NIA infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
NIA integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.nia/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.nia/logs/` for unauthorized access attempts
10. **Keep updated** — run `nia update` regularly for security patches
