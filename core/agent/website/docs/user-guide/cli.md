...
sidebar_position: 1
title: "CLI Interface"
description: "Master the NIA terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

NIA's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
NIA also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `nia --tui` — see the [TUI](tui.md) guide.
:::
...
nia chat --model "anthropic/claude-sonnet-4"

# With a specific provider
nia chat --provider nous        # Use Nous Portal
nia chat --provider openrouter  # Force OpenRouter

# With specific toolsets
nia chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
nia -s nia-agent-dev,github-auth
nia chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
nia --continue             # Resume the most recent CLI session (-c)
nia --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
nia chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nia -w                         # Interactive mode in worktree
nia -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the NIA CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The NIA CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`nia -c` or `nia --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend NIA to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. NIA supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for NIA replies |
...
```bash
nia -s nia-agent-dev,github-auth
nia chat -s github-pr-workflow -s github-auth
```

NIA loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.nia/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.nia/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain NIA..."
  # Add your own!
```
...
The very first time you press Enter while NIA is working, NIA prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend NIA to the background — just like any terminal process. The shell prints a confirmation:

```
NIA has been suspended. Run `fg` to bring NIA back.
```
...
Resume this session with:
  nia --resume 20260225_143052_a1b2c3
...
```bash
nia --continue                          # Resume the most recent CLI session
nia -c                                  # Short form
nia -c "my project"                     # Resume a named session (latest in lineage)
nia --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nia --resume "refactoring auth"         # Resume by title
nia -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nia sessions rename <id> <title>` from the command line. Use `nia sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in NIA's SQLite state database under `~/.nia/state.db`. The database keeps:
...
NIA immediately confirms the task and gives you back the prompt:
