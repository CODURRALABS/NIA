...
NIA includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. Run `nia model` or `nia tools` to enable it.
:::
...
When a cloud provider is configured, NIA auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`nia setup tools → Browser Automation`
...
Or configure via `nia tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.nia/config.yaml`:
...
Then fully restart NIA so the new config is picked up.

:::warning Nested path matters
NIA reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — NIA ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, NIA silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active NIA profile, so different NIA profiles get different browser profiles (profile isolation).

##### What NIA does not do
- It does not force persistence on the Camofox server. NIA only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), NIA cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start NIA and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted NIA after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

NIA derives the stable `userId` from the profile-scoped directory `~/.nia/browser_auth/camofox/` (or the equivalent under `$NIA_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding NIA profile's state directory.

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. NIA automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach NIA browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start NIA from the terminal (`nia` or `nia chat`) and issue `/browser connect` there.
:::
...
If Chrome isn't already running with remote debugging, NIA will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
google-chrome \
  ...
  --user-data-dir=$HOME/.nia/chrome-debug \
  --no-first-run \
...
Then launch the NIA CLI and run `/browser connect`.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, NIA can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `nia config set toolsets '["nia-cli", "browser"]'`.
:::
...
```
Navigate to Local Sovereign Labs
```
...
```
Type "NIA" into the search field @e3
```
...
Screenshots are stored in `~/.nia/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.nia/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, NIA automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
