...
NIA has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — NIA can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`nia fallback` reuses the provider picker from `nia model` — same provider list, same credential prompts, same validation. Press `a` to add a fallback, `↑`/`↓` to reorder, `d` to remove, `q` to save and exit. Changes persist under `model.fallback_providers` in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.nia/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — NIA still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `nia fallback` writes to this key. When both are set, NIA merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `nia auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `nia model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `nia model` (Google OAuth; optional: `NIA_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `nia model` (Qwen Portal OAuth; optional: `NIA_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `nia model` (MiniMax portal OAuth) |
...
NIA uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), NIA tries providers in order until one works:
...
If the resolved provider fails at call time, NIA also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, NIA also supports:
...
| `"codex"` | Force Codex OAuth | `nia model` → Codex |
...
`base_url` takes precedence over `provider`. NIA uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, NIA drops middle conversation turns without generating a summary rather than failing the session.
