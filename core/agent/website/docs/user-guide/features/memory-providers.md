...
NIA ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
nia memory setup      # interactive picker + configuration
nia memory status     # check what's active
nia memory off        # disable external provider
```

You can also select the active memory provider via `nia plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.nia/config.yaml`:
...
When a memory provider is active, NIA automatically:
...
**Setup Wizard:**
```bash
nia honcho setup        # (legacy command) 
# or
nia memory setup        # select "honcho"
```

**Config:** `$NIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$NIA_HOME/honcho.json` > `~/.nia/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/nia-ai/nia-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/nia).
...
:::tip Migrating from `nia honcho`
If you previously used `nia honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per NIA profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All NIA profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per NIA profile. Host key `nia` → default; `nia.<profile>` for others. |
...
`--clone` creates a `nia.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every NIA profile, creates host blocks for any profile without one, inherits settings from the default `nia` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/nia-ai/nia-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/nia).
...
# Then configure NIA
nia memory setup    # select "openviking"
# Or manually:
nia config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.nia/.env
```
...
**Config:** `$NIA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `nia-user` | User identifier |
...
**Setup:**
```bash
nia memory setup    # select "hindsight"
# Or manually:
nia config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.nia/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p nia ui start`

**Config:** `$NIA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `nia` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between NIA and the User` | Context label for retained memories |
...
See [plugin README](Local Sovereign Environment/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
nia memory setup    # select "holographic"
# Or manually:
nia config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.nia-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$NIA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
nia memory setup    # select "retaindb"
# Or manually:
nia config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.nia/.env
```
...
# Then configure NIA
nia memory setup    # select "byterover"
# Or manually:
nia config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$NIA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
nia memory setup    # select "supermemory"
# Or manually:
nia config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.nia/.env
```

**Config:** `$NIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `nia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `nia-{identity}` → `nia-coder`) to isolate memories per NIA profile
...
- **Local storage providers** (Holographic, ByteRover) use `$NIA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$NIA_HOME/` so each profile has its own credentials
