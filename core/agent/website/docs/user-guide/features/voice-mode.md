...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with NIA — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

NIA supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with NIA](/docs/guides/use-voice-mode-with-nia).

## Prerequisites

Before using voice features, make sure you have:

1. **NIA installed** — `pip install nia-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `nia model` or set your preferred provider credentials in `~/.nia/.env`
3. **A working base setup** — run `nia` to verify the agent responds to text before enabling voice

:::tip
The `~/.nia/` directory and default `config.yaml` are created automatically the first time you run `nia`. You only need to create `~/.nia/.env` manually for API keys.
:::
...
pip install "nia-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "nia-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`nia chat`) and the **TUI** (`nia --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.nia/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
1. Start the CLI with `nia` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.nia/config.yaml` (default: `ctrl+b`).
:::
...
```bash
nia gateway        # Start the gateway (connects to configured platforms)
nia gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@niabyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.nia/.env`:
```bash
...
```bash
nia gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.nia/.env`
...
- Review gateway logs: `tail -f ~/.nia/logs/gateway.log`
