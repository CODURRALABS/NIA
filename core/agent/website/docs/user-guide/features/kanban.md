...
description: "Durable SQLite-backed task board for coordinating multiple NIA profiles"
---
...
NIA Kanban is a durable task board, shared across all your NIA profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.nia/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.nia/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`. The dispatcher spawns each worker with these tools already in its schema; the model reads its task and hands work off by calling them directly, *not* by shelling out to `nia kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `nia kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/nia-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.nia/kanban/workspaces/<id>/` (or `~/.nia/kanban/boards/<slug>/workspaces/<id>/` on non-default boards).
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `NIA_KANBAN_BOARD` pinned so they can't see other boards. After ~5 consecutive spawn failures on the same task the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
nia kanban boards list

# Create a new board.
nia kanban boards create atm10-server \
    --name "ATM10 Server" \
...
nia kanban --board atm10-server list
nia kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
nia kanban boards switch atm10-server
nia kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
nia kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `NIA_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.nia/kanban/current` — the slug persisted by `nia kanban
   boards switch`.
4. `default`.
...
`nia dashboard` → Kanban tab shows a board switcher at the top as soon
...
nia kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `nia kanban show t_abcd`.
...
Override the config flag at runtime via `NIA_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `nia gateway
...
until one comes up — `nia kanban create` warns about this at creation
time.

Running `nia kanban daemon` as a separate process is **deprecated**;
...
nia kanban create "nightly ops review" \
    --assignee ops \
...
```bash
nia kanban complete t_abc t_def t_hij --result "batch wrap"
nia kanban archive  t_abc t_def t_hij
nia kanban unblock  t_abc t_def
nia kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `nia kanban`.** When the dispatcher spawns a worker it sets `NIA_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema — seven tools that read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `nia kanban` CLI.
...
kanban_show()                                     # no args — uses NIA_KANBAN_TASK
...
### Why tools instead of shelling to `nia kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `nia kanban complete` *inside* the container, where `nia` isn't installed and `~/.nia/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.nia/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `nia chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `NIA_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
2. `cd $NIA_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
nia skills install devops/kanban-worker
```
...
```bash
nia kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

nia kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
nia skills install devops/kanban-orchestrator
```
...
```bash
nia kanban init      # one-time: create kanban.db if not already present
nia dashboard        # "Kanban" tab appears in the nav, after "Skills"
```
...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas a specifier is expected to flesh out. Tasks created with `nia kanban create --triage` (or via the Triage column's inline create) land here and the dispatcher leaves them alone until a human or specifier promotes them to `todo` / `ready`.
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.nia/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `nia dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.nia/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `nia -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard NIA dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
nia kanban init                                     # create kanban.db + print daemon hint
nia kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
nia kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
nia kanban show <id> [--json]
nia kanban assign <id> <profile>                    # or 'none' to unassign
nia kanban link <parent_id> <child_id>
nia kanban unlink <parent_id> <child_id>
nia kanban claim <id> [--ttl SECONDS]
nia kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
nia kanban complete <id>... [--result "..."]
nia kanban block <id> "<reason>" [--ids <id>...]
nia kanban unblock <id>...
nia kanban archive <id>...

nia kanban tail <id>                                # follow a single task's event stream
nia kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
nia kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
nia kanban runs <id> [--json]                       # attempt history (one row per run)
nia kanban assignees [--json]                       # profiles on disk + per-assignee task counts
nia kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
nia kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `nia gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
nia kanban stats [--json]                           # per-status + per-assignee counts
nia kanban log <id> [--tail BYTES]                  # worker log from ~/.nia/kanban/logs/
nia kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
nia kanban notify-list [<id>] [--json]
nia kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
nia kanban context <id>                             # what a worker sees
nia kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `nia kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `nia chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `nia_cli.kanban.run_slash()` entry point that reuses the `nia kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `nia kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.nia/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`nia kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
For worked examples of each, see `docs/nia-kanban-v1-spec.pdf`.
...
```bash
nia kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$NIA_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
nia kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
nia kanban notify-list
nia kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
nia kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
nia kanban runs t_abcd
...
**Bulk close caveat.** `nia kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`nia kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `nia kanban heartbeat $TASK` to signal liveness during long operations. |
...
`nia kanban tail <id>` shows these for a single task. `nia kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.nia/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/nia-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
