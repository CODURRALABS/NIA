...
Schedule tasks to run automatically with natural language or cron expressions. NIA exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. NIA disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
nia cron create "every 2h" "Check server status"
nia cron create "every 1h" "Summarize new feed items" --skill blogwatcher
nia cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
NIA will use the unified `cronjob` tool internally.
...
nia cron create "every 1d at 09:00" \
...
```bash
nia cron edit <job_id> --schedule "every 4h"
nia cron edit <job_id> --prompt "Use the revised task"
nia cron edit <job_id> --skill blogwatcher --skill maps
nia cron edit <job_id> --add-skill maps
nia cron edit <job_id> --remove-skill blogwatcher
nia cron edit <job_id> --clear-skills
```
...
```bash
nia cron list
nia cron pause <job_id>
nia cron resume <job_id>
nia cron run <job_id>
nia cron remove <job_id>
nia cron status
nia cron tick
```
...
sudo nia gateway install --system   # Linux: boot-time system service for servers
nia gateway             # Or run in foreground
...
1. loads jobs from `~/.nia/cron/jobs.json`
...
A file lock at `~/.nia/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.nia/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.nia/cron/output/`), but no message is sent to the delivery target.
...
Or set the `NIA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, NIA skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `nia tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `nia tools` cron-platform config wins; otherwise NIA falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether NIA should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.nia/cron/jobs.json`. Output from job runs is saved to `~/.nia/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, NIA resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
