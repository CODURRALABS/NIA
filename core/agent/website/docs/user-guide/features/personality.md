...
description: "Customize NIA's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

NIA's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NIA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who NIA is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

NIA now seeds a default `SOUL.md` automatically in:

```text
~/.nia/SOUL.md
```

More precisely, it uses the current instance's `NIA_HOME`, so if you run NIA with a custom home directory, it will use:
...
- NIA creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- NIA loads `SOUL.md` only from `NIA_HOME`
- NIA does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, NIA falls back to a built-in default identity
...
If NIA loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NIA_HOME`, the personality belongs to the NIA instance itself.

That also makes it easier to teach users:
- "Edit `~/.nia/SOUL.md` to change NIA' default personality."
...
- how NIA should handle uncertainty, disagreement, or ambiguity
...
## What NIA injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, NIA falls back to a built-in default identity ("You are NIA, an intelligent AI assistant created by Anonymousinsaan & Codurra Labs..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
NIA ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain NIA, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives NIA its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.nia/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.nia/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-nia)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how NIA speaks
- `display.skin` and `/skin` affect how NIA looks in the terminal
