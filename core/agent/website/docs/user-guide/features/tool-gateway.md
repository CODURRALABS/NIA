...
When you run `nia model` and select Nous Portal as your provider, NIA automatically offers to enable the Tool Gateway:
...
Set the `use_gateway` flag directly in `~/.nia/config.yaml`:
...
The gateway authenticates using your Nous Portal credentials (stored in `~/.nia/auth.json` after `nia model`).
...
```bash
nia tools    # Select the tool → choose a direct provider
```
...
When you select a non-gateway provider in `nia tools`, the `use_gateway` flag is automatically set to `false` to prevent contradictory config.
...
For self-hosted or custom gateway deployments, you can override the gateway endpoints via environment variables in `~/.nia/.env`:
...
Tools that were routed through the gateway will stop working until you [renew your subscription](https://portal.nousresearch.com/manage-subscription) or switch to direct API keys via `nia tools`.
...
Modal (serverless terminal backend) is available as an optional add-on through the Nous subscription. It's not enabled by the Tool Gateway prompt — configure it separately via `nia setup terminal` or in `config.yaml`.
