...
description: "Connect NIA to external tool servers via MCP — and control exactly which MCP tools NIA loads"
---

# MCP (Model Context Protocol)

MCP lets NIA connect to external tool servers so the agent can use tools that live outside NIA itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted NIA to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native NIA tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want NIA to see
...
2. Add an MCP server to `~/.nia/config.yaml`:
...
4. Ask NIA to use the MCP-backed capability.
...
NIA will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints NIA connects to directly.
...
- you do not want NIA spawning a local subprocess for that integration

## Basic configuration reference

NIA reads MCP config from `~/.nia/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, NIA skips the server entirely |
...
## How NIA registers MCP tools

NIA prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — NIA sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, NIA also registers utility tools around MCP resources and prompts:
...
- NIA only registers resource utilities if the MCP session actually supports resource operations
- NIA only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to NIA, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, NIA skips the server completely and does not even attempt a connection.
...
You can also separately disable NIA-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, NIA does not create an empty runtime MCP toolset for that server.
...
NIA discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify NIA when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When NIA receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, NIA does not blindly pass your full shell environment.
...
Then verify your config and restart NIA.
...
Because NIA now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from NIA via the `sampling/createMessage` protocol. This allows an MCP server to ask NIA to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running NIA as an MCP server

In addition to connecting **to** MCP servers, NIA can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use NIA's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through NIA
- You want a single MCP server that bridges to all of NIA's connected messaging platforms at once
- You already have a running NIA gateway with connected platforms
...
Add NIA to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed NIA in a specific location:

```json
{
  "mcpServers": {
    "nia": {
      "command": "/home/user/.nia/nia-agent/venv/bin/nia",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a NIA-specific channel browser:
...
The MCP server includes a live event bridge that polls NIA's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
nia mcp serve              # Normal mode
nia mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from NIA's session store (`~/.nia/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the NIA itself.
