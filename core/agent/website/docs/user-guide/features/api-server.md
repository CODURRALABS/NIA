...
sidebar_position: 14
title: "API Server"
description: "Expose nia-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes nia-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to nia-agent and use it as a backend.
...
# Optional: only if a browser must call NIA directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "nia-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus NIA' custom `nia.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: NIA emits `event: nia.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: NIA emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/docs/user-guide/profiles) name (or `nia-agent` for the default profile). Required by most frontends for model discovery.
...
```json
{
  "object": "nia.api_server.capabilities",
  "platform": "nia-agent",
  "model": "nia-agent",
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running NIA version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, NIA surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while NIA asks the active agent to stop at the next safe interruption point.
...
Create a new scheduled job. Body accepts the same shape as `nia cron` — prompt, schedule, skills, provider override, delivery target.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), nia-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call NIA directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.

:::warning Security
The API server gives full access to nia-agent's toolset, **including terminal commands**. When binding to a non-loopback address like `0.0.0.0`, `API_SERVER_KEY` is **required**. Also keep `API_SERVER_CORS_ORIGINS` narrow to control browser access.
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `nia-agent` for default profile. |
...
To give multiple users their own isolated NIA instance (separate config, memory, skills), use [profiles](/docs/user-guide/profiles):

```bash
# Create a profile per user
nia profile create alice
nia profile create bob

# Configure each profile's API server on a different port
nia -p alice config set API_SERVER_ENABLED true
nia -p alice config set API_SERVER_PORT 8643
nia -p alice config set API_SERVER_KEY alice-secret

nia -p bob config set API_SERVER_ENABLED true
nia -p bob config set API_SERVER_PORT 8644
nia -p bob config set API_SERVER_KEY bob-secret
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated NIA instance. See the [Open WebUI guide](/docs/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
