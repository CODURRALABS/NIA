...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, NIA automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, NIA auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
nia auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
nia auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
nia auth add anthropic --type oauth
...
Run `nia auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `nia auth` | Interactive pool management wizard |
| `nia auth list` | Show all pools and credentials |
| `nia auth list <provider>` | Show a specific provider's pool |
| `nia auth add <provider>` | Add a credential (prompts for type and key) |
| `nia auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `nia auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `nia auth remove <provider> <index>` | Remove credential by 1-based index |
| `nia auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `nia auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `nia model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via nia model:
nia auth list
# Shows:
...
nia auth add Together.ai --api-key sk-together-second-key
```
...
NIA automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| NIA PKCE OAuth | `~/.nia/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `nia auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `nia auth add`) are never auto-pruned.
...
2. **`nia_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`nia_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.nia/auth.json` under the `credential_pool` key:
