...
Type `@` followed by a reference to inject content directly into your message. NIA expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- NIA env: `$NIA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$NIA_HOME/skills/.hub/`
