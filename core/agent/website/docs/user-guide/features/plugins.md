...
title: "Plugins"
description: "Extend NIA with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

NIA has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in NIA
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a NIA Plugin](/docs/guides/build-a-nia-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.nia/plugins/` with a `plugin.yaml` and Python code:
...
Start NIA — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.nia/plugins/hello-world/plugin.yaml`**
...
**`~/.nia/plugins/hello-world/__init__.py`**

```python
"""Minimal NIA plugin — registers a tool and a hook."""
...
Drop both files into `~/.nia/plugins/hello-world/`, restart NIA, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.nia/plugins/` are disabled by default. Enable them only for trusted repositories by setting `NIA_ENABLE_PROJECT_PLUGINS=true` before starting NIA.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `nia <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `nia plugins install` |
| Distribute via pip | `[project.entry-points."nia_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with NIA — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.nia/plugins/` | Personal plugins |
| Project | `.nia/plugins/` | Project-specific plugins (requires `NIA_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `nia_agent.plugins` entry_points | Distributed packages |
| Nix | `services.nia-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

## Plugins are opt-in

**Every plugin — user-installed, bundled, or pip — is disabled by default.** Discovery finds them (so they show up in `nia plugins` and `/plugins`), but nothing loads until you add the plugin's name to `plugins.enabled` in `~/.nia/config.yaml`. This stops anything with hooks or tools from running without your explicit consent.
...
```bash
nia plugins                    # interactive toggle (space to check/uncheck)
nia plugins enable <name>      # add to allow-list
nia plugins disable <name>     # remove from allow-list + add to disabled
```

After `nia plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### Migration for existing users

When you upgrade to a version of NIA that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.nia/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled plugins are NOT grandfathered — even existing users have to opt in explicitly.
...
NIA has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.nia/plugins/` |
...
On NixOS, plugins can be installed declaratively via the module options — no `nia plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
nia plugins                               # unified interactive UI
nia plugins list                          # table: enabled / disabled / not enabled
nia plugins install user/repo             # install from Git, then prompt Enable? [y/N]
nia plugins install user/repo --enable    # install AND enable (no prompt)
nia plugins install user/repo --no-enable # install but leave disabled (no prompt)
nia plugins update my-plugin              # pull latest
nia plugins remove my-plugin              # uninstall
nia plugins enable my-plugin              # add to allow-list
nia plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `nia plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `nia plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-nia-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
