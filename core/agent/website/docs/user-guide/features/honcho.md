...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of NIA's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple NIA instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nia memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$NIA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `nia` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — NIA syncs them back at session init.
...
```bash
nia honcho status          # Connection status, config, and key settings
nia honcho setup           # Interactive setup wizard
nia honcho strategy        # Show or set session strategy
nia honcho peer            # Update peer names for multi-agent setups
nia honcho mode            # Show or set recall mode
nia honcho tokens          # Show or set context token budget
nia honcho identity        # Show Honcho peer identity
nia honcho sync            # Sync host blocks for all profiles
nia honcho enable          # Enable Honcho
nia honcho disable         # Disable Honcho
```

## Migrating from `nia honcho`

If you previously used the standalone `nia honcho setup`:
...
No re-login or re-setup needed. Run `nia memory setup` and select "honcho" — the wizard detects your existing config.
