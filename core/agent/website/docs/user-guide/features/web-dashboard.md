...
The web dashboard is a browser-based UI for managing your NIA installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `nia --tui` via PTY/WebSocket). Alternatively set `NIA_DASHBOARD_TUI=1`. |

```bash
# Custom port
nia dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
nia dashboard --host 0.0.0.0

# Start without opening browser
nia dashboard --no-open
```

## Prerequisites

The default `nia-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'nia-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install nia-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `nia dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.
...
- Node.js (same requirement as `nia --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'nia-agent[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `nia config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.nia/skills/` and grouped by category.
...
This re-reads `~/.nia/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `nia_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `nia update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `nia dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **NIA Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
