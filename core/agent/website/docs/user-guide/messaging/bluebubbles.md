...
Connect NIA to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between NIA and the BlueBubbles server
...
Or set environment variables directly in `~/.nia/.env`:
...
When someone messages your iMessage, NIA automatically sends them a pairing code. Approve it with:
```bash
nia pairing approve bluebubbles <CODE>
```
Use `nia pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.nia/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.nia/.env`):
```bash
...
NIA will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → NIA
NIA → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** NIA sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.nia/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — NIA resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `nia logs gateway` for webhook errors (or `nia logs -f` to follow in real-time)
