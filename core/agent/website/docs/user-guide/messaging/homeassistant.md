...
description: Control your smart home with NIA via Home Assistant integration.
...
NIA integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "NIA"
5. Copy the token
...
NIA registers four tools for smart home control:
...
Configure which events the agent sees in `~/.nia/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "NIA".
