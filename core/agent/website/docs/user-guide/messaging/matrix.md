...
sidebar_position: 9
title: "Matrix"
description: "Set up NIA as a Matrix bot"
---

# Matrix Setup

NIA integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the NIA pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

NIA works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how NIA behaves once it's connected.

## How NIA Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | NIA responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, NIA requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | NIA supports Matrix threads (MSC3440). If you reply in a thread, NIA keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, NIA auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, NIA isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `nia` — the full user ID will be `@nia:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `nia-bot`).

### Option C: Use Your Own Account

You can also run NIA as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

NIA needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give NIA the bot's user ID and password. NIA will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@nia:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

NIA uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.nia/.env` file:
...
# MATRIX_USER_ID=@nia:matrix.example.org
...
Optional behavior settings in `~/.nia/config.yaml`:
...
:::tip
You can run `nia gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

NIA supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with nia extras
pip install 'nia-agent[matrix]'
```
...
When E2EE is enabled, NIA:

- Stores encryption keys in `~/.nia/platforms/matrix/store/` (legacy installs: `~/.nia/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, NIA imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning[Deleting the crypto store]
If you delete `~/.nia/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

NIA detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nia:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nia:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nia:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@nia:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40nia%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart NIA:
   ```bash
   rm -f ~/.nia/platforms/matrix/store/crypto.db*
   # restart nia
   ```
...
If you previously used NIA with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@nia:your-server.org"},
       "password": "***",
       "initial_device_display_name": "NIA"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.nia/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.nia/platforms/matrix/store/crypto.db
   rm -f ~/.nia/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.nia/.env`:
...
a working E2EE setup with a previous version of NIA and are upgrading.
...
services:
  nia-matrix:
    ...
    environment:
      ...
      MATRIX_ENCRYPTION: "true"
      MATRIX_DEVICE_ID: "NIA_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.nia/platforms/matrix/store
```
...
RUN pip install 'nia-agent[matrix]'
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote NIA API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-NIA-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the NIA logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The NIA gateway isn't running, or it failed to connect.

**Fix**: Check that `nia gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.nia/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your NIA deployment, see the [Security Guide](../security.md).
...
- **Media support**: NIA can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
