...
Connect NIA to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
NIA supports two WeCom integration modes:
...
3. NIA decrypts the message, queues it for the agent
...
(Use `nia gateway start` only after `nia gateway install` has registered the systemd/launchd service.)
