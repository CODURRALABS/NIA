...
1. Enable via `nia gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `nia webhook subscribe`
...
Add the `github-pr` route to your `~/.nia/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, NIA processes the event, and posts a review comment on the PR.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `nia webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
nia webhook remove github-issues
```

### Test a subscription

```bash
nia webhook test github-issues
nia webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.nia/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `nia webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `nia gateway run`
