...
sidebar_position: 4
title: "Slack"
description: "Set up NIA as a Slack bot using Socket Mode"
---

# Slack Setup

Connect NIA to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your NIA instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. NIA uses the modern
...
The fastest path is to paste a manifest NIA generates for you. It
...
### Option A: From a NIA-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   nia slack manifest --write
   ```
   This writes `~/.nia/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "NIA") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, NIA can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `nia-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a NIA configuration issue.
:::
...
NIA uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
nia gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
nia gateway              # Foreground
nia gateway install      # Install as a user service
sudo nia gateway install --system   # Linux only: boot-time system service
```
...
Every NIA command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
NIA command with its description.

Under the hood: NIA ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](Local Sovereign Environment/blob/main/nia_cli/commands.py)
...
When NIA adds new commands (e.g. after `nia update`), regenerate
...
3. Paste the new contents of `~/.nia/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/nia <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/nia btw run the tests` — NIA routes it the same way as `/btw
run the tests`. Free-form questions also work: `/nia what's the
...
```bash
nia slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how NIA behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@NIA what time is it?`). In channels, NIA replies in a thread attached to that message. |
| **Threads** | If you @mention NIA inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.nia/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to NIA in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before NIA will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where NIA will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @NIA`).

---

## Multi-Workspace Support

NIA can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, NIA also loads tokens from an **OAuth token file** at:

```
~/.nia/slack_tokens.json
```
...
- When a message arrives, NIA uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @NIA` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @NIA` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. NIA now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @NIA`)
...
- Tokens should be stored in `~/.nia/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your NIA config directory
