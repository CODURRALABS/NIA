...
sidebar_position: 1
title: "Telegram"
description: "Set up NIA as a Telegram bot"
---

# Telegram Setup

NIA integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "NIA") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_nia_bot`)
...
NIA uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, NIA connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
You can also set it manually in `~/.nia/.env`:
...
Voice messages you send on Telegram are automatically transcribed by NIA's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running NIA — no API key required
...
NIA works in Telegram group chats with a few considerations:
...
- Use `telegram.ignored_threads` to keep NIA silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, NIA keeps the previous open-group behavior and responds to normal group messages it can see

### Example group trigger configuration

Add this to `~/.nia/config.yaml`:
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with NIA.
...
1. Open your private chat with the NIA bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, NIA will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting — the bot cannot enable it programmatically.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.nia/config.yaml`:
...
1. On gateway startup, NIA calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.nia/config.yaml`:
...
1. When a message arrives in a mapped group topic, NIA looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | NIA creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
Telegram's MarkdownV2 has no native table syntax — pipe tables render as backslash-escaped noise if passed through raw. NIA normalizes markdown tables automatically:
...
When enabled, NIA attaches Telegram's `LinkPreviewOptions(is_disabled=True)` to every outgoing message and falls back to the legacy `disable_web_page_preview` parameter on older `python-telegram-bot` versions.
...
When you send `/model` with no arguments in a Telegram chat, NIA shows an interactive inline keyboard for switching models:
...
Or add it to `~/.nia/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional NIA configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that NIA uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `nia gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.nia/.env`. |
