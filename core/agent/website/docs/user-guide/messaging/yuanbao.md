...
sidebar_position: 16
title: "Yuanbao"
description: "Connect NIA to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect NIA to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.nia/.env`:
...
All standard NIA commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.nia/logs/gateway.log`
...
4. Consider enabling verbose logging: `NIA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. NIA automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
NIA_LOG_LEVEL=debug nia gateway
```
...
```bash
nia chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
