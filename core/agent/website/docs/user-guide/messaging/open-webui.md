...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to NIA via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With NIA's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["nia-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to NIA's API server just like it would connect to OpenAI. Your agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

Open WebUI talks to NIA server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
```bash
nia config set API_SERVER_ENABLED true
nia config set API_SERVER_KEY your-secret-key
```

`nia config set` auto-routes the flag to `config.yaml` and the secret to `~/.nia/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
nia gateway stop && nia gateway
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **nia-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **nia-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and NIA responds accordingly. Each request includes the full conversation history.
...
2. Edit your nia-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and NIA can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, NIA also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. NIA creates an AIAgent instance with its full toolset
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | NIA's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `nia-agent`
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your NIA models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
NIA may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in NIA.

## Multi-User Setup with Profiles

To run separate NIA instances per user — each with their own config, memory, and skills — use [profiles](/docs/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated NIA.
...
```bash
nia -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
