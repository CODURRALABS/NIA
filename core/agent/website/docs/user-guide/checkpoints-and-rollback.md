...
NIA automatically snapshots your project before **destructive operations** and lets you restore it with a single command. Checkpoints are **enabled by default** — there's zero cost when no file-mutating tools fire.

This safety net is powered by an internal **Checkpoint Manager** that keeps a separate shadow git repository under `~/.nia/checkpoints/` — your real project `.git` is never touched.
...
- NIA detects when tools are about to **modify files** in your working tree.
...
Checkpoints are enabled by default. Configure in `~/.nia/config.yaml`:
...
checkpoints:
  ...
  # Auto-maintenance (opt-in): sweep ~/.nia/checkpoints/ at startup
  ...
  # .last_prune marker inside ~/.nia/checkpoints/.
...
NIA responds with a formatted list showing change statistics:
...
To keep checkpointing safe and fast, NIA applies several guardrails:

- **Git availability** — if `git` is not found on `PATH`, checkpoints are transparently disabled.
- **Directory scope** — NIA skips overly broad directories (root `/`, home `$HOME`).
...
- A `NIA_WORKDIR` file pointing back to the original project root
...
- **Combine with Git worktrees** for maximum safety — keep each NIA session in its own worktree/branch, with checkpoints as an extra layer.
