...
t1 = kanban_create(
    ...
    tenant=os.environ.get("NIA_TENANT"),
)["task_id"]
...
>
> The dispatcher will pick up T1 and T2 now. T3 starts when both finish. You'll get a gateway ping when T4 completes. Use the dashboard or `nia kanban tail <id>` to follow along.
...
**Tenant inheritance.** If `NIA_TENANT` is set in your env, pass `tenant=os.environ.get("NIA_TENANT")` on every `kanban_create` call so child tasks stay in the same namespace.
