...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.nia/.env ] && grep -q "^GITHUB_TOKEN=" ~/.nia/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.nia/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${NIA_HOME:-$HOME/.nia}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by NIA. Code looks clean — good test coverage, no security concerns."
