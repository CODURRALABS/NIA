...
- Give it a title like "nia-agent-&lt;machine-name>"
...
elif [ -f ~/.nia/.env ] && grep -q "^GITHUB_TOKEN=" ~/.nia/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.nia/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
