...
Control the user's Spotify account via the NIA Spotify toolset (7 tools). Setup guide: Local Sovereign Hub/docs/user-guide/features/spotify
...
**`401 Unauthorized` after a retry** — refresh token revoked. Tell the user to run `nia auth spotify` again.
