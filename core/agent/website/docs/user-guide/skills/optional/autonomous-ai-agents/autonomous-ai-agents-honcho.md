...
description: "Configure and use Honcho memory with NIA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with NIA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nia skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for NIA

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every NIA profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/nia#running-honcho-locally-with-nia

### Verify

```bash
nia honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this NIA profile's AI peer
...
Honcho models conversations as interactions between **peers**. NIA creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this NIA instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each NIA run |
| `global` | Single session across all directories |

Manual override: `nia honcho map my-project-name`
...
Each NIA profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nia.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nia.coder` host block in `honcho.json`
...
```bash
nia honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for NIA when Honcho memory is active.
...
Config file: `$NIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nia honcho setup`. Ensure `memory.provider: honcho` is in `~/.nia/config.yaml`.

### Memory not persisting across sessions
Check `nia honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nia profile create <name> --clone`. For existing profiles: `nia honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nia honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nia honcho status` | Show resolved config, connection test, peer info for active profile |
| `nia honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nia honcho disable` | Disable Honcho for the active profile |
| `nia honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nia honcho peers` | Show peer identities across all profiles |
| `nia honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nia honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nia honcho sessions` | List known directory-to-session-name mappings |
| `nia honcho map <name>` | Map current working directory to a Honcho session name |
| `nia honcho identity` | Seed AI peer identity or show both peer representations |
| `nia honcho sync` | Create host blocks for all NIA profiles that don't have one yet |
| `nia honcho migrate` | Step-by-step migration guide from OpenClaw native memory to NIA + Honcho |
| `nia memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nia memory status` | Show active memory provider and config |
| `nia memory off` | Disable external memory provider |
