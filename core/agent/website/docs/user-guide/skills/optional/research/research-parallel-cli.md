...
| | |
|---|---|
| Source | Optional — install with `nia skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a NIA core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with NIA native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer NIA native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer NIA native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended NIA usage patterns
...
- Do not silently choose Parallel when NIA native tools are already sufficient.
