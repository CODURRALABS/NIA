...
| | |
|---|---|
| Source | Optional — install with `nia skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into NIA or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to NIA. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is NIA integration, either:

- configure the server in `~/.nia/config.yaml` using the `native-mcp` skill, or
...
### NIA cannot see the deployed server

The server-building part may be correct while the NIA config is not. Load the `native-mcp` skill and configure the server in `~/.nia/config.yaml`, then restart NIA.
