...
| | |
|---|---|
| Source | Optional — install with `nia skills install official/productivity/canvas` |
...
4. Name the token (e.g., "NIA"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.nia/.env`:
...
```bash
CANVAS="python $NIA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
