...
| | |
|---|---|
| Source | Optional — install with `nia skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NIA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NIA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nia
```
...
```bash
PUBLISH="${NIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nia
```
...
```bash
DRIVE="${NIA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nia`)    |
