...
| | |
|---|---|
| Source | Optional — install with `nia skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by NIA |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during NIA terminal calls (desktop app flow only)
...
### Service Account (recommended for NIA)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.nia/.env` (the skill will prompt for this on first load).
...
## NIA Execution Pattern (desktop app flow)

NIA terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nia-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nia-op.sock"
