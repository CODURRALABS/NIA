---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into NIA"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into NIA"
---
...
Migrate a user's OpenClaw customization footprint into NIA. Imports NIA-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nia skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | NIA (Anonymousinsaan & Codurra Labs) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `NIA`, `Memory`, `Persona`, `Import` |
| Related skills | [`nia-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nia-agent) |
...
```bash
nia claw migrate              # Full interactive migration
...
nia claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `nia setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_nia.py` to:

- import `SOUL.md` into the NIA home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into NIA memory entries
- merge OpenClaw command approval patterns into NIA `command_allowlist`
- migrate NIA-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.nia/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen NIA workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.nia/tts/`
- archive non-secret docs that do not have a direct NIA destination
...
- `scripts/openclaw_to_nia.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py`

Do not guess a shorter path like `~/.nia/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.nia/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of NIA-compatible secrets, currently:
...
NIA CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in NIA. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a NIA workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing NIA skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.nia/skills/migration/openclaw-migration/scripts/openclaw_to_nia.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of NIA unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty NIA targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean NIA destination. Unsupported auth blobs must still be reported as skipped.
...
- NIA persona state imported
- NIA memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.nia/skills/openclaw-imports/`
