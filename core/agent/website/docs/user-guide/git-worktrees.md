...
title: "Git Worktrees"
description: "Run multiple NIAs safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

NIA is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with NIA so each session has a clean, isolated working directory.

## Why Use Worktrees with NIA?

NIA treats the **current working directory** as the project root:

- CLI: the directory where you run `nia` or `nia chat`
...
git worktree add ../repo-feature feature/nia-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nia-experiment` checked out in that directory

Now you can `cd` into the new worktree and run NIA there:

```bash
cd ../repo-feature

# Start NIA in the worktree
nia
```
...
git worktree add ../repo-experiment-a feature/nia-a
git worktree add ../repo-experiment-b feature/nia-b
```
...
- Works on its own branch (`feature/nia-a` vs `feature/nia-b`).
...
- NIA checkpoint data under `~/.nia/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per NIA experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nia-checkpoints-docs`, `feature/nia-refactor-tests`.
- **Commit frequently**
...
- **Avoid running NIA from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nia -w` (Automatic Worktree Mode)

NIA has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nia/nia-<hash>`).
...
```bash
nia -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nia -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each NIA session its own clean checkout.
