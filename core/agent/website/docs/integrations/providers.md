...
This page covers setting up inference providers for NIA — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use NIA.

## Inference Providers

You need at least one way to connect to an LLM. Use `nia model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `nia model` (OAuth, subscription-based) |
| **OpenAI Codex** | `nia model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `nia model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `nia model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `nia model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.nia/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.nia/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.nia/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.nia/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.nia/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.nia/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.nia/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.nia/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.nia/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.nia/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.nia/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.nia/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.nia/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.nia/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.nia/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.nia/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.nia/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.nia/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `nia model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `nia model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `nia model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — NIA auto-provisions your free tier on first request
```

NIA ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.nia/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `NIA_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of NIA works unchanged
...
| Workspace / Standard / Enterprise account | Set `NIA_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | NIA detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. NIA shows
...
```bash
NIA_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
NIA_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). NIA stores the resulting credentials in its own auth store under `~/.nia/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), NIA routes these tasks to your **main chat model** — the same model you picked in `nia model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/docs/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `nia model` setup, or enable it later with `nia tools`.
:::

### Two Commands for Model Management

NIA has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`nia model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a NIA chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `nia model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `nia model`, complete the provider setup, then start a new session.
...
When you authenticate via `nia model` → Anthropic OAuth (or via `nia auth add anthropic --type oauth`), NIA routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by NIA — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
nia chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `nia model`
# NIA will use Claude Code's credential store directly when available
nia model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
nia chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
nia chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `nia model`, NIA prefers Claude Code's own credential store over copying the token into `~/.nia/.env`. That keeps refreshable Claude credentials refreshable.
...
NIA supports GitHub Copilot as a first-class provider with two modes:
...
```bash
nia chat --provider copilot --model gpt-5.4
```
...
If no token is found, `nia model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `nia model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `nia model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in NIA
NIA sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, NIA now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). NIA therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
nia chat --provider copilot-acp --model copilot-acp
...
| `NIA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `NIA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
nia chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.nia/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
nia chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.nia/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
nia chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.nia/.env

# MiniMax (global endpoint)
nia chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.nia/.env

# MiniMax (China endpoint)
nia chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.nia/.env

# Alibaba Cloud / DashScope (Qwen models)
nia chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.nia/.env

# Xiaomi MiMo
nia chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.nia/.env

# Tencent TokenHub (Hy3 Preview)
nia chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.nia/.env

# Arcee AI (Trinity models)
nia chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.nia/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
nia chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.nia/.env
```
...
When using the Z.AI / GLM provider, NIA automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.nia/.env` and pick xAI in `nia model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

When using xAI as a provider (any base URL containing `x.ai`), NIA automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `nia tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

### Ollama Cloud — Managed Ollama Models, OAuth + API Key

[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `nia model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and NIA auto-discovers the available models.
...
nia chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 nia chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `nia model`, sign in through the browser, and NIA persists the refresh token.

```bash
nia model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.nia/auth.json

nia chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `NIA_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), NIA exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
nia chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `nia model`, sign in through the browser, and NIA persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
nia chat   # uses api.minimax.io/anthropic endpoint
```
...
nia chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.nia/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 nia chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
nia chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.nia/.env

# Short alias
nia chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
NIA works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point NIA at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of NIA — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `nia setup` or config migration. Use `nia model` or edit `config.yaml` directly.
:::
...
:::warning nia model vs /model
**`nia model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active NIA chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `nia model`, set up the new provider, then start a new session.
:::
...
When switching providers, NIA persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like NIA.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in NIA) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `nia` (Qwen 2.5, NIA 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure NIA to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but NIA won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), NIA 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
NIA will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, NIA) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since NIA requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your NIA config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your NIA config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with NIA.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running NIA inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser nia` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. NIA's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what NIA thinks the context is
...
NIA auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), NIA uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure NIA with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in NIA.
...
Then configure NIA with `nia model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure NIA with `nia model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `nia model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). NIA uses this to decide when to compress history and to validate API requests.
...
NIA uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`nia model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — NIA uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `nia model` menu.
...
:::tip
You can switch between providers at any time with `nia model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point NIA at your instance (no API key needed):
   ```bash
   nia config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.nia/config.yaml`:
...
Configure a backup provider:model that NIA switches to automatically when your primary model fails (rate limits, server errors, auth failures):
