...
NIA connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what NIA can do and where it can run.

## AI Providers & Routing

NIA supports multiple AI inference providers out of the box. Use `nia model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/docs/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. NIA auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/docs/user-guide/features/mcp)** — Connect NIA to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native NIA tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support four backend providers, configured via `config.yaml` or `nia tools`:
...
NIA includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/docs/user-guide/features/acp)** — Use NIA inside ACP-compatible editors such as VS Code, Zed, and JetBrains. NIA runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/docs/user-guide/features/api-server)** — Expose NIA as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use NIA as a backend with its full toolset.
...
NIA runs as a gateway bot on 15+ messaging platforms, all configured through the same `gateway` subsystem:
...
- **[Plugin System](/docs/user-guide/features/plugins)** — Extend NIA with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.nia/plugins/`, project-local `.nia/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/docs/guides/build-a-nia-plugin)** — Step-by-step guide for creating NIA plugins with tools, hooks, and CLI commands.
