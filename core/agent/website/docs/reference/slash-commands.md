...
NIA has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `nia_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.nia/plans/` relative to the active workspace/backend working directory.
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of NIA config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
| `/goal <text>` | Set a standing goal NIA works toward across turns — our take on the Ralph loop. After each turn an auxiliary judge model decides whether the goal is done; if not, NIA auto-continues. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Budget defaults to 20 turns (`goals.max_turns`); any real user message preempts the continuation loop, and state survives `/resume`. See [Persistent Goals](/docs/user-guide/features/goals) for the full walkthrough. |
...
| `/busy [queue\|steer\|interrupt\|status]` | CLI-only: control what pressing Enter does while NIA is working — queue the new message, steer mid-turn, or interrupt immediately. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.nia/config.yaml`:
