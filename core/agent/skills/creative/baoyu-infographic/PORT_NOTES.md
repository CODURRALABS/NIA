...
| Change | Upstream | NIA |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `nia` |
...
| File operations | Bash commands | NIA file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains NIA-specific adaptations.
