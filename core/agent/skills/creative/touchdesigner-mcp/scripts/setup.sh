...
NIA_HOME_DIR="${NIA_HOME:-$HOME/.nia}"
NIA_CFG="${NIA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure NIA config has twozero_td MCP entry ──
if [[ ! -f "$NIA_CFG" ]]; then
    echo -e " ${FAIL} NIA config not found at ${NIA_CFG}"
    manual_steps+=("Create ${NIA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NIA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in NIA config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to NIA config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NIA_CFG} manually"); }
    manual_steps+=("Restart NIA session to pick up config change")
fi
