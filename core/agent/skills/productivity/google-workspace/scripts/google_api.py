#!/usr/bin/env python3
"""Google Workspace API CLI for NIA.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing NIA-facing JSON contract and falls back to the Python client
...
from _nia_home import get_nia_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NIA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for NIA")
