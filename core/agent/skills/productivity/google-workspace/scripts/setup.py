#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for NIA.
...
# Ensure sibling modules (_nia_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _nia_home import display_nia_home, get_nia_home

NIA_HOME = get_nia_home()
TOKEN_PATH = NIA_HOME / "google_token.json"
CLIENT_SECRET_PATH = NIA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = NIA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same NIA profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'nia-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to NIA home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_nia_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for NIA")
