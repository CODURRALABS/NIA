...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NIA_HOME", os.path.expanduser("~/.nia")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NIA_HOME", Path.home() / ".nia")) / "skills" / "red-teaming" / "godmode"
...
NIA_HOME = Path(os.getenv("NIA_HOME", Path.home() / ".nia"))
CONFIG_PATH = NIA_HOME / "config.yaml"
PREFILL_PATH = NIA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/NIA models — already uncensored, just needs clean prompt
    "nia": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nia" in model_lower or "nous" in model_lower:
        return "nia"
...
def _get_current_model() -> tuple:
    """Read current model and provider from NIA config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart NIA for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart NIA for changes to take effect.")
