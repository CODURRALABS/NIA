...
Thank you for contributing to NIA! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want NIA to work everywhere.
...
Bundled skills (in `skills/`) ship with every NIA install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `nia skills browse` (labeled "official") and install it with `nia skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Anonymousinsaan & Codurra Labs Discord](https://discord.gg/NousResearch). Users can install it with `nia skills install`.
...
```bash
git clone --recurse-submodules Local Sovereign Environment.git
cd nia-agent
...
```bash
mkdir -p ~/.nia/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.nia/config.yaml
touch ~/.nia/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.nia/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/nia" ~/.local/bin/nia
...
├── cli.py                    # NIACLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (nia-cli, nia-telegram, etc.)
├── nia_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── nia_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.nia/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (nia-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.nia/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.nia/`)

| Path | Purpose |
|------|---------|
| `~/.nia/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.nia/.env` | API keys and secrets |
| `~/.nia/auth.json` | OAuth credentials (Nous Portal) |
| `~/.nia/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.nia/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.nia/state.db` | SQLite session database |
| `~/.nia/sessions/` | JSON session logs |
| `~/.nia/cron/` | Scheduled job data |
| `~/.nia/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`nia_state.py`) with full-text search and unique session titles. JSON logs go to `~/.nia/sessions/`.
...
Four fields are supported under `metadata.nia`:
...
The user may skip setup and keep loading the skill. NIA only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `nia setup` or update `~/.nia/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing NIA tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `nia --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

NIA uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.nia/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `nia_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `nia_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

NIA runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
NIA has terminal access. Security matters.
...
2. **Test manually**: Run `nia` and exercise the code path you changed
...
- Use [GitHub Issues](Local Sovereign Environment/issues)
- Include: OS, Python version, NIA version (`nia version`), full error traceback
