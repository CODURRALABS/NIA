#!/usr/bin/env bash
# Canonical test runner for nia-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.nia/nia-agent/venv"; do
...
# Unset NIA_* behavioral vars too.
unset NIA_YOLO_MODE NIA_INTERACTIVE NIA_QUIET NIA_TOOL_PROGRESS \
      NIA_TOOL_PROGRESS_MODE NIA_MAX_ITERATIONS NIA_SESSION_PLATFORM \
      NIA_SESSION_CHAT_ID NIA_SESSION_CHAT_NAME NIA_SESSION_THREAD_ID \
      NIA_SESSION_SOURCE NIA_SESSION_KEY NIA_GATEWAY_SESSION \
      NIA_PLATFORM NIA_INFERENCE_PROVIDER NIA_MANAGED NIA_DEV \
      NIA_CONTAINER NIA_EPHEMERAL_SYSTEM_PROMPT NIA_TIMEZONE \
      NIA_REDACT_SECRETS NIA_BACKGROUND_NOTIFICATIONS NIA_EXEC_ASK \
      NIA_HOME_MODE 2>/dev/null || true
