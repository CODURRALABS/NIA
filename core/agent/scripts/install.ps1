# ============================================================================
# NIA Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/nia-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$NIAHome = "$env:LOCALAPPDATA\nia",
...
$RepoUrlSsh = "git@github.com:NousResearch/nia-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/nia-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ NIA Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$NIAHome\node;$env:Path"
        Write-Success "Node.js $version found (NIA-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.nia/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$NIAHome\node") { Remove-Item -Recurse -Force "$NIAHome\node" }
                Move-Item $extractedDir.FullName "$NIAHome\node"
                $env:Path = "$NIAHome\node;$env:Path"

                $version = & "$NIAHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.nia/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/nia-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\nia-agent-$Branch.zip"
...
function Set-PathVariable {
    Write-Info "Setting up nia command..."
    
    if ($NoVenv) {
        $niaBin = "$InstallDir"
    } else {
        $niaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nia is globally available
    # On Windows, the nia.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$niaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$niaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $niaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NIA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nia instead
    # of the Unix default ~/.nia
    $currentNIAHome = [Environment]::GetEnvironmentVariable("NIA_HOME", "User")
    if (-not $currentNIAHome -or $currentNIAHome -ne $NIAHome) {
        [Environment]::SetEnvironmentVariable("NIA_HOME", $NIAHome, "User")
        Write-Success "Set NIA_HOME=$NIAHome"
    }
    $env:NIA_HOME = $NIAHome
    
    # Update current session
    $env:Path = "$niaBin;$env:Path"
    
    Write-Success "nia command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.nia directory structure
    New-Item -ItemType Directory -Force -Path "$NIAHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NIAHome\skills" | Out-Null
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.nia/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.nia/.env"
        }
    } else {
        Write-Info "~/.nia/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$NIAHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.nia/config.yaml from template"
        }
    } else {
        Write-Info "~/.nia/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$NIAHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.nia/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.nia/"
    
    # Seed bundled skills into ~/.nia/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.nia/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.nia/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NIAHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.nia/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (nia --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nia_cli.main setup
    } else {
        python -m nia_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $niaCmd = "$InstallDir\venv\Scripts\nia.exe"
    if (-not (Test-Path $niaCmd)) {
        $niaCmd = "nia"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NIAHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nia whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NIAHome\logs\gateway.log"
            Start-Process -FilePath $niaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NIAHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nia gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nia gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NIAHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NIAHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NIAHome\cron\, sessions\, logs\"
    ...
    Write-Host ""
    Write-Host "   nia              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nia setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nia config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nia config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nia gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nia update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/nia-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
