...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/nia-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/nia-agent.git"
NIA_HOME="${NIA_HOME:-$HOME/.nia}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NIA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NIA_INSTALL_DIR"
...
#   code at /usr/local/lib/nia-agent, command at /usr/local/bin/nia,
#   data still at /root/.nia (NIA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.nia/nia-agent"
            echo "                   default (root, Linux): /usr/local/lib/nia-agent"
            echo "  --nia-home PATH  Data directory (default: ~/.nia, or \$NIA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, NIA installs the code under"
            echo "  /usr/local/lib/nia-agent and links the command into"
            echo "  /usr/local/bin/nia (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NIA_HOME"
            echo "  (default /root/.nia).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$NIA_HOME/nia-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `nia` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NIA_HOME/nia-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NIA_HOME/nia-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nia-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $NIA_HOME/nia-agent — then preserve it)
#
# Always no-op when the user set --dir or $NIA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NIA_HOME.
    if is_termux; then
        INSTALL_DIR="$NIA_HOME/nia-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NIA_HOME/nia-agent/.git" ]; then
            INSTALL_DIR="$NIA_HOME/nia-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nia-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nia-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/nia"
        log_info "  Data:    $NIA_HOME (unchanged)"
        return 0
    fi
...
get_nia_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nia" ]; then
        echo "$link_dir/nia"
    else
        echo "nia"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/nia-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$NIA_HOME/node/bin/node" ]; then
        export PATH="$NIA_HOME/node/bin:$PATH"
        local found_ver=$("$NIA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (NIA-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.nia/node/..."
    ...
    # Place into ~/.nia/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$NIA_HOME/node"
    mkdir -p "$NIA_HOME"
    mv "$extracted_dir" "$NIA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$NIA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$NIA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$NIA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$NIA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$NIA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.nia/node/"
    HAS_NODE=true
}
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="nia-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    log_info "Setting up nia command..."

    if [ "$USE_VENV" = true ]; then
        NIA_BIN="$INSTALL_DIR/venv/bin/nia"
    else
        NIA_BIN="$(which nia 2>/dev/null || echo "")"
        if [ -z "$NIA_BIN" ]; then
            log_warn "nia not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$NIA_BIN" ]; then
        log_warn "nia entry point not found at $NIA_BIN"
    ...
    # Create a user-facing shim for the nia command.
    mkdir -p "$command_link_dir"
    ln -sf "$NIA_BIN" "$command_link_dir/nia"
    log_success "Symlinked nia → $command_link_display_dir/nia"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nia command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nia' \
                >/dev/null 2>&1; then
        ...
        log_info "nia not on PATH in non-login shells (common on RHEL-family)"
    ...
    # Export for current session so nia works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.nia directory structure (config at top level, code in subdir)
    mkdir -p "$NIA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.nia/.env (top level, easy to find)
    if [ ! -f "$NIA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NIA_HOME/.env"
            log_success "Created ~/.nia/.env from template"
        else
            touch "$NIA_HOME/.env"
            log_success "Created ~/.nia/.env"
        fi
    else
        log_info "~/.nia/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.nia/config.yaml (top level, easy to find)
    if [ ! -f "$NIA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NIA_HOME/config.yaml"
            log_success "Created ~/.nia/config.yaml from template"
        fi
    else
        log_info "~/.nia/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$NIA_HOME/SOUL.md" ]; then
        cat > "$NIA_HOME/SOUL.md" << 'SOUL_EOF'
# NIA Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how NIA communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.nia/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.nia/"

    # Seed bundled skills into ~/.nia/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.nia/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.nia/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NIA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$NIA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.nia/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — NIA itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (nia --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nia setup' after install."
        return 0
    fi
    ...
    # Run nia setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m nia_cli.main setup < /dev/tty
    else
        python -m nia_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$NIA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$NIA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'nia whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NIA_CMD="$(get_nia_command_path)"
                $NIA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nia whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nia gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NIA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NIA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nia gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nia gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NIA_CMD gateway > "$NIA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.nia/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nia gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nia gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NIA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NIA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NIA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}nia${NC}              Start chatting"
    echo -e "   ${GREEN}nia setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}nia config${NC}       View/edit configuration"
    echo -e "   ${GREEN}nia config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}nia gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nia update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nia' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nia' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nia' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
