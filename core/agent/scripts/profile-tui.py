#!/usr/bin/env python3
"""Drive the NIA TUI under NIA_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.nia/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with NIA_TUI_RESUME set so it
bypasses the nia_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  NIA_PERF_LOG     (default ~/.nia/perf.log)
  NIA_PERF_NODE    (default node from $PATH)
  NIA_TUI_DIR      (default /home/bb/nia-agent/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests NIA_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_nia_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("NIA_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".nia"

DEFAULT_TUI_DIR = Path(os.environ.get("NIA_TUI_DIR", "/home/bb/nia-agent/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("NIA_PERF_LOG", str(get_nia_home() / "perf.log")))
DEFAULT_STATE_DB = get_nia_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — NIA_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["NIA_DEV_PERF"] = "1"
    env["NIA_DEV_PERF_MS"] = str(args.threshold_ms)
    env["NIA_DEV_PERF_LOG"] = str(log)
    env["NIA_TUI_RESUME"] = sid
    ...
    node = os.environ.get("NIA_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="NIA_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "nia-ink" / "src"
