...
VERSION_FILE = REPO_ROOT / "nia_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+nia-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "nia@nousresearch.com": "NousResearch",
    ...
    "nia@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "nia@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="Local Sovereign Environment",
    ...
    lines.append(f"# NIA v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for NIA. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="NIA Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"NIA v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"NIA v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'NIA v{new_version} ({calver_date})' "
