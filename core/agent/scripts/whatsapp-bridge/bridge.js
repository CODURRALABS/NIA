 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.nia/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.nia', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.nia', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.nia', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.nia', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *NIA*\n────────────\n';
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore NIA' own reply messages in self-chat mode to avoid loops.
