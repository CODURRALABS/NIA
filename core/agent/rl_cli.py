...
from nia_constants import OPENROUTER_BASE_URL, get_nia_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['NIA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to nia-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.nia/config.yaml if not provided)
    ...
    """
    # Load config from ~/.nia/config.yaml
    config = load_nia_config()
