#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `nia kanban list` and
...
def nia_available() -> bool:
    return shutil.which("nia") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed nia CLI."""
    try:
        out = subprocess.run(
            ["nia", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `nia kanban list`
    out = subprocess.run(
        ["nia", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not nia_available():
        print("ERROR: 'nia' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
