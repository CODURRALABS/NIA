...
Maps each role archetype to the NIA skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nia-agent
...
## NIA skills relevant to video production

### Visual / rendering skills (`nia-agent/skills/creative/`)
...
### Design / pre-production skills (`nia-agent/skills/creative/`)
...
### Audio / media skills (`nia-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`nia-agent/skills/devops/`)
...
These are **not** NIA skills but external CLIs / APIs that profiles invoke.
...
## Built-in NIA tools for media review

These are native NIA tools — not invoked via terminal but through their own
...
key is present in `~/.nia/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every NIA profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.nia/.env` → environment variable.
...
When a new NIA-public video skill ships:
