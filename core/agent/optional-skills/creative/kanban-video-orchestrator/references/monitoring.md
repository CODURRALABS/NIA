...
nia kanban watch --tenant <project-slug>

# Snapshot of the board
nia kanban list --tenant <project-slug>
nia kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nia kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nia dashboard

# Inspect a specific task (includes comments + events)
nia kanban show <task-id>

# Follow a single task's event stream
nia kanban tail <task-id>
```

Verify available subcommands with `nia kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nia kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nia kanban block <id>` then `nia kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nia kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nia kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nia profile list` (profile exists?), `nia status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(nia kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nia kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nia kanban block <task-id>
nia kanban archive <task-id>

# Diagnose what happened
nia kanban show <task-id>      # task body, comments, recent events
nia kanban tail <task-id>      # follow the live event stream
nia kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nia kanban create` for the director
...
while true; do
    clear
    nia kanban list --tenant <slug>
    echo "---"
    nia kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nia kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nia profile list` and start
