...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with nia-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.nia/plugins/<name>/``
3. **Project plugins** – ``./.nia/plugins/<name>/`` (opt-in via
   ``NIA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``nia_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "nia_agent.plugins"

_NS_PARENT = "nia_plugins"
...
class PluginManifest:
    ...
    #              in ~/.nia/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``nia plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``nia honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``nia <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.nia/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to nia_cli/. Two layouts are
        ...
        # 2. User plugins (~/.nia/plugins/)
        user_dir = get_nia_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.nia/plugins/)
        if _env_enabled("NIA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".nia" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with nia and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform NIA ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `nia plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``nia_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``nia_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``nia tools`` TUI so plugin-provided toolsets appear
