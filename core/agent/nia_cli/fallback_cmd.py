"""
nia fallback — manage the fallback provider chain.
...
Subcommands:
  nia fallback [list]   Show the current fallback chain (default when no subcommand)
  nia fallback add      Pick provider + model via the same picker as `nia model`,
                           then append the selection to the chain
  nia fallback remove   Pick an entry to delete from the chain
  nia fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nia/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nia_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nia model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nia fallback add")
        print()
        return
    ...
    print("  Docs: Local Sovereign Hub/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nia model`, then append the selection to the chain."""
    from nia_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nia model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nia fallback list` to view, or `nia fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nia fallback [subcommand]``."""
