"""``nia debug`` debug tools for NIA.

Currently supports:
    nia debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.nia/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[nia debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.nia/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``nia debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``nia debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, NIA version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `nia debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nia-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``nia debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.nia/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``nia debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nia-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from nia_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_nia_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``nia dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``nia dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "nia debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  nia debug delete <url>")

    print(f"\nShare these links with the NIA team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nia debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'nia debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``nia debug`` call.
    ...
    # best-effort — any failure is swallowed so ``nia debug`` stays
