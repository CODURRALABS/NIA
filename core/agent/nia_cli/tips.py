...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nia -c.",
    ...
    "Ctrl+Z suspends NIA to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nia -c resumes your most recent CLI session. nia -c \"project name\" resumes by title.",
    "nia -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nia -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nia chat -t web,terminal enables only specific toolsets for a focused session.",
    "nia chat -s github-pr-workflow preloads a skill at launch.",
    "nia chat -q \"query\" runs a single non-interactive query and exits.",
    "nia chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "nia chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nia --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nia chat --source telegram tags the session for filtering in nia sessions list.",
    "nia -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nia doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nia dump outputs a compact setup summary — great for bug reports.",
    "nia config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nia config edit opens config.yaml in your default editor.",
    "nia config check scans for missing or stale configuration options.",
    "nia sessions browse opens an interactive session picker with search.",
    "nia sessions stats shows session counts by platform and database size.",
    "nia sessions prune --older-than 30 cleans up old sessions.",
    "nia skills search react --source skills-sh searches the skills.sh public directory.",
    "nia skills check scans installed hub skills for upstream updates.",
    "nia skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nia skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nia mcp add github --command npx adds MCP servers from the command line.",
    "nia mcp serve runs NIA itself as an MCP server for other agents.",
    "nia auth add lets you add multiple API keys for credential pool rotation.",
    "nia completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nia logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nia backup creates a zip backup of your entire NIA home directory.",
    "nia profile create coder creates an isolated profile that becomes its own command.",
    "nia profile create work --clone copies your current config and keys to a new profile.",
    "nia update syncs new bundled skills to ALL profiles automatically.",
    "nia gateway install sets up NIA as a system service (systemd/launchd).",
    "nia memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nia webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nia tools disables unused tools, nia skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nia models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nia -w).",
    ...
    "NIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call NIA tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'nia profile create coder' creates the 'coder' command.",
    "nia profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, NIA prints a resume command with session ID and stats.",
    "nia sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nia -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nia skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nia/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nia cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "NIA runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "nia gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to NIA via a local macOS server.",
    ...
    "Context files (.nia.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.nia/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "NIA loads project context from .nia.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — nia tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nia/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nia chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nia plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nia acp runs NIA as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NIA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "nia login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "nia status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.nia/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make NIA your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.nia/SOUL.md to customize.",
    ...
    "Before a gateway session reset, NIA auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nia/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at NIA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "NIA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NIA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nia/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nia profile create ls' would be rejected.",
    "nia profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nia claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NIA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via nia tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.nia/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.nia/skills/.hub/quarantine/ holds skills pending security review.",
]
