"""
Doctor command for nia CLI.

Diagnoses issues with NIA setup.
"""
...
from nia_cli.config import get_project_root, get_nia_home, get_env_path
from nia_constants import display_nia_home

PROJECT_ROOT = get_project_root()
NIA_HOME = get_nia_home()
_DHH = display_nia_home()  # user-facing display path (e.g. ~/.nia or ~/.nia/profiles/coder)

# Load environment variables from ~/.nia/.env so API key checks work
...
from nia_cli.models import _NIA_USER_AGENT
from nia_cli.vercel_auth import describe_vercel_auth
from nia_constants import OPENROUTER_MODELS_URL
...
from nia_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.nia/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `nia`.
    os.environ.setdefault("NIA_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 NIA Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.nia/.env (primary location for user config)
    env_path = NIA_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nia setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nia setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'nia setup' to create one")
                issues.append("Run 'nia setup' to create .env")
    
    # Check ~/.nia/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from nia_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'nia config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from nia_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.nia/.env or run 'nia setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nia setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nia config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from nia_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'nia setup' to migrate config")
                else:
                    issues.append("Run 'nia doctor --fix' or 'nia setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'nia doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses NIA' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `nia auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    for subdir_name in expected_subdirs:
        subdir_path = nia_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give NIA a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# NIA Persona\n\n"
                "<!-- Edit this file to customize how NIA communicates. -->\n\n"
    ...
    memories_dir = nia_home / "memories"
    ...
    state_db_path = nia_home / "state.db"
    ...
    wal_path = nia_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'nia doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (nia bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nia"
        ...
        _cmd_link = _cmd_link_dir / "nia"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(nia not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/nia → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/nia points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/nia → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/nia — run 'nia doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/nia exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/nia not found",
                    "(nia command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/nia → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/nia symlink — run 'nia doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'nia-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'nia-agent[vercel]'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'nia config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'nia setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: nia skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = NIA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: nia memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: nia memory setup")
                issues.append("No Honcho API key — run 'nia memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run nia memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: nia memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: nia memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from nia_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "nia -p" in content:
                            _m = _re.search(r"nia -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'nia doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
