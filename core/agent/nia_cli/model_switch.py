...
- ``nia_cli.providers``        -- canonical provider identity + overlays
- ``nia_cli.model_normalize``  -- per-provider name formatting
...
_NIA_MODEL_WARNING = (
    "Anonymousinsaan & Codurra Labs NIA 3 & 4 models are NOT agentic and are not designed "
    "for use with NIA. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Anonymousinsaan & Codurra Labs NIA 3 / NIA 4 chat families.
# The previous substring check (`"nia" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``nia-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nia" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/NIA-3-Llama-3.1-70B, nia-4-405b, openrouter/nia3:70b
# Negative examples it must NOT match:
#   nia-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_NIA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nia[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_nia_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous NIA 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_NIA_NON_AGENTIC_RE.search(model_name))


def _check_nia_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous NIA 3/4 chat model."""
    if is_nous_nia_non_agentic(model_name):
        return _NIA_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nia_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nia model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nia doctor' — config issues detected:"
    ...
    # Mirrors the same logic in nia_cli.runtime_provider.resolve_runtime_provider;
    ...
    nia_warn = _check_nia_model_warning(new_model)
    if nia_warn:
        warnings.append(nia_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from nia_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nia_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nia provider ID
    ...
    # --- 1. Check NIA-mapped providers ---
    for nia_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(nia_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NIA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from nia_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and nia_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a NIA release.
        model_ids = curated.get(nia_id, [])
        if nia_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(nia_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check NIA-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nia_cli.providers import NIA_OVERLAYS
    from nia_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → NIA provider ID.
    # NIA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use NIA IDs ("copilot").
    _mdev_to_nia = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NIA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve NIA slug — e.g. "github-copilot" → "copilot"
        nia_slug = _mdev_to_nia.get(pid, pid)
        if nia_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(nia_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, nia_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nia_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(nia_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", nia_slug, exc)
        ...
        # configured.
        if not has_creds and nia_slug == "anthropic":
            try:
                ...
                if (nia_creds and nia_creds.get("accessToken")) or \
        ...
        if nia_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(nia_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(nia_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(nia_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by NIA slug, fall back to overlay key
            model_ids = curated.get(nia_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if nia_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nia_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": nia_slug,
            "name": get_label(nia_slug),
            "is_current": nia_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nia",
        ...
        seen_slugs.add(nia_slug.lower())
        _record_builtin_endpoint(nia_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NIA_OVERLAYS (keeps /model in sync
    # with `nia model`).
    try:
        from nia_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is NIA's canonical write key (matches
            ...
            # NIA writes ``models:`` as a dict keyed by model id
            # (see nia_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # NIA's own writer uses; a hyphen variant is accepted
            ...
            # active model. NIA's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nia_cli/config.py) already consume that dict.
