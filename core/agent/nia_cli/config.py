"""
Configuration management for NIA.

Config files are stored in ~/.nia/ for easy access:
- ~/.nia/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nia/.env         - API keys and secrets

This module provides:
- nia config          - Show current configuration
- nia config edit     - Open config in editor
- nia config set      - Set a specific value
- nia config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `nia plugins enable
    # observability/langfuse` or `nia tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NIA_LANGFUSE_ENV",
    "NIA_LANGFUSE_RELEASE",
    "NIA_LANGFUSE_SAMPLE_RATE",
    "NIA_LANGFUSE_MAX_CHARS",
    "NIA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from nia_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NIA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nia_home() / ".managed"
...
def is_managed() -> bool:
    """Check if NIA is running in package-manager-managed mode.

    Two signals: the NIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in NIA_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "nia update"


def format_managed_message(action: str = "modify this NIA installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NIA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this NIA installation is managed by NixOS "
            f"(NIA_MANAGED={env_hint}).\n"
            "Edit services.nia-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this NIA installation is managed by Homebrew "
            f"(NIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nia-agent"
        )

    return (
        f"Cannot {action}: this NIA installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall NIA."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NIA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nia_bin
    or None if container mode is not active, we're already inside the
    container, or NIA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NIA_DEV") == "1":
        return None
    ...
    container_mode_file = get_nia_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nia-agent")
    exec_user = info.get("exec_user", "nia")
    nia_bin = info.get("nia_bin", "/data/current-package/bin/nia")
...
# Re-export from nia_constants — canonical definition lives there.
from nia_constants import get_nia_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nia_home() / "config.yaml"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nia group can
    share state with the gateway service.

    The mode can be overridden via the NIA_HOME_MODE environment variable
    (e.g. NIA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NIA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NIA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When NIA runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NIA_CONTAINER") or os.environ.get("NIA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NIA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NIA_HOME if the user doesn't have one yet."""
...
def ensure_nia_home():
    """Ensure ~/.nia directory structure exists with secure permissions.
...
def _ensure_nia_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the NIA-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, NIA auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), NIA sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when NIA
        ...
        # bundled NIA image, which drops to the `nia` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, NIA sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.nia/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, nia sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: NIA_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, NIA truncates the
    ...
    "auxiliary": {
        ...
        # model; override via `nia model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `nia --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nia -c` muscle memory.  Default off so existing
        # users aren't surprised.  NIA_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/nia`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nia/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nia/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, NIA feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before NIA auto-pauses the goal and
    ...
    # always goes to ~/.nia/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NIA_SKILL_DIR} and ${NIA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nia curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.nia/skills/ into
        # ~/.nia/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nia curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nia-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *NIA*" header.
    ...
    # stored approval from ~/.nia/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NIA_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via NIA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nia -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     nia-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.nia/logs/.
    ...
    # update model picker lists without shipping a nia-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "Local Sovereign Hub/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nia model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.nia/state.db.
    ...
    # ``nia update`` behaviour.
    "updates": {
        # Run a full ``nia backup``-style zip of NIA_HOME before every
        # ``nia update``.  Backups land in ``<NIA_HOME>/backups/`` and
        # can be restored with ``nia import <path>``.  Off by default —
        # on large NIA_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nia model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the NIA auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nia:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NIA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nia)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nia-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nia-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote NIA API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote NIA API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote NIA API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NIA_TOOL_PROGRESS and NIA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nia.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nia update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older NIA versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nia_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nia setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — NIA won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nia doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NIA_HOME", "~/.nia")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NIA_TOOL_PROGRESS")
            old_mode = get_env_value("NIA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NIA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NIA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NIA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NIA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_nia_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nia plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nia curator status` has no
    ...
    #   3. Creates `~/.nia/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nia_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nia_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: nia config set <key> <value>")
    ...
    # metadata.nia.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nia_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nia/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nia/config.yaml.
    ...
    (which change ``NIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_nia_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — nia auth) — OpenAI Codex
#   nous         (OAuth — nia auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.nia/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nia/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real NIA env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nia/.env in place.
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nia/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nia/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to NIA — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known NIA vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nia/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ NIA Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  nia config edit     # Edit config file", Colors.DIM))
    print(color("  nia config set <key> <value>", Colors.DIM))
    print(color("  nia setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nia config set <key> <value>")
            print()
            print("Examples:")
            print("  nia config set model anthropic/claude-sonnet-4")
            print("  nia config set terminal.backend docker")
            print("  nia config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nia config migrate' to add them")
        ...
        print("  nia config           Show current configuration")
        print("  nia config edit      Open config in editor")
        print("  nia config set <key> <value>   Set a config value")
        print("  nia config check     Check for missing/outdated config")
        print("  nia config migrate   Update config with new options")
        print("  nia config path      Show config file path")
        print("  nia config env-path  Show .env file path")
        sys.exit(1)
