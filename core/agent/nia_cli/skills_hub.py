#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the NIA Skills Hub.

Powers both:
  - `nia skills <subcommand>` (CLI argparse entry point)
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.nia/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.nia/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: nia skills inspect <identifier> to preview, "
            "nia skills install <identifier> to install[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Anonymousinsaan & Codurra Labs[/]")
    c.print()
    ...
    c.print("[dim]Tip: 'nia skills search <query>' searches deeper across all registries[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not bundle:
        ...
        if rate_limited:
            c.print(
                "[yellow]Sovereign Mode:[/] Network connections to external registries are restricted.\n"
                "Please install skills locally using [bold]nia skills add <path>[/].\n"
            )
        else:
            c.print()
        return
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]nia skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Anonymousinsaan & Codurra Labs.[/]\n\n"
                "It ships with nia-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_nia_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="nia skills install <id> to install"))
...
def do_list(source_filter: str = "all",
    ...
    config — ``nia -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``NIA_HOME`` at process
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nia skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nia skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: nia skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_nia_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via NIA Skills Hub.\n\n"
                        f"This skill was scanned by the NIA Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `nia skills <subcommand>` — called from nia_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: nia skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: nia skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: nia skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
        _console.print("Run 'nia skills <command> --help' for details.\n")
