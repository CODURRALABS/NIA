...
- Keep data: Remove code but keep ~/.nia/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove NIA PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nia-agent or nia PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# NIA" comment and following line
                if '# NIA' in line or '# nia-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nia' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nia
                if 'nia' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nia wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nia",
        Path("/usr/local/bin/nia"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nia_cli reference)
                content = wrapper.read_text()
                if 'nia_cli' in content or 'nia-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nia gateway run`` processes
    ...
    try:
        from nia_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nia_cli.gateway import get_launchd_plist_path
...
def _is_default_nia_home(nia_home: Path) -> bool:
    """Return True when ``nia_home`` points at the default (non-profile) root."""
    try:
        from nia_constants import get_default_nia_root
        return nia_home.resolve() == get_default_nia_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NIA_HOME directory.

    We shell out to ``nia -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nia_cli.main` so we don't depend on a `nia`
    #    wrapper that may be half-removed mid-uninstall.
    nia_invocation = [_sys.executable, "-m", "nia_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nia_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NIA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nia/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nia/ for future reinstall
    """
    project_root = get_project_root()
    nia_home = get_nia_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NIA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nia_home(nia_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ NIA Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nia_home / 'config.yaml'}")
    print(f"  Secrets: {nia_home / '.env'}")
    print(f"  Data:    {nia_home / 'cron/'}, {nia_home / 'sessions/'}, {nia_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL NIA data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the NIA code but keep your configuration and data.")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nia/, just remove the nia-agent subdir
            if nia_home in project_root.parents or project_root.parent == nia_home:
    ...
    # 5. Optionally remove ~/.nia/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NIA_HOME dirs live under
        ...
        try:
            if nia_home.exists():
                shutil.rmtree(nia_home)
                log_success(f"Removed {nia_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nia_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nia_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/nia-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using NIA! ⚕")
    print()
