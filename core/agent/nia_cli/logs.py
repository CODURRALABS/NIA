"""``nia logs`` — view and filter NIA log files.
...
Usage examples::
    ...
    nia logs                    # last 50 lines of agent.log
    nia logs -f                 # follow agent.log in real time
    nia logs errors             # last 50 lines of errors.log
    nia logs gateway -n 100    # last 100 lines of gateway.log
    nia logs --level WARNING    # only WARNING+ lines
    nia logs --session abc123   # filter by session ID substring
    nia logs --component tools  # only tool-related lines
    nia logs --since 1h         # lines from the last hour
    nia logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nia_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when NIA runs — try 'nia chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nia_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nia_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nia_home()}/logs/")
        return

    print(f"Log files in {display_nia_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nia chat' to generate logs)")
