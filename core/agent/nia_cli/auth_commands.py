...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_nia_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:nia_pkce",
    ...
    if provider == "nous":
        ...
        # ~/.nia/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `nia --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `nia auth
    ...
    raise SystemExit(f"`nia auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source NIA reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `nia auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `nia auth` is called bare."""
