"""nia hooks — inspect and manage shell-script hooks.

Usage::

    nia hooks list
    nia hooks test <event> [--for-tool X] [--payload-file F]
    nia hooks revoke <command>
    nia hooks doctor

Consent records live under ``~/.nia/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nia/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nia hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nia hooks {list|test|revoke|doctor}")
        print("Run 'nia hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nia/config.yaml.")
        print("See `nia hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nia hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nia hooks test` and `nia hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (NIA wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nia hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nia hooks doctor` would execute
