"""
Status command for nia CLI.

Shows the status of all NIA components.
"""
...
from nia_cli.auth import AuthError, resolve_provider
from nia_cli.colors import Colors, color
from nia_cli.config import get_env_path, get_env_value, get_nia_home, load_config
from nia_cli.models import provider_label
from nia_cli.nous_subscription import get_nous_subscription_features
from nia_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nia config``'s
...
def show_status(args):
    """Show status of all NIA components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ NIA Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: nia auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: nia model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: nia auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nia model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from nia_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'nia-agent[vercel]')"
    ...
    try:
        from nia_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_nia_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_nia_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'nia doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nia setup' to configure", Colors.DIM))
    print()
