"""
Interactive setup wizard for NIA.
...
from nia_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "Local Sovereign Hub/docs"
...
# display_nia_home imported lazily at call sites (stale-module safety during nia update)
...
from nia_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ NIA Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure NIA using environment variables or config commands:")
    print_info("  nia config set model.provider custom")
    print_info("  nia config set model.base_url http://localhost:8080/v1")
    print_info("  nia config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nia setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from nia_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nia_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nia setup')")


def _print_setup_summary(config: dict, nia_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nia setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from nia_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nia setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nia setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'nia setup terminal'"))
    ...
    # Spotify (OAuth via nia auth spotify — check auth.json, not env vars)
    try:
        from nia_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nia setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nia_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nia setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nia setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nia setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nia setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nia setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nia config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nia config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nia config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nia', Colors.GREEN)}              Start chatting")
    print(f"   {color('nia gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nia doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nia tools` and `nia setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nia model``)
    ...
    provider added to ``nia model`` is automatically available here.
    ...
    # Delegate to the shared nia model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nia_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: nia model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from nia_cli.auth_commands import auth_add_command
            ...
            print_info(
                "NIA can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one NIA can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'nia setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nia/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"nia setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nia setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where NIA runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'nia-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'nia-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NIA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NIA_MAX_ITERATIONS")
    ...
    print_info("  Run `nia setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NIA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where NIA delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nia update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: Local Sovereign Hub/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NIA_HOME, and print
    ...
    """
    try:
        from nia_cli.slack_cli import _build_full_manifest
        from nia_constants import get_nia_home

        manifest = _build_full_manifest(
            bot_name="NIA",
            bot_description="Your NIA on Slack",
        )
        target = Path(get_nia_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `nia slack manifest --write` anytime to refresh after "
            "NIA adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where NIA delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where NIA delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  nia config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects NIA to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nia_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: Local Sovereign Hub/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   Local Sovereign Hub/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nia config edit")
    print_info("   Open config in your editor:  nia config edit")
...
def setup_gateway(config: dict):
    ...
    from nia_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with NIA from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nia setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nia config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_nia_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  nia gateway")
        else:
            from nia_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nia gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nia gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nia setup tools` and `nia tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``nia_cli.auth`` — lists every supported
    ...
    """
    try:
        from nia_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nia_cli.config
    ...
    if section_key == "model":
        ...
        from nia_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_nia migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nia", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and NIA.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure NIA to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point NIA at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point NIA at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point NIA at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point NIA at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to NIA equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing NIA config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in NIA.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ NIA's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nia_home: Path) -> bool:
    ...
    print_info("NIA can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nia claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nia claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing NIA configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing NIA config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in NIA (use nia claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nia setup           — full or quick (auto-detected)
      nia setup model     — just model/provider
      nia setup tts       — just text-to-speech
      nia setup terminal  — just terminal backend
      nia setup gateway   — just messaging platforms
      nia setup tools     — just tool configuration
      nia setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ NIA Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ NIA Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your NIA installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nia_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have NIA configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nia setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nia_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up NIA?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, nia_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {nia_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nia config edit'")
    ...
    _print_setup_summary(config, nia_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch nia chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, nia_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nia setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nia setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nia setup gateway")
    print()
...
def _run_quick_setup(config: dict, nia_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nia setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect NIA to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nia setup gateway'.")
