...
Add, remove, or reorder entries here — both `nia setup` and
`nia` provider-selection will pick up the change automatically.
"""
...
_NIA_USER_AGENT = f"nia-cli/{_NIA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nia model`
    flow without maintaining a separate static list.
    """
    from nia_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NIA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NIA_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. NIA refreshes the cache from
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from nia_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  nia model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nia model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `nia model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Anonymousinsaan & Codurra Labs subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``nia model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``nia auth add openai-codex`` without
    ``nia model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nia-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nia_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nia-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nia_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from nia_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return nia-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; nia's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nia model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nia_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from nia_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to NIA' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nia_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether NIA should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nia auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nia/.env``.
    ...
    """
    try:
        from nia_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from nia_cli.auth import read_credential_pool
        from nia_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a NIA release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a NIA release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from nia_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NIA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: NIA' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NIA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_nia_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"NIA will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so NIA cannot verify the model name."
