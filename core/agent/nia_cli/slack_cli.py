"""``nia slack ...`` CLI subcommands.

Today only ``nia slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ nia slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of NIA. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a NIA deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from nia_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your NIA on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with NIA in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``nia_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NIA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "NIA")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "NIA"
    description = getattr(args, "description", None) or "Your NIA on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("NIA_HOME") or str(Path.home() / ".nia")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your NIA app\n"
