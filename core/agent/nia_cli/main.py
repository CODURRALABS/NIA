#!/usr/bin/env python3
"""
NIA CLI - Main entry point.

Usage:
    nia                     # Interactive chat (default)
    nia chat                # Interactive chat
    nia gateway             # Run gateway in foreground
    nia gateway start       # Start gateway as service
    nia gateway stop        # Stop gateway service
    nia gateway status      # Show gateway status
    nia gateway install     # Install gateway service
    nia gateway uninstall   # Uninstall gateway service
    nia setup               # Interactive setup wizard
    nia logout              # Clear stored authentication
    nia status              # Show status of all components
    nia cron                # Manage cron jobs
    nia cron list           # List cron jobs
    nia cron status         # Check if cron scheduler is running
    nia doctor              # Check configuration and dependencies
    nia honcho setup                    # Configure Honcho AI memory integration
    nia honcho status                   # Show Honcho config and connection status
    nia honcho sessions                 # List directory → session name mappings
    nia honcho map <name>               # Map current directory to a session name
    nia honcho peer                     # Show peer names and dialectic settings
    nia honcho peer --user NAME         # Set user peer name
    nia honcho peer --ai NAME           # Set AI peer name
    nia honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    nia honcho mode                     # Show current memory mode
    nia honcho mode [hybrid|honcho|local]  # Set memory mode
    nia honcho tokens                   # Show token budget settings
    nia honcho tokens --context N       # Set session.context() token cap
    nia honcho tokens --dialectic N     # Set dialectic result char cap
    nia honcho identity                 # Show AI peer identity representation
    nia honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    nia honcho migrate                  # Step-by-step migration guide: OpenClaw native → NIA + Honcho
    nia version             Show version
    nia update              Update to latest version
    nia uninstall           Uninstall NIA
    nia acp                 Run as an ACP server for editor integration
    nia sessions browse     Interactive session picker with search

    nia claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to NIA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (nia tools, nia setup, nia model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'nia {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any nia module import.
#
# Many modules cache NIA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NIA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.nia/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NIA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors nia_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If NIA_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("NIA_HOME"):
        return

    # 2. If no flag, check active_profile in the nia root
    if profile_name is None:
        try:
            from nia_constants import get_default_nia_root

            active_path = get_default_nia_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set NIA_HOME
    if profile_name is not None:
        try:
            from nia_cli.profiles import resolve_profile_env

            nia_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent nia from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["NIA_HOME"] = nia_home
...
# Load .env from ~/.nia/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from nia_cli.config import get_nia_home
from nia_cli.env_loader import load_nia_dotenv

load_nia_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → NIA_REDACT_SECRETS env
# var BEFORE nia_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "NIA_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_nia_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["NIA_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `nia` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from nia_logging import setup_logging as _setup_logging
...
try:
    from nia_cli.config import load_config as _load_config_early
    from nia_constants import apply_ipv4_preference as _apply_ipv4
...
from nia_cli import __version__, __release_date__
from nia_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from nia_cli.config import get_env_path, get_nia_home, load_config
    from nia_cli.auth import get_auth_status

    # Determine whether NIA itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from nia_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_nia_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from nia_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_nia_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if NIA has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants NIA to use their tokens.
    if _has_nia_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from nia_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, nia_bin
        cli_args: the original CLI arguments (everything after 'nia')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    nia_bin = container_info["nia_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo nia {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo nia {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  nia --tui --resume {target}")
    if title:
        print(f'  nia --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @nia/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@nia" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: NIA_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("NIA_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _nia_ink_bundle_stale(tui_dir):
        return True
...
def _nia_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "nia-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``NIA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("NIA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    nia_home = os.environ.get("NIA_HOME") or str(Path.home() / ".nia")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NIA_HOME": nia_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(nia_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (NIA_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("NIA_NODE")
    ...
    # pre-built dist + node_modules (nix / full NIA_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("NIA_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("NIA_QUIET"):
    ...
    if tui_dev:
        if _nia_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/nia-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nia_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="nia-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["NIA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["NIA_PYTHON_SRC_ROOT"] = os.environ.get(
        "NIA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("NIA_PYTHON", sys.executable)
    env.setdefault("NIA_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["NIA_MODEL"] = model
        env["NIA_INFERENCE_MODEL"] = model
    if provider:
        env["NIA_TUI_PROVIDER"] = provider
        env["NIA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["NIA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["NIA_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("NIA_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'nia sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like NIA isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  nia setup")
        print()

        from nia_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'nia setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from nia_cli.banner import prefetch_update_check
    ...
    # user's ~/.nia/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["NIA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["NIA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["NIA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from nia_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from nia_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with NIA?")
        print()
    ...
    session_dir = get_nia_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: nia gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  nia gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ NIA'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  nia gateway")
        ...
        print()
        print("  Or install as a service: nia gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'nia whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from nia_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``nia model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from nia_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from nia_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from nia_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("NIA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'nia model' for "
                "available providers, or run 'nia doctor' to diagnose config "
                "issues."
            )
    ...
    from nia_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from nia_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.nia/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nia/.env if the active provider is not 'custom'.
    ...
    """
    from nia_cli.config import get_env_value, save_env_value, load_config
...
#
# NIA uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `nia model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `nia model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  NIA only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``nia model`` flow, then route aux tasks to them here.
    """
    from nia_cli.config import load_config
    from nia_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from nia_cli.auth import _prompt_model_selection
    from nia_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from nia_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from nia_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from nia_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from nia_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from nia_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from nia_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `nia auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from nia_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from nia_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from nia_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from nia_cli.auth import _save_model_choice, deactivate_provider
    from nia_cli.config import get_env_value, load_config, save_config
    ...
    from nia_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `nia model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from nia_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from nia_cli.config import get_env_value, save_env_value, load_config, save_config
    from nia_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  NIA will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from nia_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from nia_cli.config import save_env_value, load_config, save_config
    from nia_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from nia_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from nia_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates NIA turns to `copilot --acp`.")
    print("  NIA currently starts its own ACP subprocess for each request.")
    print("  NIA uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set NIA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so nia knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   NIA typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with NIA, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from nia_cli.auth import AuthError
        from nia_cli.models import fetch_lmstudio_models
        ...
        from nia_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from nia_constants import display_nia_home as _dhh_fn

            print(
                f"    NIA will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from nia_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'nia model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate NIA CLI with a provider."""
    from nia_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nia_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nia_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nia_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nia_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from nia_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``nia slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: nia slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `nia slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from nia_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nia_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from nia_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from nia_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from nia_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nia_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nia_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up NIA home directory to a zip file."""
    if getattr(args, "quick", False):
        from nia_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nia_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a NIA backup from a zip file."""
    from nia_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"NIA v{__version__} ({__release_date__})")
    ...
    try:
        from nia_cli.banner import check_for_updates
        from nia_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from nia_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``nia update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from nia_constants import get_nia_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``nia_cli/web_dist/`` (per vite.config.ts
    outDir: "../nia_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "nia_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``nia update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``nia web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (nia web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``nia dashboard`` processes other than ourselves.

    ``nia dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``nia update`` replaces files on disk, the running
    ...
    patterns = [
        "nia dashboard",
        "nia_cli.main dashboard",
        "nia_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "nia.*dashboard"`) keeps us consistent
            # with `nia_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `nia update`.
    ...
    print("  Preview now:  nia curator run --dry-run")
    print("  Pause it:     nia curator pause")
    print("  Docs:         Local Sovereign Hub/docs/user-guide/features/curator")


def _kill_stale_dashboard_processes(
    reason: str = "the running backend no longer matches the updated frontend",
) -> None:
    """Kill running ``nia dashboard`` processes.

    Called at the end of ``nia update`` (default ``reason``) and also
    from ``nia dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    """Update NIA by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"Local Sovereign Environment/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="nia-update-")
        zip_path = os.path.join(tmp_dir, f"nia-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to nia-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"nia-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "nia-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if NIA behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes nia completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but NIA couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but NIA couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if NIA behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `nia update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "Local Sovereign Entity",
    "Sovereign Core",
}
OFFICIAL_REPO_URL = "Local Sovereign Entity"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_nia_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_nia_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official NIA repository.")
        print("  This means you may miss updates from NousResearch/nia-agent.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: Local Sovereign Environment.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream Local Sovereign Environment.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``nia update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from nia_constants import get_default_nia_root

    default_home = get_default_nia_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "nia-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``nia update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.nia/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``nia update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``nia update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.nia/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``nia update --gateway``) the update is already
    ...
    try:
        ...
        # nia_cli.config.get_nia_home to simulate setup failure.
        from nia_cli.config import get_nia_home as _get_nia_home

        logs_dir = _get_nia_home() / "logs"
...
def _cmd_update_check():
    """Implement ``nia update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``nia update`` without requiring a reinstall.  The
    ...
    ``nia`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/nia.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v nia'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/nia, code at /usr/local/lib/nia-agent).
    fhs_link = Path("/usr/local/bin/nia")
    ...
    path_comment = (
        "# NIA — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of NIA_HOME before running the update.
    ...
    NIA_HOME directories.  The ``--backup`` flag on ``nia update``
    ...
    try:
        from nia_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_nia_home so the user sees ~/.nia/...
    try:
        from nia_constants import get_nia_home, display_nia_home
        home = get_nia_home()
        try:
            display_path = f"{display_nia_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  nia import {out_path}")
...
def cmd_update(args):
    """Update NIA to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from nia_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update NIA")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_nia_home added to nia_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload nia_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_nia_home() added since the last release.
        try:
            import importlib
            import nia_constants as _hc
        ...
        if needs_migration:
            ...
            if assume_yes:
                ...
                print(
                    "    Run 'nia config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if (gateway_mode or assume_yes) and missing_env:
                    print("  ℹ API keys require manual entry: nia config migrate")
            else:
                print()
                print("Skipped. Run 'nia config migrate' later to configure.")
        else:
        ...
        # When running as ``nia update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_nia_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all nia-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. nia-gateway.service or nia-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: nia gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: nia -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy NIA gateway unit files are still installed.
        # When both nia.service (from a pre-rename install) and the
        # current nia-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `nia update` surfaces the issue until the user migrates.
        try:
            from nia_cli.gateway import (
                has_legacy_nia_units,
                _find_legacy_nia_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_nia_units():
                print()
                print("⚠ Legacy NIA gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_nia_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (nia.service) fight the current")
                print("  nia-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    nia gateway migrate-legacy")
                print()
        ...
        print("  nia model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``nia -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from nia_constants import display_nia_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `nia profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_nia_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → nia -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.nia)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  nia profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     nia -p {name} chat")
                else:
        ...
        from nia_cli.profiles import profile_exists
        ...
        from nia_cli.profiles import rename_profile
        ...
        from nia_cli.profiles import export_profile
        ...
        from nia_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``nia dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``nia dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No nia dashboard processes running.")
        return 0

    print(f"{len(pids)} nia dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No nia dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `nia update`.
    ...
    if "NIA_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from nia_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("NIA_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from nia_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter NIA log files."""
    from nia_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for nia CLI."""
    from nia_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: nia-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from nia_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Local Sovereign Hub/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `nia model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy nia.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy NIA gateway unit files "
            "(e.g. nia.service) left over from older installs. Profile "
            "units (nia-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure NIA with an interactive wizard. "
        "Run a specific section: nia setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'nia setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for NIA.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $NIA_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "NIA")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for NIA CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: nia-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate NIA with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set NIA_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of NIA components",
    )
    ...
    # =========================================================================
    from nia_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.nia/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.nia/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with NIA setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your NIA setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for NIA. Use 'nia debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nia debug share              Upload debug report and print URL
    nia debug share --lines 500  Include more log lines
    nia debug share --expire 30  Keep paste for 30 days
    nia debug share --local      Print report locally (no upload)
    nia debug share --no-redact  Disable upload-time secret redaction
    nia debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'nia debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up NIA home directory to a zip file",
        description="Create a zip archive of your entire NIA configuration, "
        "skills, sessions, and data (excludes the nia-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/nia-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a NIA backup from a zip file",
        description="Extract a previously created NIA backup into your "
        "NIA home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage NIA configuration",
    )
    ...
    def cmd_pairing(args):
        from nia_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.nia/skills/.bundled_manifest) "
            "so future 'nia update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from nia_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from nia_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/nia-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `nia plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from nia_cli.plugins_cmd import plugins_command
    ...
    try:
        from nia_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_nia_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_nia_home()}/memories/\n")
        else:
            from nia_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'nia tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # nia tools disable <name...> [--platform cli]
    ...
    # nia tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from nia_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from nia_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run NIA as an MCP server",
        description=(
            "Manage MCP server connections and run NIA as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'nia mcp add' to connect to a new server, or\n"
            "'nia mcp serve' to expose NIA conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run NIA as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from nia_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_nia_home() / "sessions"
            ...
            # Launch nia --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from nia_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to NIA",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to NIA",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.nia/ (by default a "
        "single restore-point archive is written to ~/.nia/backups/ "
        "before apply; restorable with 'nia import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from nia_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update NIA to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'nia config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall NIA",
        description="Remove NIA from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run NIA as an ACP (Agent Client Protocol) server",
        description="Start NIA in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch NIA as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated NIA instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the NIA web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `nia --tui` via PTY/WebSocket). "
            "Alternatively set NIA_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `nia dashboard` cmdlines and SIGTERM them directly — the same
    # path `nia update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running nia dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running nia dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter NIA log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nia logs                    Show last 50 lines of agent.log
    nia logs -f                 Follow agent.log in real time
    nia logs errors             Show last 50 lines of errors.log
    nia logs gateway -n 100     Show last 100 lines of gateway.log
    nia logs --level WARNING    Only show WARNING and above
    nia logs --session abc123   Filter by session ID
    nia logs --component tools  Only show tool-related lines
    nia logs --since 1h         Lines from the last hour
    nia logs --since 30m -f     Follow, starting from 30 min ago
    nia logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``nia -c Pokemon Agent Dev`` → ``nia -c 'Pokemon Agent Dev'``
    ...
    from nia_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'nia -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (nia hooks list, cron
    ...
    if (
        ...
        try:
            from nia_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
    ...
    if getattr(args, "oneshot", None):
        from nia_cli.oneshot import run_oneshot
