"""
Dump command for nia CLI.

Outputs a compact, plain-text summary of the user's NIA setup
...
from nia_cli.config import get_nia_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``nia dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nia_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(nia_home: Path) -> int:
...
def _cron_summary(nia_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nia_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nia_home:      {display_nia_home()}")
    ...
    toolsets = config.get("toolsets", ["nia-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nia_home)}")
    lines.append(f"  skills:             {_count_skills(nia_home)}")
