"""Slash command definitions and autocomplete for the NIA CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of NIA config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal NIA works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while NIA is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nia/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update NIA to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nia_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nia`` subcommand mapping, and
    ...
    """
    try:
        from nia_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nia skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nia skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nia skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nia <verb>`` subcommand.
    ...
    can still reach them via ``/nia <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nia`` is always reserved as the first entry so the
    legacy ``/nia <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nia as the catch-all top-level command.
    entries.append(("nia", "Talk to NIA or run a subcommand", "[subcommand] [args]"))
    seen.add("nia")
...
def slack_app_manifest(request_url: str = "https://nia-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nia handler.

    Maps both canonical names and aliases so /nia bg do stuff works
    the same as /nia background do stuff.

    Plugin-registered slash commands are included so ``/nia <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nia_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
