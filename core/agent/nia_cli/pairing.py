...
Usage:
    nia pairing list              # Show all pending + approved users
    nia pairing approve <platform> <code>  # Approve a pairing code
    nia pairing revoke <platform> <user_id> # Revoke user access
    nia pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle nia pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: nia pairing {list|approve|revoke|clear-pending}")
        print("Run 'nia pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print("  Run 'nia pairing list' to see pending codes.\n")
