"""
Unified self-relaunch for NIA CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``nia sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``nia`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched nia."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_nia_bin() -> Optional[str]:
    """Find the nia entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("nia")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m nia_cli.main``.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with nia.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_nia_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "nia_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh nia invocation."""
