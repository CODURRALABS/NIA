"""nia claw — OpenClaw migration commands.

Usage:
    nia claw migrate              # Preview then migrate (always shows preview first)
    nia claw migrate --dry-run    # Preview only, no changes
    nia claw migrate --yes        # Skip confirmation prompt
    nia claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    nia claw migrate --no-backup  # Skip pre-migration snapshot
    nia claw cleanup              # Archive leftover OpenClaw directories
    nia claw cleanup --dry-run    # Preview what would be archived
"""
...
from nia_cli.config import get_nia_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "NIA may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a NIA gateway is running with connected platforms.
    ...
    print()
    print_error(
        "NIA gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'nia stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_nia.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_nia", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: nia claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to NIA")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'nia claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → NIA migration."""
    ...
    # migrate-nia posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: nia claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.nia/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: nia claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the NIA home ─────────
    # Delegates to nia_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `nia import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from nia_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(nia_home=nia_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: nia import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the NIA home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: nia claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: nia claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  nia claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  nia claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  nia config set OPENROUTER_API_KEY sk-or-v1-...")
