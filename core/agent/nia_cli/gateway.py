"""
Gateway subcommand for nia CLI.

Handles: nia gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_nia_home is imported lazily at call sites to avoid ImportError
# when nia_constants is cached from a pre-update version during `nia update`.
from nia_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from nia_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "nia-gateway*",
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``nia gateway status`` from falsely counting the ``nia`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``nia gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "nia_cli.main gateway",
        "nia_cli.main --profile",
        "nia_cli.main -p",
        "nia_cli/main.py gateway",
        "nia_cli/main.py --profile",
        "nia_cli/main.py -p",
        "nia gateway",
        "gateway/run.py",
    ]
    current_home = str(get_nia_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"NIA_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "NIA_HOME=" in command and f"NIA_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``nia update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            NIA profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to NIA profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from nia_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "nia_cli.main"]
    if profile != "default":
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}nia gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from nia_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `nia gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``nia gateway status`` output so users with
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (NIA_HOME-scoped).
...
from nia_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "nia-gateway"
SERVICE_DESCRIPTION = "NIA Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current NIA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (NIA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from nia_constants import get_default_nia_root
    home = get_nia_home().resolve()
    default = get_default_nia_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary NIA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(nia_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when NIA_HOME is a named profile.

    For ``~/.nia/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        nia_home: Optional explicit NIA_HOME path. Defaults to the current
            ``get_nia_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from nia_constants import get_default_nia_root
    home = Path(nia_home or str(get_nia_home())).resolve()
    default = get_default_nia_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this NIA_HOME.

    Default ``~/.nia`` returns ``nia-gateway`` (backward compatible).
    Profile ``~/.nia/profiles/coder`` returns ``nia-gateway-coder``.
    Any other NIA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older NIA installs that predate the
# nia-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (nia-gateway-*.service) and unrelated third-party
# "nia" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("nia.service",)
...
def _find_legacy_nia_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy NIA gateway units.

    Detects unit files installed by older NIA versions that used a
    different service name (e.g. ``nia.service`` before the rename to
    ``nia-gateway.service``). When both a legacy unit and the current
    ``nia-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``nia-gateway-coder.service`` and unrelated third-party
      ``nia-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``nia.service`` running an unrelated
...
def has_legacy_nia_units() -> bool:
    """Return True when any legacy NIA gateway unit files exist."""
    return bool(_find_legacy_nia_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy NIA gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_nia_units()
    if not legacy:
        return
    print_warning("Legacy NIA gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current nia-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    nia gateway migrate-legacy")


def remove_legacy_nia_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy NIA gateway unit files.

    Iterates over whatever ``_find_legacy_nia_units()`` returns — which is
    ...
    legacy = _find_legacy_nia_units()
    if not legacy:
        print("No legacy NIA gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy NIA gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: nia gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo nia gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    nia gateway uninstall")
    print_info("    sudo nia gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so NIA can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo nia gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo nia gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo nia gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode NIA often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.nia`` → ``ai.nia.gateway.plist`` (backward compatible).
    Profile ``~/.nia/profiles/coder`` → ``ai.nia.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.nia.gateway-{suffix}" if suffix else "ai.nia.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.nia/nia-agent  -> /home/alice/.nia/nia-agent
      /opt/nia                 -> /opt/nia  (kept as-is)
...
def _nia_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NIA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_nia_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.nia                    → /home/alice/.nia
      /root/.nia/profiles/coder     → /home/alice/.nia/profiles/coder
      /opt/custom-nia               → /opt/custom-nia  (kept as-is)
    """
    current_nia = get_nia_home().resolve()
    current_default = (Path.home() / ".nia").resolve()
    target_default = Path(target_home_dir) / ".nia"

    # Default ~/.nia → remap to target user's default
    if current_nia == current_default:
        return str(target_default)

    # Profile or subdir of ~/.nia → preserve the relative structure
    try:
        relative = current_nia.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.nia) — keep as-is
        return str(current_nia)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        nia_home = _nia_home_for_target_user(home_dir)
        profile_arg = _profile_arg(nia_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m nia_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    nia_home = str(get_nia_home().resolve())
    profile_arg = _profile_arg(nia_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current NIA install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (nia.service from pre-rename installs)
    # before installing the new nia-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}nia gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}nia gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}nia gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `nia gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}nia gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}nia gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}nia gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.nia.gateway-{suffix}" if suffix else "ai.nia.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current NIA install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  nia gateway status             # Check status")
    from nia_constants import display_nia_home as _dhh
...
def launchd_stop():
    ...
    # `nia gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current NIA install")
    else:
        print("⚠ Service definition is stale relative to the current NIA install")
        print("  Run: nia gateway start")
    ...
    if deep:
        log_file = get_nia_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # /restart) rather than through `nia gateway restart` which already
    ...
    # `nia gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and NIA will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @nia:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. nia) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where NIA delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your NIA",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address NIA will use (e.g., nia@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. NIA connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: nia pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and NIA will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``nia setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.nia/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_nia_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'nia pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: nia pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'nia gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from nia_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from nia_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. NIA will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. NIA will store the returned account_id/token in ~/.nia/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `nia gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_nia_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `nia pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `nia pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"NIAAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``nia_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from nia_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.nia/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.nia/.env: {', '.join(required)}")
    else:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: nia gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: nia gateway install")
                else:
                    print_info("  You can install later: nia gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo nia gateway install --system")
                    print_info("  Or run in foreground:  nia gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: nia gateway run")
                print_info("  For persistence:   tmux new -s nia 'nia gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: nia gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup nia gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: nia gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'nia gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: nia gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: nia gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s nia 'nia gateway run'")
                print()
            ...
            print()
            print("  nia gateway run                              # direct foreground")
            print("  tmux new -s nia 'nia gateway run'         # persistent via tmux")
            print("  nohup nia gateway run > ~/.nia/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: nia gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: nia gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: nia gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: nia gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: nia gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  nia gateway install")
                    print("  sudo nia gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  nia gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup nia gateway run > ~/.nia/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s nia 'nia gateway run'         # persistent via tmux")
                    print("  nohup nia gateway run > ~/.nia/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  nia gateway install  # Install as user service")
                    print("  sudo nia gateway install --system  # Install as boot-time system service")

        # Show other profiles' gateway status for multi-profile awareness
        _print_other_profiles_gateway_status()

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy NIA gateway unit files from
        # pre-rename installs (e.g. nia.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_nia_units(interactive=not yes, dry_run=dry_run)
