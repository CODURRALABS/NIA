...
``<root>`` is the **shared NIA root** (the parent of any active
...
``nia -p <profile>`` joins the same board as the dispatcher that
...
the profile's NIA home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NIA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NIA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nia kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nia``. In Docker / custom
deployments where ``NIA_HOME`` points outside ``~/.nia`` (e.g.
``/opt/nia``), ``<root>`` is ``NIA_HOME``. Legacy env-var
overrides still work:

* ``NIA_KANBAN_DB`` — pin the database file path directly.
* ``NIA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NIA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NIA_KANBAN_DB``,
``NIA_KANBAN_WORKSPACES_ROOT``, and ``NIA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nia-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``nia-agent``
...
def kanban_home() -> Path:
    """Return the shared NIA root that anchors the kanban board.

    Resolution order:

    1. ``NIA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_nia_root()``, which already returns ``<root>``
       when ``NIA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NIA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NIA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NIA_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nia kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NIA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nia kanban boards
       switch``).
    ...
    hand-edited a file.
    """
    env = os.environ.get("NIA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nia kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NIA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NIA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NIA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NIA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nia kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NIA_KANBAN_DB`` env → ``NIA_KANBAN_BOARD`` env →
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nia kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nia_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``nia --skills ...`` alongside the built-in
    ...
    # `nia --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nia kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nia kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``nia kanban complete <id>``) works without requiring
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nia kanban
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nia -p <profile> chat -q ...`` subprocess.
    ...
    ``NIA_KANBAN_DB`` / ``NIA_KANBAN_BOARD`` / workspaces_root env
    ...
    env = dict(os.environ)
    if task.tenant:
        env["NIA_TENANT"] = task.tenant
    env["NIA_KANBAN_TASK"] = task.id
    env["NIA_KANBAN_WORKSPACE"] = workspace
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`nia -p <name>`
    # rewrites NIA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_nia_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NIA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NIA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NIA_KANBAN_BOARD"] = resolved_board
    # NIA_PROFILE is the author the kanban_comment tool defaults to.
    # `nia -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NIA_PROFILE"] = profile_arg
    ...
    # `nia kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nia` executable not found on PATH. "
            "Install NIA or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nia kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.nia/profiles/`` directly so this module has no import
    dependency on ``nia_cli.profiles`` (which pulls in a large chunk
    ...
    try:
        from nia_constants import get_default_nia_root
        home = get_default_nia_root() / "profiles"
    except Exception:
        return []
    if not home.is_dir():
        return []
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nia kanban assignees`` for the terminal.
