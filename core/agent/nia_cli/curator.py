"""CLI subcommand: `nia curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `nia curator status` later")
    if dry:
        print(
            "dry-run: no changes applied. When the report lands, read it with "
            "`nia curator status` and run `nia curator run` (no flag) to apply."
        )
    return 0
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.nia/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`nia curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.nia/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from nia_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.nia/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.nia/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by nia_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="nia curator")
    register_cli(parser)
