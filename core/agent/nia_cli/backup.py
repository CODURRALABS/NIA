"""
Backup and import commands for nia CLI.

`nia backup` creates a zip archive of the entire ~/.nia/ directory
(excluding the nia-agent repo and transient files).

`nia import` restores from a backup zip, overlaying onto the current
NIA_HOME root.
"""
...
from nia_constants import get_default_nia_root, get_nia_home, display_nia_home
...
_EXCLUDED_DIRS = {
    "nia-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nia root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the NIA home directory."""
    nia_root = get_default_nia_root()

    if not nia_root.is_dir():
        print(f"Error: NIA home directory not found at {nia_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nia-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nia-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_nia_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(nia_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nia_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nia_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside nia root
            try:
    ...
    print(f"\nRestore with: nia import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a NIA backup.
    ...
    # Look for telltale files that a nia home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a NIA backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nia/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nia dir name
        if prefix in (".nia", "nia"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nia_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nia_root / "config.yaml").exists()
        has_env = (nia_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has NIA configuration.")
        ...
        nia_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(nia_root.resolve())
            except ValueError:
        ...
        profiles_dir = nia_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # nia_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: nia profile list  (after installing nia)")

        # Guidance
        print()
        if not (nia_root / "nia-agent").is_dir():
            print("Note: The nia-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nia update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nia -p {pname} gateway install")

        print("Done. Your NIA configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nia backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NIA_HOME).
...
# — `nia update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(nia_home: Optional[Path] = None) -> Path:
    home = nia_home or get_nia_home()
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nia_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nia_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for nia backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_nia_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, nia_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nia_root`` to ``out_path``.
...
def _pre_update_backup_dir(nia_home: Optional[Path] = None) -> Path:
    home = nia_home or get_nia_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NIA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NIA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nia update``) should continue even if the backup fails.
    """
    nia_root = nia_home or get_default_nia_root()
    if not nia_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nia_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nia_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nia claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NIA_HOME under ``backups/`` before a
    ``nia claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nia import <archive>``.  Writes to
    ``<NIA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nia import` and the
