"""Shell completion script generation for nia CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nia_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_nia() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_nia_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# NIA fish completion",
        "# Add to your config:",
        "#   nia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nia_profiles",
        "    echo default",
        "    if test -d $HOME/.nia/profiles",
        "        ls $HOME/.nia/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nia -f -s p -l profile"
        " -d 'Profile name' -xa '(__nia_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nia_profiles)' -d 'Profile name'"
                )
