...
Usage:
    python -m nia_cli.main web          # Start on http://127.0.0.1:9119
    python -m nia_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["NIA_WEB_DIST"]) if "NIA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="NIA", version=__version__)
...
_SESSION_HEADER_NAME = "X-NIA-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``nia dashboard --tui``
# or NIA_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.nia/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_nia_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "nia-update": "nia-update.log",
}
...
def _spawn_nia_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``nia <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``nia_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "nia_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "NIA_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``nia gateway restart`` in the background."""
    try:
        proc = _spawn_nia_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/nia/update")
async def update_nia():
    """Kick off ``nia update`` in the background."""
    try:
        proc = _spawn_nia_action(["update"], "nia-update")
    except Exception as exc:
        _log.exception("Failed to spawn nia update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    NIA supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in nia_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.nia/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``nia auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    NIA resolves Anthropic creds in this order at runtime:
    1. ``~/.nia/.anthropic_oauth.json`` — NIA-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_nia_oauth_credentials,
            read_claude_code_credentials,
            _NIA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_nia_oauth_credentials = None  # type: ignore
        _NIA_OAUTH_FILE = None  # type: ignore

    nia_creds = None
    if read_nia_oauth_credentials:
        try:
            nia_creds = read_nia_oauth_credentials()
        except Exception:
            nia_creds = None
    if nia_creds and nia_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "nia_pkce",
            "source_label": f"NIA PKCE ({_NIA_OAUTH_FILE})",
            "token_preview": _truncate_token(nia_creds.get("accessToken")),
            "expires_at": nia_creds.get("expiresAt"),
            "has_refresh_token": bool(nia_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into NIA even
    when they also have a separate NIA-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "nia auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nia auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "nia auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "nia auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from nia_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("nia_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same NIA-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _NIA_OAUTH_FILE
            if _NIA_OAUTH_FILE.exists():
                _NIA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from nia_cli.auth import clear_provider_auth
...
#          → persists to ~/.nia/.anthropic_oauth.json AND credential pool
...
# Guarded so nia web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both NIA file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``nia auth add anthropic``.
    """
    from agent.anthropic_adapter import _NIA_OAUTH_FILE
    payload = {
    ...
    _NIA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _NIA_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from nia_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("NIA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from nia_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from nia_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from nia_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_nia_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from nia_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_nia_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from nia_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "nia setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from nia_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from nia_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``nia --tui`` binary the CLI uses, behind
...
from nia_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``nia --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``NIA_TUI_RESUME`` env var —
    matching what ``nia_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``NIA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from nia_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["NIA_TUI_RESUME"] = resume

    if sidecar_url:
        env["NIA_TUI_SIDECAR_URL"] = sidecar_url
...
# NIA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__NIA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__NIA_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "NIA Teal",  "description": "Classic dark teal — the canonical NIA look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.nia/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.nia/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_nia_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.nia/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as nia_cli.plugins):
    1. User plugins:    ~/.nia/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.nia/plugins/  (only if NIA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from nia_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_nia_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("NIA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".nia" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from nia_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_nia_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from nia_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from nia_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nia_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nia_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"nia_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  NIA Web UI → http://{host}:{port}")
