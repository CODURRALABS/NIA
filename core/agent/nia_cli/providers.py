"""
Single source of truth for provider identity in NIA.
...
2. **NIA overlays** — transport type, auth patterns, aggregator flags,
...
# -- NIA overlay ----------------------------------------------------------
# NIA-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class NIAOverlay:
    """NIA-specific provider metadata layered on top of models.dev."""
...
NIA_OVERLAYS: Dict[str, NIAOverlay] = {
    "openrouter": NIAOverlay(
    ...
    "openai-codex": NIAOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": NIAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NIA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": NIAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": NIAOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": NIAOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": NIAOverlay(
    ...
    "alibaba-coding-plan": NIAOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "nia", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. NIA overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + NIA overlay
    ...
    overlay = NIA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + nia extra
    ...
    if overlay is not None:
        # NIA-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in NIA_OVERLAYS
    if provider == "bedrock":
