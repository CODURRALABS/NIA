"""
Profile management for multiple isolated NIA instances.

Each profile is a fully independent NIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nia/profiles/<name>/`` by default.

The "default" profile is ``~/.nia`` itself — backward compatible,
zero migration needed.

Usage::

    nia profile create coder          # fresh profile + bundled skills
    nia profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nia profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nia -p coder chat                 # or via flag
    nia profile use coder             # set as sticky default
    nia profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nia_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.nia`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.nia) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nia-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "nia", "default", "test", "tmp", "root", "sudo",
})

# NIA subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nia root, NOT to the current NIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NIA_HOME points outside
    ``~/.nia``, profiles live under ``NIA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nia_home() / "profiles"


def _get_default_nia_home() -> Path:
    """Return the default (pre-profile) NIA_HOME path.

    In standard deployments this is ``~/.nia``.
    In Docker/custom deployments where NIA_HOME is outside ``~/.nia``
    (e.g. ``/opt/data``), returns NIA_HOME directly.
    """
    from nia_constants import get_default_nia_root
    return get_default_nia_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_nia_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NIA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nia_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nia subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NIA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nia subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_nia_home()
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nia)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nia/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NIA_HOME at module level.
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nia).\n"
            "To remove everything, use: nia uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NIA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NIA_HOME")
    try:
        os.environ["NIA_HOME"] = str(profile_dir)
        from nia_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NIA_HOME"] = old_home
        elif "NIA_HOME" in os.environ:
            del os.environ["NIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nia/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nia profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NIA_HOME.

    Returns ``"default"`` if NIA_HOME is not set or points to ``~/.nia``.
    Returns the profile name if NIA_HOME points into ``~/.nia/profiles/<name>``.
    Returns ``"custom"`` if NIA_HOME is set to an unrecognized path.
    """
    from nia_constants import get_nia_home
    nia_home = get_nia_home()
    resolved = nia_home.resolve()

    default_resolved = _get_default_nia_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nia itself — its parent is ~/ and its
        # directory name is ".nia", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.nia itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nia). "
            "Specify a different name: nia profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nia.{old_name}"
    new_host = f"nia.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nia_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for nia profile names."""
    return '''# NIA profile completion
# Add to ~/.bashrc: eval "$(nia completion bash)"

_nia_profiles() {
    local profiles_dir="$HOME/.nia/profiles"
...
_nia_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nia_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nia_completion nia
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for nia profile names."""
    return '''#compdef nia
# NIA profile completion
# Add to ~/.zshrc: eval "$(nia completion zsh)"

_nia() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nia/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.nia/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NIA_HOME path string.

    Called early in the CLI entry point, before any nia modules
    are imported, to set the NIA_HOME environment variable.
    """
