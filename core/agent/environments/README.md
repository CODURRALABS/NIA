# nia-agent Atropos Environments

This directory contains the integration layer between **nia-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  NIAAgentBaseEnv    │  nia_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     NIASweEnv    TerminalBench2EvalEnv
...
**NIAAgentBaseEnv** (`nia_base_env.py`) extends BaseEnv with nia-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves nia-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `NIAAgentBaseEnv` and implement:
...
`NIAAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as nia-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** nia-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any nia-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some nia-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `nia` -- NIA/ChatML `<tool_call>` XML format
...
├── nia_base_env.py            # Abstract base (NIAAgentBaseEnv)
...
│
├── nia_swe_env/               # SWE-bench style training environment
│   └── nia_swe_env.py
│
...
2. Create your env file inheriting from `NIAAgentBaseEnv`
...
2. Inherit from `NIAAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which nia toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `nia` |
