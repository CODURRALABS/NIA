"""
NIAAgentBaseEnv -- Abstract Base Environment for nia-agent + Atropos

Provides the Atropos integration plumbing that all nia-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via NIAAgentLoop
...
# Ensure the nia-agent repo root is on sys.path so that imports like
...
# Load API keys from nia-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, NIAAgentLoop
...
# Import nia-agent toolset infrastructure
...
class NIAAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for nia-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of nia toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: nia, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/nia-results/ "
...
class NIAAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for nia-agent Atropos integration.
    ...
    name: Optional[str] = "nia-agent"
    env_config_cls = NIAAgentEnvConfig

    def __init__(
        self,
        config: NIAAgentEnvConfig,
        ...
        # Set terminal environment variables so nia tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY nia-agent tool (terminal, file, web,
