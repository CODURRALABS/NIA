...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs NIAAgentLoop with terminal tool only
...
Requires: pip install nia-agent[yc-bench]
"""
...
class YCBenchEvalConfig(NIAAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends NIAAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(NIAAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The NIAAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs NIAAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our NIAAgentLoop.
