...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from NIA"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from NIA",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from NIA",
    },
    {
...
class TerminalTestEnv(NIAAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/nia-agent/.env.
        """
