"""
NIAAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the nia-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to nia-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by NIAAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by NIAAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class NIAAgentLoop:
    """
    Runs nia-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # nia-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("nia")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via nia-agent's dispatch
